...
The hardcoded `claude` authoring agent is replaced with a configurable agent command. An agent-command picker is surfaced in the schedule Add/Edit form. The default value is the `default_command` from your configuration; any bare name or full path is accepted in the Command field.
