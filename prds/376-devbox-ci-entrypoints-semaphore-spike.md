...
**The cache is now treated as an untrusted input.** Content-derived keys authenticate content, not producers. A same-repository branch that leaves the lockfiles alone derives `main`'s keys while running its own `ci.sh`, and Semaphore's cache CLI is project-wide. So: one namespace, written only on a push to the default branch; branch and PR runs restore read-only and store nothing (stricter than a per-ref namespace, and it keeps per-branch multi-GB archives from LRU-evicting the shared ones); restores unpack into a scratch directory and only an allowlisted member is moved into place; no prefix-fallback restores. The file says plainly that the ref check is defence in depth and not an authorization boundary, since the untrusted ref controls the script performing the check. `SEMAPHORE_GIT_REF_TYPE` is checked, not just `SEMAPHORE_GIT_BRANCH` — the latter is the *target* branch on a PR run, so branch-only logic would classify every PR against `main` as trusted.
