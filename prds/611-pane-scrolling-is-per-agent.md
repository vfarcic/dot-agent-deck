...
- **M3's carry-over is threaded through `process_agent_output_chunk`, not parked on the pane.** Resolved in M2 (was an Open Question), and implemented in M3 rather than here. That function exists precisely so *"both backends produce identical render state from identical bytes"*, and a carry-over buffer is exactly such a derivation — parking it on the pane is more local but has to be remembered separately for each backend, which is the failure mode the shared function was created to prevent. The widened signature is the acknowledged cost, and M2 already paid the same one for the same reason: the since-spawn byte counter that feeds the trigger above is incremented in that function, not at its caller.
...
- [x] **M2 — The deck says so on screen instead of no-opping silently.** A failed scroll now overlays the pane with `SCROLL_NOTICE_TEXT` — one centred reversed line in the command banner's own idiom, through the same `draw_centred_banner_line`, degrading to a short form on a pane too narrow for the sentence and omitted below that. Armed only inside `scroll_focused_pane_scrollback`, so nothing but a real attempt can fire it; cleared at the top of `handle_key_event`, before the key is classified, so dismissal cannot consume anything. The trigger is a conjunction of live vt100 depth 0 and `SCROLL_NOTICE_MIN_SCREENFULS` screenfuls fed since spawn, and it names no agent. State lives on `Pane` behind `EmbeddedPaneController`; the byte counter is fed by `process_agent_output_chunk`, the shared path both backends derive render state through. `mode/scroll/001`, `/002` and `/005` in `tests/mode_indication.rs` cover it, with their `tests/CATALOG.md` entries.
- [x] **M3 — Derive mouse state correctly (`scan_mouse_mode`).** The substring search over four fixed patterns is now a real scan of `ESC[?<params><final>` sequences: parameters are parsed as whole numbers (so the combined `ESC[?1000;1002;1006h` is detected and `ESC[?11000h` still is not), directives are applied in byte order as they are met, and the trailing bytes that could still open a sequence carry into the next chunk under `MOUSE_SCAN_CARRY_MAX` (64) so a stream that opens `ESC[?` and never terminates it cannot grow the buffer. 1004 is not among the tracked modes, so codex's focus reporting reads as what it is. Each mode is an independent latch and the flag is `any()` over the four — withdrawing one of them no longer reads as withdrawing all of them, which is the claude case the Risk section names. State lives in `MouseModeScanner`, threaded through `process_agent_output_chunk` beside `Osc8Filter` per the Decision above. 20 unit tests in `src/embedded_pane.rs`'s `mod tests`, 11 of which fail against the pre-fix function; the terminal-managed case reuses `synthetic_decstbm_repaint_stream` rather than copying it.
