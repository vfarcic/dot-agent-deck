...
The GUI is a separate, opt-in binary. A user who wants it builds/installs it deliberately; it is not part of the default release artifacts and does not replace `dot-agent-deck`. On launch it connects to the running daemon over the same socket the TUI uses (auto-starting the daemon per the existing always-external-daemon rules, PRD #93). If no daemon is reachable it shows a clear connect/retry state.

### What it shows

- **Decks and tabs** as GUI-native navigation (sidebar / tab bar), mirroring the TUI's Mode vs Orchestration bucketing from `AgentRecord.tab_membership`.
...
- [ ] **M2.2** — Pane lifecycle from the GUI: `StartAgent`, `StopAgent`, `SetAgentLabel`, `Resize`, `WriteAndSubmit`; pane-status surface (running / waiting-for-input / finished) driven by `SubscribeEvents`.
