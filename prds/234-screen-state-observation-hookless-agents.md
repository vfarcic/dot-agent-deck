...
- **A readiness source for [PRD #225](done/225-wrapper-agent-readiness-and-stable-launch-shape.md)'s hookless case** — `wait_for_session_start`'s gate can consult observed readiness for agents whose registry spec has no native hooks, instead of falling through to the timeout.
...
- [ ] **M4 — `is_ready` predicate + readiness gate integration.** Input-box presence, combined with stability, consulted by the delegate gate for agents whose registry spec has no native hooks. Coordinated with [PRD #225](done/225-wrapper-agent-readiness-and-stable-launch-shape.md)'s discriminator so the two do not implement competing notions of readiness.
