...
**Status**: In progress — M1–M7 complete and green (the desktop resolves projects through the daemon; `PENDING_M6` is empty, which is the tripwire's proof the client-side reads are gone). M8 in flight. **M9 (rule 12 cross-version) and M10 (remote proof) are outstanding and both are manual.** Four of the issue's premises and two of this document's own did not survive reconnaissance, review and audit — see [What the reconnaissance changed](#what-the-reconnaissance-changed) and the Work Log.
