...
**Status**: In progress — M1–M8 complete and green; the automated implementation is done, reviewed and audited. **M9 (rule 12 cross-version) and M10 (remote proof) are outstanding, both manual, and both are merge blockers rather than PR blockers** — this is a genuine `PROTOCOL_VERSION` 8 → 9 wire break. Four of the issue's premises and three of this document's own did not survive reconnaissance, review and audit — see [What the reconnaissance changed](#what-the-reconnaissance-changed) and the Work Log.
...
- **A caller-selected path cannot exhaust or stall the daemon**: every bounded-reader hostile-input case and every publish case has a test, and the publish is atomic, owner-only and symlink-safe.
