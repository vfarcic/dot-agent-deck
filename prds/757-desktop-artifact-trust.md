...
**Status**: Complete (2026-09-26) — v0.42.0 shipped the first Developer ID signed, notarized and stapled macOS `.dmg`, and the maintainer verified it on a Mac (M6). Follow-ups: #1324 (site copy), #1325 (notarization polling), #1326 (certificate renewal before 2027-02-01). Earlier status: **decision REVERSED 2026-09-24: the paid macOS path is now in scope** (maintainer). The prior decision of 2026-09-04 was *free options only*, and it is preserved below and in the Work Log rather than deleted, because the reasoning that produced it is what a future reader needs in order to weigh this one. What changed is the answer to M3, not the analysis: Decision 2's finding still stands that Developer ID signing plus notarization is the **only** thing that removes the macOS warning, and the trigger for revisiting was a maintainer meeting that warning on a real install of v0.41.2 — *"damaged and cannot be opened"*, exactly the failure mode Decision 2 predicted. **Decisions 4 and 5 therefore stop being a record of what a reversal would cost and become the work**: the `desktop-sign` job split of Decision 4, and the threat model of Decision 5 that the maintainer has now answered. What already landed is unaffected and still worth having: M2's release-note correction and M2a's build provenance (Decision 12), the latter being the only trust mechanism that reaches the Linux `.deb`. M1 survives the reversal as a **stopgap** rather than a substitute — see M1.
**Priority**: Medium
**Created**: 2026-09-04
...
**$99/yr, and ACCEPTED as of 2026-09-24 (was declined 2026-09-04):** Developer ID signing plus Apple notarization. This is the only thing that actually removes the macOS warning, and Decision 2 records that there is no free substitute at any effort. The mechanism is designed and written down (Decisions 4 and 5) so that this reversal is a build rather than a re-think — which is now the claim being cashed rather than a hypothetical. The cost the old decision carried, and which the reversal buys out: **every macOS user met a security dialog on first launch, and on v0.41.2 one of them was the maintainer.**
...
*Being built, as M4 — the 2026-09-24 reversal of M3 made this the design to implement. Until then this banner read "Not being built", because the free-options decision of 2026-09-04 had declined the paid path; the body below was written under that decision and is kept as written, so its conditional phrasing ("if the $99 is ever spent") is the history, not the current status.*
...
*Live since the 2026-09-24 reversal of M3: the maintainer has answered this threat model with yes, and M4's `desktop-sign` job is where the secrets below will be named. Until then this banner read "Not live — no secret is being added", and the body below was written under the free-options decision — so its conditional phrasing, and the sentence "Nothing is added to this repository under this PRD", record the pre-reversal state rather than the current one.*
...
**The acceptance test a reversal would have to pass** — M6's acceptance test since the 2026-09-24 reversal — preserved rather than deleted: a user downloads the `.dmg` on a clean Mac, drags the app to `/Applications`, and launches it **offline** with no dialog beyond the ordinary "downloaded from the internet" confirmation; `xcrun stapler validate` succeeds and `spctl` reports a Notarized Developer ID; and no signing credential is named anywhere in `release.yml` outside `desktop-sign`.

## Milestones

M3 was answered *no* on 2026-09-04 and **reversed to yes on 2026-09-24**, so M4 and M6 are **live work** again. Under the free-options decision they had been taken off the board and kept struck through with their reopening condition named, because a milestone deleted is a decision that has to be made again; the reversal is that condition being met, and the strike-throughs are gone.

- [x] **M1 — Measure, on a Mac, for free.** *(Superseded 2026-09-26, not run: M6 answered both of its questions. The dialog question existed only while no certificate did, and the paragraph below already says it "becomes redundant the day M4 lands". The sidecar question is answered by v0.42.0 itself: `desktop-sign` ran `codesign --verify --deep --strict` over the bundle with the daemon sidecar inside, and Apple then returned **Accepted** for both the app and the `.dmg`. So tauri#11992's failure class did not reach this app. Ad-hoc signing was never measured, and is now moot.)* Two questions, one session, no credential and no money: does ad-hoc signing turn *"damaged"* into *"unverified, Open Anyway"* (Decision 3's hypothesis), and does `codesign --verify --deep --strict` survive the daemon sidecar (tauri#11992's failure class)? **Kept after the 2026-09-24 reversal, as a STOPGAP rather than a substitute**: signing takes days of procurement plus the engineering of M4, and every macOS install until then meets the dialog. If ad-hoc signing turns a buttonless *"damaged"* into an actionable *"Open Anyway"*, that is worth shipping in the interim and costs nothing; it becomes redundant the day M4 lands, and the sidecar half of the question (`codesign --verify --deep --strict`, tauri#11992) stops being a curiosity and becomes a prerequisite M4 has to satisfy anyway. Needs Apple hardware, which is available; needs nothing else.
...
- [x] **M3 — The $99 decision.** *Answered 2026-09-04: **no**. **REVERSED 2026-09-24: yes**, for macOS.* Decision 2 records that this is a spend-or-accept-the-warning choice rather than a build-or-buy one, which is why reversing it changes no analysis — only the answer. The trigger was the consequence arriving in practice: a maintainer installing the v0.41.2 `.dmg` met *"damaged and cannot be opened"*, which Decision 2 had predicted in those words. Windows is **not** reversed and stays as Decision 6 leaves it: there is still no Windows artifact to sign.
- [x] **M4 — `desktop-sign`, wired and guarded.** *(Implemented 2026-09-24 on branch `agent/dispatch-prd-757`; see the Work Log entry of that date. **Proof requires a real signed tag (M6)** — nothing in this repository can execute `release.yml`, and nothing here has run on a Mac.)* **LIVE as of 2026-09-24** — M3 was revisited, which is the condition this milestone was parked behind. Build it to Decision 4 (the three-job split, the key never sharing a runner with third-party build code, `desktop-bundle` emitting a `ditto` archive of the `.app` rather than the `.dmg`) and guard it with Decision 11's assertions. Two traps Decision 4 already names and which are worth reading before starting rather than discovering: the hardened-runtime **JIT entitlement** the WebView needs, whose absence yields a bundle that signs cleanly and crashes on launch; and `upload-artifact` flattening permissions to 755/644, which strips the executable bit off the binaries `codesign` is about to sign.
- [x] **M5 — Docs.** *(`docs/develop/desktop-signing.md` written 2026-09-24 alongside M4: secret inventory, environment and tag-ruleset registration, expiry and renewal, rotation, revocation runbook, tauri#11992, and M6's acceptance commands. Folding in M1's measured answer is left to M1.)* Scoped down by M3 to what actually landed: `docs/develop/desktop-gui.md`'s **Release bundles** section now records both guarded properties of the note and points here. A `docs/develop/desktop-signing.md` was **owed** once M3 reversed (it was declined while no certificate existed, on the ground that a certificate-lifecycle page for a certificate nobody holds rots unread — that ground is gone). It has to carry the thing a secret-handling page exists for: **expiry**. Before M4, no workflow under `.github/workflows/` named an Apple secret, and the precedent a guard would have been modelled on — `release.yml`'s `Verify RELEASE_TOKEN is present` step — tests only for *emptiness*, so a certificate guard copied from it would have let an expired certificate through to fail inside a release. M4 does not inherit that gap: `desktop-sign` reads the certificate's end date with `openssl x509 -enddate`, fails the job when under a day remains (`-checkend 86400`) and warns under thirty days (`-checkend 2592000`), and `desktop-signing.md` carries the renewal procedure. User-facing install instructions stay with [#765](https://github.com/vfarcic/dot-agent-deck/issues/765). Remaining: fold M1's measured answer in once it exists.
- [x] **M6 — Verified by a real signed release.** *(Done 2026-09-26 on v0.42.0; see the Work Log entry of that date.)* **REACHABLE as of 2026-09-24.** Its substance was preserved in the test plan precisely as the acceptance test a reversal would have to pass, and this is that reversal: download on a clean Mac, `stapler validate`, `spctl --assess`, and launch **offline** — the last because stapling is what makes a notarized app open without contacting Apple, so a run that never tests offline has not tested stapling. As with M2a, proof needs a real tag: nothing in this repository can execute `release.yml`.
...
*Written under the free-options decision of 2026-09-04; the 2026-09-24 reversal changes how to read it.* That decision removed most of this list — a certificate that is never bought cannot expire, leak, or be revoked — and the reversal puts those three risks back: a bought certificate can expire (M5 owes the handling), leak, or be revoked (Decision 5's threat model, now live). The bullets below are what the *free* position carries plus what a reversal would inherit; the first is the cost the reversal buys out once M4 ships, and tauri#11992 is no longer dormant but on M4's path.
...
**Both questions below are closed as of 2026-09-26**, and they are kept as written. The first: #757 closes on M6, with the remaining work tracked as #1324, #1325 and #1326. The second: M1 was superseded rather than run.

The $99 question was answered *no* on 2026-09-04 and **reversed to yes, for macOS, on 2026-09-24**; the Windows question is answered by there being no artifact. The two questions below were written before the reversal. The first is overtaken by it — #757 now stays open for M4-M6, not as a reminder to revisit $99 — and the second still holds, since M1 survives as a stopgap.
...
Hence Decision 12 and the `attest` job. `actions/attest-build-provenance` obtains a short-lived Sigstore certificate through the job's OIDC token, so there is **no private key in this repository** — none of Decision 5's threat model applies, there is nothing to rotate, and nothing a leak could expose. It is free on public repositories on every current GitHub plan. Every asset is covered: the `.dmg`, the `.deb` and the four CLI binaries, verified with `gh attestation verify <file> --repo vfarcic/dot-agent-deck`. (Amended 2026-09-22 by [#1152](https://github.com/vfarcic/dot-agent-deck/issues/1152): that enumeration was the whole subject list as shipped, and it left out the two assets a full release also publishes — `checksums.txt` and `checksums-desktop-alpha.txt`, which matched neither collection pattern and so carried no provenance at all. Both are subjects now, each travelling to `attest` as an artifact of its own so the attested bytes are the published ones. The enumeration above is kept as what Decision 12 delivered rather than rewritten.)
...
### 2026-09-24 — Reversed: the $99 is being spent

M3 was answered *no* on 2026-09-04, and this entry records the maintainer reversing it. **Nothing in the analysis changed** — Decision 2's finding that Developer ID signing plus notarization is the only thing that removes the macOS warning is what made the old answer costly and what makes the new one worth paying for. Only the answer moved.

**The trigger was the predicted consequence arriving in practice.** Decision 2 wrote that without signing "a downloaded build is quarantined and reads as *damaged and can't be opened*, which looks like a broken app rather than an unsigned one". On 2026-09-24 the maintainer installed the v0.41.2 `.dmg` — the first time anyone had run the desktop app from a release rather than a local build — and met exactly that dialog, reporting it as *"it says that it's damaged and cannot be opened"*. A consequence accepted in the abstract was re-weighed once it had a face, which is a legitimate reason to revisit a decision and worth recording as such rather than as a change of mind.

**What this makes live.** Decisions 4 and 5 were written and kept explicitly so that a reversal would be "a build rather than a re-think"; that is the claim now being cashed. M4 and M6 come off their strike-throughs, and `docs/develop/desktop-signing.md` is owed (M5) because the ground for declining it — no certificate to document — is gone.

**Two things the reversal deliberately does not touch.** Windows is not reversed: Decision 6's reasoning is unaffected because there is still no Windows artifact to sign, and reversing a decision about macOS should not quietly re-open one about a platform this project does not ship. And M1 is not cancelled: signing is days of procurement plus the engineering of M4, and every macOS install in between meets the dialog, so the free ad-hoc measurement keeps its value as a stopgap — with the sidecar half of it (`codesign --verify --deep --strict` against tauri#11992) promoted from curiosity to prerequisite, since M4 has to satisfy it regardless.

**The first thing the implementing agent should read is Decision 5, not Decision 4.** Decision 4 is the engineering and it is settled; Decision 5 is the threat model, and it names the property that makes this unlike adding an API key: an exposed Developer ID key is not rotated, it is **revoked**, and revocation stops already-installed copies from launching. The maintainer has accepted that; the record of what was accepted is what Decision 5 is for.

### 2026-09-24 — M4 and M5 implemented

`desktop-sign` is wired to Decision 4. The macOS leg of `desktop-bundle` builds `--bundles app --no-sign` and uploads a `ditto -c -k --sequesterRsrc --keepParent` archive. `desktop-sign` has no checkout, `permissions: {}`, and reads the five `APPLE_*` values from the `desktop-signing` **environment**, not from repository secrets. That departs from the #757 handoff comment, deliberately: an environment scoped to `v*` tags and `main` keeps the secrets out of Renovate branches and every other job. With the secrets absent it builds an unsigned `.dmg` and stays green; a partial set, or a certificate within a day of expiry, turns it red. The release note is now a function of the signed result (Decision 10), and the `xattr` workaround prints only on the unsigned path.

A security audit added four things the design had not named:
- **A mandatory `v*` tag ruleset** before any secret is registered. The repository had none, and the environment's ref restriction does not stop someone who can push a tag from running that tag's own `release.yml`.
- **A Renovate rule** that holds the two artifact actions running next to the key for manual review.
- **Validation of the app archive** before it is unpacked on the signing runner.
- **A pinned SHA-256** for Apple's G2 intermediate certificate.

Review then found that the validator would have rejected the bundle-root AppleDouble `__MACOSX/._<name>.app`. That was fixed test-first.

Decision 11's guards live in `xtask/linkage-check/src/release_workflow_wiring.rs`. They include an in-memory mutation sweep, and they run the real credential-classifier and archive-validator blocks under `bash`/`python3` (unix only). None of it has executed on a Mac: M6 is the first real test.

### 2026-09-26 — M6 verified; the PRD is complete

**v0.42.0 is the first release with a signed macOS `.dmg`**, and it took three attempts of release run [36209188544](https://github.com/vfarcic/dot-agent-deck/actions/runs/36209188544). Neither failure was in the signing design.

- **Attempt 1** failed in `desktop-sign`'s certificate import with `asn1_d2i_read_bio: too long`. That is a corrupted `.p12`, not a wrong password. The certificate had been registered by piping `gcloud secrets versions access` into `gh secret set`, and gcloud writes a binary payload to stdout as UTF-8, which mangled it. Re-registering from `--out-file`, after checking it with `openssl pkcs12`, fixed it. #1326 carries that warning for the renewal.
- **Attempt 2** got through the credential check, the archive checks, the certificate import, and signing, including `codesign --verify --deep --strict` with the sidecar inside. It uploaded submission `9d0a17b7-6528-4123-92d0-f0f8e4391817`. About 15 minutes into `notarytool submit --wait`, a single status poll timed out (`NSURLErrorDomain -1001`), the step read the status as `unknown`, and the job failed. The submission then stayed `In Progress` at Apple for over a day and was **Accepted**: the account's first notarization. #1325 is the fix, so that a transient poll error is retried rather than reported as a verdict.
- **Attempt 3** passed end to end. `desktop-publish` attached the `.dmg` with `--clobber` and rewrote the release note's delimited desktop block: the heading lost "unsigned", the macOS paragraph says signed and notarized, and the `xattr` line is gone. `attest` attested the published bytes.
