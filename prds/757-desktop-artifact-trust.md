...
- [x] **M5 — Docs.** *(`docs/develop/desktop-signing.md` written 2026-09-24 alongside M4: secret inventory, environment and tag-ruleset registration, expiry and renewal, rotation, revocation runbook, tauri#11992, and M6's acceptance commands. Folding in M1's measured answer is left to M1. **The body below was written before M4 and describes that earlier state**: `desktop-sign` now names the five `APPLE_*` secrets, and it does test expiry rather than only emptiness — it reads the certificate's end date with `openssl x509 -enddate`, fails the job when under a day remains (`-checkend 86400`) and warns under thirty days, and the renewal procedure is in the page.)* Scoped down by M3 to what actually landed: `docs/develop/desktop-gui.md`'s **Release bundles** section now records both guarded properties of the note and points here. A `docs/develop/desktop-signing.md` is **now owed** (it was declined while no certificate existed, on the ground that a certificate-lifecycle page for a certificate nobody holds rots unread — that ground is gone). It has to carry the thing a secret-handling page exists for: **expiry**. No certificate guard exists yet — no workflow under `.github/workflows/` names an Apple secret. The precedent a guard would be modelled on is `release.yml`'s `Verify RELEASE_TOKEN is present` step, whose own comment records that it tests only for *emptiness*, so a token that is present but expired passes it and fails later, at the push. A certificate guard copied from it would inherit that gap and let an expired certificate through to fail inside a release, so expiry handling — in the guard, in the page's rotation procedure, or both — is work still to do, not a safeguard that exists. User-facing install instructions stay with [#765](https://github.com/vfarcic/dot-agent-deck/issues/765). Remaining: fold M1's measured answer in once it exists.
- [ ] **M6 — Verified by a real signed release.** **REACHABLE as of 2026-09-24.** Its substance was preserved in the test plan precisely as the acceptance test a reversal would have to pass, and this is that reversal: download on a clean Mac, `stapler validate`, `spctl --assess`, and launch **offline** — the last because stapling is what makes a notarized app open without contacting Apple, so a run that never tests offline has not tested stapling. As with M2a, proof needs a real tag: nothing in this repository can execute `release.yml`.
