...
- **What is not covered, plainly**: no automated check asserts that the app *looks* right in either theme. There is no visual regression tooling and no driver-level tier in this repo at all — `playwright`, `tauri-driver`, `webdriver` and `selenium` return zero matches workspace-wide. The compensating control is the manual smoke step added to `docs/develop/desktop-gui.md`: open the app in both OS appearances, walk every sheet and dialog, and confirm the override wins in both directions. (Superseded in part on 2026-09-09 by issues #823/#836: a Playwright tier now drives the built bundle in Chromium and in a WebKit engine, and `desktop/e2e/appearance.spec.ts` asserts the `color-scheme` meta as engine behaviour, plus a well-formedness and drift guard over the `theme-color` pair — whether an engine applies `media` to `theme-color` is not observable from inside a page and is not asserted anywhere automated. The zero-matches count above was true when written and is not any more; the manual walk stays, because that tier launches neither WebKitGTK nor WKWebView.)
