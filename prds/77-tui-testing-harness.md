...
- [ ] **M3 — First chain-smoke tests + STOP for validation** (per Decision 29). One real Claude Code chain-smoke test (using `claude-haiku-4-5-20251001`), one real OpenCode chain-smoke test (using `openrouter/google/gemini-2.5-flash-lite`). Both picked from M1's catalog. After this milestone lands, **stop and wait for explicit validation** before continuing to M4+.
  - **Status (2026-06-06):** PARTIAL — `chain-smoke/claude/001` shipped green (commits `ee58882`..`1788d54`). `chain-smoke/opencode/001` BLOCKED by a deck-side bug surfaced during the M3 run and escalated to Discovered Issues as `di-001` (see `## Discovered Issues` table). Harness side for OpenCode is ready (`with_imported_opencode_credentials`, fixture marker file) — the test drops in trivially once the deck install-path bug is fixed. Reviewer + auditor + 13-item fix batch (`1788d54`) all clean. Checkbox stays unchecked pending user decision: (a) fix the deck bug in #77 as M3.2 and ship the second chain-smoke before M3 stop, or (b) defer the deck fix to a separate narrow PRD/issue and close M3 as "1 of 2 chain-smoke tests + di-001 escalation", with `chain-smoke/opencode/001` still allowlisted.
