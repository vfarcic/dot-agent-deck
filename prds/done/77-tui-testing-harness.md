...
> **Moved.** The authoritative Test-Case Catalog now lives at
> [`tests/CATALOG.md`](../../tests/CATALOG.md) — a permanent, PRD-lifecycle-
> independent home parsed by `cargo xtask linkage-check` / `docs`. It was
> relocated out of this (archived) PRD so the tooling no longer breaks when
> the PRD file moves. See that file for every catalog entry.
