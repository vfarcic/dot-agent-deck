...
- Running the dashboard from a freshly-built local binary leaves existing valid hook paths unchanged. **Met for all four as of [#730](https://github.com/vfarcic/dot-agent-deck/issues/730).** As shipped by this PRD it was met for Claude and OpenCode only: Codex's and Devin's `install_impl`s stripped every deck-owned rule by command suffix and re-added unconditionally, so a valid-but-different pin was repointed on each launch — pre-existing behaviour, unchanged by this work, and tracked rather than silently accepted. #730 gave both adapters Claude's two-predicate gate (prune only rules belonging to the binary currently installing, plus deck pins sharing that binary's basename whose target is one the deck could not have written — missing, bare or relative, non-executable, or a build-artifact path, per `pin_is_repairable`; note that is wider than the "positively missing" this document says elsewhere, which was accurate for the `try_exists`-only check of the day), so a valid foreign pin is now left in place with the new rule added beside it, exactly as described under "Self-heal" above. The safety property underneath it was intact throughout — suffix ownership means a user-authored command that merely mentions `dot-agent-deck` is never touched on any of the four — and #730 narrowed it a second way: removal is now per-COMMAND rather than per-rule, so a handler of the user's own sharing a rule object with a deck command survives an install or uninstall instead of going with it.
...
- [x] **M4 — Self-heal for Claude and Codex hooks.** Claude needed no new mechanism — `command_is_dead_deck` already gated on `try_exists() == Ok(false)`, i.e. *positively* missing, treating a stat error as "leave alone". One real gap did have to be closed: `auto_install_to` returned early on `installed.is_empty()`, so a file holding both a dead rule and a current one pruned the dead rule in memory and then dropped the repair. See the caveat under Success Criteria for what M4 did **not** deliver for Codex (or Devin) at the time, and which [#730](https://github.com/vfarcic/dot-agent-deck/issues/730) has since closed by porting this same two-predicate gate to both adapters.
