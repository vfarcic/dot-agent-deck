...
### Phase 2: Return-edge routing — SHIPPED 2026-09-14; deferral premise revised 2026-09-10

**The original deferral** (maintainer call, 2026-07-28) read: the callback maps *dispatched unit → dispatcher pane*, but the dispatcher is the SHORT-LIVED side (an `↳ authoring (one-off)` card) while units are long-lived, and `unregister_pane` drops the callbacks when the dispatcher closes — so the common case is that the dispatcher is already gone when a unit finishes, which suggests completion should be reported somewhere durable rather than injected into a pane. Phases 1/3 shipped without it; the dispatcher reports where each unit is running and the user watches the tabs appear.

**That short-lived premise is wrong, and this document already said so.** The decision two sections above removed the `↳ authoring (one-off)` hint from dispatcher panes precisely because "a dispatcher pane has continued purpose, so the hint was telling the user the opposite of the truth" — the same characterisation the deferral leans on. The intended use is the opposite of one-off: the user **stays** in the dispatcher, dispatches from it, and wants each unit's outcome to arrive there. That is what gives two workable modes instead of one — (1) open the unit's own tab and work with it directly, as today; (2) stay in the dispatcher and receive status. Only (1) exists, which is what leaves the dispatcher write-only: it can start work but never tells you anything again.

**Decision (2026-09-10): inject into the dispatcher's pane — M2.1 as already written — with pane-is-gone as a FALLBACK rather than the driver.** The durable-store alternative is not rejected on its merits; an outcome with no live recipient is a deck-wide attention question and belongs with [#630 (Human-attention inbox)](https://github.com/vfarcic/dot-agent-deck/issues/630) if it is wanted. This PRD owns the live-recipient path only.

Two gaps the milestones did not cover, both found while revising this:

- **`--single` emits no completion signal at all.** A single-agent dispatch has no orchestrator template and no `work-done --done` instruction, so half the dispatch shapes have nothing for M2.1 to resolve.
- **Delivery into a live conversation is not free.** An unsolicited report injected into a dispatcher the user is mid-task in is issue #617's "unsolicited work-done" case; submit vs notice is a real choice, and #617's identity binding is what stops a completion landing on whoever inherited a recycled pane id.

- [x] **M2.0** — Register a `dispatch-id → caller pane` callback at dispatch time; ride the id into the spawned unit. → Shipped as `src/dispatch_return.rs`: a `DispatchReturns` map held in `AgentPtyRegistry`, keyed by the dispatched unit's TERMINAL pane (`SpawnHandle::delivery_pane_id`) and holding the caller's retained `(pane_id, agent_id)` pair. Keyed on that pane rather than the worktree so resolution is a map lookup at exactly the pane a terminal `work-done` already arrives under — which also means an ordinary worker inside a dispatched orchestration can never match.
- [x] **M2.1** — On the dispatched unit's terminal `work-done`, resolve `dispatch-id → pane` and inject via `write_to_pane_and_submit`, bypassing the `(name, cwd)` tuple lookup; survives detach/reattach. → `return_dispatch_completion` (`src/state.rs`) called at two seams in `handle_work_done`, delivering through the existing `deliver_dispatch_result` seam so #617's identity gate is reused rather than duplicated. Detach/reattach pinned by `dispatch/return/002`; identity refusal by `/004`.
- [x] **M2.2** — The pane-is-gone fallback (drop-and-log, or hand to #630) and the delivery mode (submit vs notice), per the two gaps above. → **Submit**, matching the acknowledgement path; a passive notice would give a badge rather than a return edge. **Drop-and-log** on pane-is-gone, entry evicted, no durable store — #630 untouched. Pinned by `dispatch/return/005`.
- [x] **M2.3** — Cover `--single`, or state in the docs that the return edge is orchestration-only. → **Covered**, not documented away: the survey's argument held that a half-present return edge stalls a waiting consumer silently rather than failing visibly. The identity is the retained return entry itself, checked in the `None` arm of the `pane_role_map` lookup BEFORE the existing unknown-pane warning — dispatched panes are deliberately NOT added to `pane_role_map`, which is what keeps the admission gate unwidened (pinned by `/003`'s control) and what makes M5.1's answer clean. The single-agent prompt gained a `work-done --done --task-file '<daemon-generated path>'` instruction; the daemon generates the filename so no model invents a path segment.

> **Tracking:** this is #220's OWN Phase 2, not #174's. #174 (cross-project orchestration dispatch) *depends on* this PRD — see Decisions — so deferring the return edge "to #174" inverts the dependency. **No separate follow-up issue**: #220 stays open and these milestones are the tracker, which M5.2 already guarantees ("#220 cannot close while Phase 2 is outstanding"). The earlier "needs its own follow-up issue" note was insurance against #220 being closed with Phase 2 orphaned; M5.2 prevents that, and a second tracker would re-create the two-places-to-look confusion this PRD records having hit twice with #174.
...
- [x] **M4.2** — Real-agent pre-PR test: Haiku dispatcher runs `dispatch` end to end against a real clone/worktree, asserted via a uniquely-named sentinel file. → `dispatch/return/006` [reel] closes the gap this milestone named. A real Haiku dispatcher runs `dispatch --single`; a real Haiku unit comes up in the dispatched worktree, discovers a sentinel whose full name was never in any prompt (only its prefix was), writes its report to the daemon-generated path, runs `work-done --done --task-file` of its own accord exactly once, and deletes the file. The completion renders fenced in the dispatcher's pane ~13s after the dispatch call. This is the sentinel assertion on the dispatched unit's OUTPUT that was missing, and it is the only thing anywhere that proves a real agent follows the composed `--single` prompt — no CI job can, since nothing credentialed runs on a runner.

### Phase 5: Docs, cross-version, release

- [x] **M5.0** — Docs: dispatcher-mode + `dispatch` as the recommended parallel-work flow; repoint #140's guard/warning copy at the dispatcher mode. → `docs/dispatcher-mode.md`. #140's guard copy is NOT yet repointed.
- [x] **M5.1** — Cross-version contract check (CLAUDE.md rule 12): the return-edge callback and any spawn-request field additions are additively compatible; classify per `docs/develop/versioning.md` and add a `.breaking.md` fragment only if the contract shifts. → Not run. Static read is non-breaking, `PROTOCOL_VERSION` unchanged: `DaemonMessage` is `#[serde(tag = "message_type")]` on the hook socket and the loop's `if let Ok(msg)` SKIPS unknown variants, so an older daemon ignores a `dispatch` message rather than closing the connection (same additive shape as the `GetSeed` precedent, which also did not move the version). Classified **feature** → `changelog.d/220.feature.md`, patch bump. **The manual matrix was RUN on 2026-09-14 and the contract holds.** Evidence it was a genuine cross-version run rather than same-version: exactly ONE `Attach protocol listening` line, the same OLD daemon (pid 3588272, build `g2fdedfb`) serving start to finish, and the branch TUI showing the build-mismatch prompt naming the live agents before being declined. Both sides `server_version: 9`. A delegate routed and hooks arrived. A `dispatch` against the old daemon delivered its acknowledgement and returned no completion — the old daemon has no retention, so the return edge degrades to absence: degradation, not disagreement. No `PROTOCOL_VERSION` bump and no `.breaking.md`.
...
- **Standalone vs #174 Phase 1 → ships standalone; #174 depends on THIS.** Recorded explicitly because the dependency has been stated backwards more than once (in `docs/dispatcher-mode.md` and in PR #232 discussion, both of which deferred #220's return edge "to #174"). #174 is *Cross-project orchestration dispatch* — a separate open PRD issue, not a PR, and not a tracker for #220's Phase 2.
- **Experimental flag → yes, then GRADUATED before shipping.** It shipped behind `features::show_dispatcher()`, its own wrapper per CLAUDE.md rule 9 (not a reuse of `show_issue_dispatch_authoring`), gating ONLY the Mode-cycler option; the `dispatch` verb and its daemon handler were always ungated. The wrapper is now deleted and the branch inlined to `true`, so no `graduate-dispatcher` follow-up issue was needed — the graduation happened in this PR. The user-facing page moved from `docs/develop/` to the published `docs/dispatcher-mode.md` at the same time.
