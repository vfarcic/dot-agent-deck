...
### 2026-09-02 — The user reviewed the first panel, and three conventions came out of it

`8be48be` (one setting, one row), `6eb8200` (the prose cut) and the chrome collapse below. All three are container-level rules rather than appearance changes, which is why they are recorded here and not only in #743: **whatever the first panel establishes is what #741 and #802 will copy**, so the first panel is where the design language gets set. All three rules are written into `docs/develop/desktop-gui.md`'s "Adding a setting" section, which is what dependents actually read.
...
**"Label left, control right" was the wrong statement of that rule, and the user's next round said so.** *"How about buttons being on the right side of 'Appearance'"* — asked of a control that was already hard right, which is the tell: measured in Chromium, the row is 743px and the control 174px, so `Appearance` sat with **502.94px of nothing** between its right edge and the control's left edge, and a label that far from its control reads as unrelated to it. Flush-right is a rule that only looks correct on a narrow row. The row is now a **two-column grid** — a 132px label column, a 16px gutter, then the control left-aligned beside it — which drops that gap to **60.77px** and, more to the point, makes every future row's control start at the same x instead of at whatever its own label's width implies. Verified with two stub rows added and removed: three labels of different lengths, one of them wrapping inside its column, all three controls starting at 827px. The label went from 11px/400 to **13px/650** in the same round, which the docs now carry as part of the convention rather than as this panel's choice, since the size a panel picks for its labels is the size the next panel inherits. `float: left` on the legend survives all of it, for the reason it was added: a floated legend stops being the fieldset's *rendered* legend, and that disqualification is what lets it be an ordinary grid item — the float itself is ignored on a grid item, so the mechanic reads oddly and needed its comment rewritten rather than deleted.

**UI text earns its place by being actionable, not by being true.** The user asked why two strings existed and observed that an app needing long text to explain itself is showing a UX problem. Three deletions followed, and the panel body went **291px → 123px → 61px** across the two commits, with the sheet header 110px → 92px (and → 58px in the third round below):
...
**Do not put a heading above a control that already labels itself.** The third round, and the user's count was generous: they objected to "2 line header plus 'Appearance' as another line and then the fourth row with 3 buttons", and the redundancy was worse than that — **the word "Appearance" was on screen twice** (a `.form-heading` eyebrow `APPEARANCE` over a `Light and dark` title, directly above a row whose own `<legend>` reads *Appearance*), the sheet said "settings" twice (`APPLICATION SETTINGS` over `Settings`), and `Light and dark` restated the three option labels the buttons were already showing. Measured over CDP against the real `pnpm build`, identical in both appearances: the sheet header **92px → 58px**, the panel's `.form-heading` **70px → gone**, and sheet-top to the bottom of the panel's content **223px → 119px**. The setting itself did not move — 61px both ways — so **all 104px came off chrome**, and a 70px heading had been sitting above a 61px setting. The `<legend>` is the one that survives, because it does two jobs the heading was not: the row's visible label *and* the fieldset's accessible group name, so `getByRole("group", { name: "Appearance" })` rides on it and the deletion took no label with it.

The word stays **"Appearance"**, not "Theme", and that is a decision rather than an omission: macOS System Settings uses "Appearance" for exactly this control with exactly these three options, and **"Theme" is specifically ambiguous in this app** — the agent terminals have their own colours and #743 deliberately excluded them, so "Theme" invites "which theme, the app's or the terminal's?" in a way "Appearance" does not.

The shared-CSS trap here is worth recording, because lowering the obvious number would have broken four other sheets silently. `.sheet-header`'s `min-height: 92px` is sized for an eyebrow, a title *and* a description, and all four `ConfigurationPanels` sheets still carry that full stack. So the override went on a `.sheet-header.is-title-only` modifier and the shared rule was not touched — a pure addition to `styles.css`, with `ConfigurationPanels.tsx` absent from the diff entirely. Verified rather than assumed: all four sheets measure **99px before and after** in both appearances, same class, same `min-height`, same padding, same alignment, same `h2` margin, and the Projects header renders **byte-identical PNGs** across the change.

**When #741 or #802 adds a second section, that is the moment to decide whether section headings earn their row — and it is left open rather than pre-built.** The argument against them survives the second section: the section column returns the moment the registry holds two rows, and that column already names the active section beside the panel, so a heading repeating it inside the panel is the same redundancy one level up. The argument for is that two sections make a panel's identity worth confirming where the user is looking. Recorded in the docs as an open question, to be decided with both panels on screen and then applied as one rule for every panel.

The brief for this round predicted that a test asserted on the deleted headings — `getByRole("heading", …)` for `Light and dark`, or on the eyebrow text — and **none did**, in either `App.test.tsx` or `SettingsSheet.test.tsx`; the whole surface was pinned by test ids, roles and the group name, so no existing assertion had to be rewritten to make room. That is a gap rather than luck, so the change adds the assertions instead: the sheet holds exactly one heading (*Settings*), the panel holds none, and no `.eyebrow` or `.form-heading` survives on the surface — which is what stops the chrome growing back one element at a time.
...
**No experimental flag**, confirming Open Question 1's recommendation and `prds/done/176-desktop-gui.md` decision 6.
