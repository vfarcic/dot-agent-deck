...
**Status**: Complete — implemented in PR [#1235](https://github.com/vfarcic/dot-agent-deck/pull/1235), merged 2026-09-24
**Priority**: Medium
**Created**: 2026-09-22
...
### 2026-09-24 — Reference grounding removed; the confirmations are the control

**The report.** With `dot-agent-deck-orchestrator-1` on the overview the user said *"Stop the orchestration 1."* and was told *you did not name “dot-agent-deck-orchestrator-1”, so nothing was done.* Their question — *"Is that intentional? I feel it would be very hard for people to name 'things' exactly as they are and, even if they do, speech to text might misunderstand (i.e., dot might be 'dot' or '.')"* — and their follow-up settled a design question rather than adding one more phrasing gap: **the actions that can do harm already stop at a confirmation, so strict name matching charged every sentence for protection the confirmation already provides.**

**The decision.** Reference grounding (`outcome::grounding`, `grounded_target`, `Unmet::NotNamed`) is gone as a gate on dispatch for every reference kind — `deck_ref`, `dir_ref`, `mode_ref`, `agent_type_ref`, `orchestration_ref`; `agent_ref` never had it. What bounds a reference now is three layers, recorded in [`docs/develop/voice-first-design.md`](../../docs/develop/voice-first-design.md) section 6 together with the user's argument, so nobody reinstates it without answering it: **action grounding** (unchanged — every row's action must be asked for, `submit_prompt` by the whole utterance), **on-screen-only resolution** (the resolvers match the live fleet, the listing and the offered chips, never anything else, so a hostile name cannot conjure a target), and **confirmation for destructive actions** (`stop_agent` and `close_orchestration` keep their D5 confirmations exactly as they are, at the user's direct instruction; they name the target, and every role for an orchestration). `start_new_agent` has no reference and still needs a start word; everything else a reference reaches is undone in one utterance.

**`CommandWordsOnly` went with it.** It existed because grounding could not tell a name made only of command or filler words (`open`, `this-directory`, `run`, `new`) from the verb that invoked the row. With no reference grounding there is nothing for it to protect, and all it still did was make ordinary directory names manual-only; the one case it mattered for, "use this directory" steered to `open_dir`, is refused by action grounding and pinned.

**Two resolver/prompt additions, so partial references actually work.** `resolve_orchestration_ref` answers a bare category reference ("the orchestration", "the run") with the one card on screen, and as ambiguous naming every card when there are several. `TOOL_INSTRUCTIONS` gains one sentence: a deck, directory or orchestration referred to by position or kind ("the first one", "the remote deck", "the other run") is answered with the entry's listed name. That second is a prompt change.

**One consequence the decision did not name, and it is a behaviour change rather than a hole.** A deck the model fills in for an utterance that names none ("spawn an agent") used to be dropped by grounding; it is now preselected, since it is on screen. The dialog shows it, choosing another deck replaces it, nothing starts without a start word — but there is no voice row for choosing a deck inside an open dialog, so undoing it by voice is "close" plus "new agent on …", not one utterance. Measured: in one of the runs below gpt-5-mini filled the local deck for "spawn an agent". **Follow-up, same day: the report now names the preselected deck** — *Opening the New agent dialog. Preselected deck: Local deck.* — for every optional param that resolves, said or not, so a wrong guess is heard before a start rather than found after one; with no deck the report is unchanged. The phrase-fixture harness checks it on every fixture whose answer preselects a deck, which covers the unasked preselections a fixture cannot pin. **Second follow-up: voice now knows which decks can take a spawn.** It resolved a deck against the whole observed fleet, so a deck the dialog shows disabled could be "Preselected deck: X" while the dialog preselected nothing, or the only eligible deck — a spoken report contradicting the screen, audible now that the report names the deck. The webview declares the dialog's deck step with every utterance (`voice::VoiceDeckChoice`: each deck's id and the reason the step shows for a disabled one); a disabled deck is never offered to the model, a named one is answered with that reason ("Deck ci@stale-box cannot take a new agent, so none is preselected: No deck is listening on the configured socket."), and with exactly one eligible deck voice dispatches it, as the dialog preselects it anyway, so "none is preselected" is never said when one is. Declared by the webview rather than derived from the daemon links because the dialog preselects from the webview's fleet, whose states and fallback sentences are computed there. Tests: `voice_outcome_new_agent_never_offers_or_preselects_a_deck_that_cannot_take_one`, `voice_outcome_new_agent_names_the_implied_deck_only_when_it_carries_information`, `voice_decks_take_eligibility_from_the_declared_deck_step`, and vitest for `voiceDeckStep`, the runtime's declaration and the bridge's `deckStep`; the phrase-fixture harness plants a disabled `ci@stale-box`, checks on every fixture that it is never preselected, and adds `open-new-agent-on-a-deck-that-cannot-take-one` (114/114 against gpt-5-mini, two runs). One prompt change: `TOOL_INSTRUCTIONS` now says `decks` lists "every deck a new agent can be started on" rather than "every deck the app can reach", which stopped being true. A second clause — telling the model to answer an unlisted deck with the user's words — was tried and dropped: in the run that measured it the model answered "the remote deck" with `remote deck` verbatim instead of the listed remote, failing `open-new-agent-on-the-remote-deck`, and the disabled-deck fixture passes without it. **Third follow-up: the implied deck is named only when that carries information.** Naming the only eligible deck on every plain "new agent" was a sentence on every utterance for a user with one deck, telling them nothing — there was no choice and no guess. It is still dispatched in every case, so the report and the dialog agree by construction, but it is named only when a deck was referred to (by the user or by the model), or when a note precedes it — a dropped, disabled, unmatched or ambiguous deck, whose "none is preselected" would otherwise be false. With several eligible decks the dialog makes no choice unless told, so a bare "new agent" preselects nothing and says nothing, and a deck the model picks among them is named. The phrase-fixture harness needed no change: its fleet has two eligible decks, so every deck it sees preselected came from the model and is still required to be named.

**Tests.**
- Reproduced first: `voice_outcome_stop_the_orchestration_1_opens_its_confirmation` failed on the user's exact sentence with their exact refusal, and passes now — the model's answer as the title, as "orchestration 1" and as "the orchestration 1" each reach the stop confirmation.
- New: `voice_outcome_partial_references_resolve_to_what_is_on_screen` (a directory by one word, by what it is and as transcribed; a deck by kind; a mode and an agent type by position and maker; an agent by role; an orchestration by ordinal, number and category), `voice_outcome_a_category_reference_among_several_orchestrations_is_ambiguous`.
- The pins on what remains, together: `voice_outcome_the_controls_that_remain_without_reference_grounding` — action grounding still refuses `go_to_parent`, `use_this_directory` and `start_new_agent` for "open docs" over a hostile listing, `stop_agent` for "open the tester" and `close_orchestration` for "close the billing agent"; `submit_prompt` still needs the whole utterance; both stops still dispatch only their `confirm*` invoke with a "nothing has been stopped yet" report (vitest holds the webview half).
- Premise changes, updated deliberately and named in each test's doc: `…refuses_a_hostile_name_the_user_did_not_say` → `voice_outcome_a_hostile_name_moves_the_browser_at_most`; `…drops_an_invented_deck_and_keeps_a_named_one` → `voice_outcome_open_new_agent_preselects_any_deck_on_screen`; `…drops_a_deck_the_user_did_not_name` → `voice_outcome_create_a_new_agent_opens_the_dialog_on_the_deck_the_model_chose`; `…is_not_voice_choosable` → `voice_outcome_a_directory_named_only_with_command_words_is_voice_choosable`; `voice_outcome_a_required_param_that_fails_still_refuses_the_action` lost its not-named case; `voice_outcome_orchestration_ref_resolves_a_card_by_title_or_name`'s category case now expects ambiguity; and the unit tests of the grounding function itself (`…grounding_accepts_what_the_user_said…`, `…grounding_names_a_command_word_only_target…`, `…choice_and_orchestration_refs_must_be_grounded`, `…refuses_a_target_reached_only_through_a_verb`) were deleted with it.

**Phrase fixtures.** Nine new, 104 → 113: the user's sentence and "close the orchestration" against a planted auto-named run (`generated_run = true`, a new harness field), the title spelled as a transcriber hears it ("the dot agent dot deck orchestration"), "shut down the whole run", "stop the coder" in that run, a directory by one word, as transcribed and by its last word, and "new agent on the remote deck". Premise change: `no_deck = true`, asserted by eight deck-less openers, held only because grounding dropped an unasked deck, and the field was removed. Runs against gpt-5-mini:

| run | manifest | passed | the miss |
| --- | --- | --- | --- |
| 1 | first | 113 of 113 | — |
| 2 | first | 112 | `open-new-agent-spawn`: the model filled the local deck, preselected — what removed `no_deck` |
| 3 | `no_deck` removed | 112 | `start-new-agent-dialog-closed`: "start it" went to `stop_agent`, refused by action grounding |
| 4 | `no_deck` removed | 112 | `open-dir-by-what-it-is` ("go into the deck repo") answered `none`; replaced by `open-dir-by-its-last-word`, since "deck" also names a screen |
| 5 | final | 112 | `start-new-agent-dialog-closed` again, the same `stop_agent` pick, refused the same way |
| 6 | final | 113 of 113 | — |

Every new fixture passed in every run it was in, except the one replaced. The open item is `start-new-agent-dialog-closed` at 2 of 6: it fails closed — action grounding refuses a stop for "start it", which is the remaining control working against a live model — but whether the new `TOOL_INSTRUCTIONS` sentence moved it is not established; the previous entry's runs did not name which fixture each miss was.

### 2026-09-25 — Closed

PR #1235 merged on 2026-09-24 without archiving this document, because the team path that built it had no archival step (#1297). Archived to `prds/done/` and the issue closed by the PR that fixed that.
