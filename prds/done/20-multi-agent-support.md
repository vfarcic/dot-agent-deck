...
Filed as optional/additive work on top of this PRD (none blocked shipping). **All four are now resolved** — one shipped, three closed as not planned on 2026-09-15 after going stale. The reasoning is recorded here rather than only in the closed issues, per CLAUDE.md rule 13.

- [#213](https://github.com/vfarcic/dot-agent-deck/issues/213) — Make project `.claude/skills` discoverable by Codex (seed/symlink project skills into `~/.codex/skills` at spawn; Codex only reads its own skills dir). **Shipped.**
- [#214](https://github.com/vfarcic/dot-agent-deck/issues/214) — Scriptable pane-creation CLI (the superseded `pane new --agent` milestone). **Closed not planned** — it was filed as "worth doing only if a scripted pane-creation need materializes", and in two months none did: orchestration creates panes through the daemon protocol, and the e2e harness drives the TUI over PTY. There is still no pane-creation subcommand for any agent, so this stays a convenience rather than a gap — the TUI new-pane Agent selector covers the interactive path, as (b) above describes. Cheap to refile if automation or demo scripting ever wants it.
- [#215](https://github.com/vfarcic/dot-agent-deck/issues/215) — Enrich the `codex exec --json` wrapper mapping to `ToolStart`/`ToolEnd`. **Closed not planned** — no consumer. Codex's registry `default_command` is plain `codex` (the interactive path), which already reaches full prompt/tool/turn parity through native hooks, and nothing under `src/` ever launches `codex exec --json` — so the enriched mapping would only pay off for a user typing that into a pane by hand, at the cost of a second, permanently-inferior event mapping maintained alongside the hooks path. The `CODEX` `RuleSet` stays the coarse Working/Idle/Error fallback it was designed to be.
- [#216](https://github.com/vfarcic/dot-agent-deck/issues/216) — Optional: migrate Codex to a pure `NativeHooks` strategy and retire the wrapper for it. **Closed not planned, folded into [PRD #211](../211-gemini-adapter.md).** The premise still holds — Codex is the only `Wrapper`-strategy agent — but whether that is worth changing is a decision #211 makes: if Gemini ships as a second `Wrapper` agent, the "Codex is the sole Wrapper agent" motivation for the cleanup disappears and the hybrid is vindicated; if #211 goes another way, the question is worth reopening then. Why the hybrid is the right shape today is documented on the registry entry itself (`src/agent_registry.rs`, the `CODEX` `AgentSpec`): the wrapper is Codex's PTY host and hook injector, and its coarse stdout fallback.
