...
- [ ] **M13.1** — Find the TUI's per-pane stream subscriber. Likely in `src/daemon_client.rs` (attach-protocol stream consumer) or `src/state.rs`/`src/ui.rs` (subscriber loop). Identify the read-from-broadcast-receiver site that today silently terminates on `RecvError::Closed`.
- [ ] **M13.2** — On `RecvError::Closed`, re-resolve `pane_id_env` via `list_agents` (or equivalent attach-protocol call), find the NEW agent's id, and re-subscribe. With a small backoff (e.g. 100 ms initial, capped) for the case where the new agent hasn't been registered yet at the exact moment Closed fires. Cap total retry attempts so a permanently-closed pane doesn't spin.
- [ ] **M13.3** — Tests:
