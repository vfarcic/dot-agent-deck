...
| M6 | The three defects this PRD made reachable (#1046's toast, the `chooseDeck` data-loss path, the settings panel under a fleet) plus the `release_if_not_selected` rename | in progress | yes (three fixes) | held out of M1–M5 so each of those stayed about one thing |
| M7 | Docs: the **Decks** section and **Current milestone limits** in `docs/develop/desktop-gui.md`, including the now-stale remote-verification bullet and the fourth trust gate | | no | corrections this PRD owes regardless of the rest |
| M8 | The two-deck run (Verification), recorded | | no | the real done-condition; inherently manual per #953 |
| M9 | File the control-half follow-up issue, carrying the seams this document names | | no | the service #741 performed for this PRD |
...
**M5 put it on the wire.** `EndpointIdentity::wire_id()` — FNV-1a over the newtype's *derived* `Hash`, rendered `deck-<16 hex>` — so a new `RemoteEndpoint` field joins the token the moment it is declared, with nothing to update. `DefaultHasher` was rejected because `std` documents its algorithm as unspecified and the webview keys `localStorage` state on this token. The event stamp had to move with the snapshot key: `isSelectedDeckEvent` compares them, so a stamp left on `describe()` would have gone on answering "yes, this is your deck" for a second daemon on the same host after the snapshot stopped doing so.
