...
- [ ] **M2.14** — **Propagate `--theme` through the `ssh -t` `connect` wrapper.** `src/connect.rs:546` has an orphaned comment referencing the original M5.5 plan; the propagation is currently a no-op. The laptop-side `--theme` flag doesn't reach the remote TUI invocation, so the user's theme preference is lost when running over `connect`. The fix: parse `--theme` on the laptop side and forward it into the remote `dot-agent-deck` command line in the ssh exec. *(Note: `--continue` was originally also in scope for this milestone, but [PRD #89](89-auto-restore-tui-state.md) replaces the flag with unconditional auto-restore; nothing left to propagate for that case.)*
...
- [ ] **M2.16** — **Clean up stale "daemon does not track display metadata yet" comment in `src/embedded_pane.rs:708`.** Cosmetic. The comment was true before M2.11 and is wrong now. One-line edit; bundle with whichever of M2.12–M2.15 lands first if convenient, or do it standalone.
