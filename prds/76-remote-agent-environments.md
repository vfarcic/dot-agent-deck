...
- [x] **M2.11** — **Persist TUI organizational state across reconnect.** Daemon rehydration (commits 822770c / 51d8459 / 9d3a42a) replayed agent PTYs and pane_ids, but the daemon never knew display names — so a reconnect showed "1 1, 2 2, 3 3" instead of the user's named panes. The TUI now writes a small JSON file `<state_dir>/tui-state.json` (mode 0o600, atomic tmp+fsync+rename, ~250 ms debounce) keyed by daemon agent_id and reads it back on bootstrap to restore display names and cwd. Entries whose agent_id is no longer in `list_agents` are dropped on reconcile. The schema carries a `version` field for future migrations. Same-user, same-VM, last-writer-wins concurrency in v1 — two laptops attached at once will overwrite each other's view, accepted as a v1 trade. Structural restoration of mode tabs and orchestration tabs is deferred: `TabManager`'s `open_*_tab` methods spawn new panes; rebuilding tabs over already-attached daemon panes needs a separate "attach existing pane to tab" entry point and is out of scope for this milestone. `LocalDeck` mode is a no-op (the in-process daemon dies with the TUI).
