...
- [ ] **M2.14** — **Propagate `--theme` and `--continue` (and any other agent-spawn flags) through the `ssh -t` `connect` wrapper.** `src/connect.rs:546` has an orphaned comment referencing the original M5.5 plan; the propagation is currently a no-op. Flags entered locally don't reach the remote TUI invocation, so the user's theme / continue preferences are lost when running over `connect`. The fix: parse the relevant flags on the laptop side and forward them into the remote `dot-agent-deck` command line in the ssh exec.
