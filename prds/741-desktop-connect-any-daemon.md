...
### 2026-09-11 — M7 and M10, shipped together because they are one surface

Taken as one unit, and the user's own framing is why: *"We definitely need the option to add remote decks in settings. If we also add a button to test it (can it connect), that can be one milestone I can test."* It is the first milestone that touches `desktop/src/**` and the first thing a user can drive.

**Where the tunnel lives, which is the design decision M5 deliberately left.** A `TrustedDaemon` is re-established every `HANDSHAKE_REVALIDATE_INTERVAL` (5 s), so a tunnel owned by one would re-authenticate ssh every five seconds — tunnel lifetime is not handshake lifetime. `desktop/src-tauri/src/endpoint_tunnels.rs` is the owner: keyed by `Endpoint::describe()`, a field of `DesktopState`, shared by `Arc` with `DaemonLinks` because establishment is where a transport is needed, and **never** a field of `TrustedDaemon`. Its module docs carry the five ownership rules and the four teardown triggers M9 and #742 inherit; the one the PRD named — a tunnel leaked per selection change — is closed by `retain` on the settings save, and the probe's own lease is released unless the tested deck is the selection (releasing the selection's would leave a live link holding one lease while the next establishment opened a second `ssh` child beside it).

**What M10 distinguishes**: twelve named states, not one "failed" — `reachable`, `build_stamp_differs`, `protocol_refused`, `handshake_refused`, `deck_not_answering`, `host_unreachable`, `host_key_unverified` (with the run-`ssh`-once remedy, built from the endpoint the tunnel uses so the bastion and the port survive), `auth_failed`, `transport_failed`, `no_remote_socket`, `unknown_deck`, `ssh_unavailable`. The last two are `SelectionFallback`'s own cases, reported as themselves; the banner's condition now includes a fallback, because a `NoRemoteSocket` substitution leaves the app **connected** and would otherwise be silent.

Socket discovery runs over the round trip `Test connection` is already making and hands the path back for the panel to write into the row — the write-back is the webview's, deliberately, because `useDesktopSettings` already serialises the document's read-modify-write and a second writer in Rust would be [#828](https://github.com/vfarcic/dot-agent-deck/issues/828) created on purpose. The forwards disclosure reuses `remote_doctor::parse_ssh_g` against the tunnel's own `-F` config and forced options, so what it reports is what the tunnel will actually inherit — audit **A3**'s residual, visible for the first time.

**One finding worth the paragraph, and it is not this PRD's to fix.** The house two-column settings row makes its label a grid item by floating a `<legend>`, which disqualifies the legend from being the fieldset's *rendered* legend. **WebKit forces a rendered legend's `float` to `none`** — measured in both browser-tier projects: `getComputedStyle(legend).float` is `"left"` under chromium and `"none"` under webkit — so the legend never becomes a grid item there, the control lands in column one beside its label, and the 132px column is wasted. The app ships on WebKit. Nothing caught it because no browser spec asserted settings-row geometry until `e2e/decks.spec.ts` did. This PRD's chooser uses `role="radiogroup"` + `aria-labelledby` instead, which lays out identically in both engines; `AppearancePanel` and `ZoomPanel` still diverge, and that is recorded in `docs/develop/desktop-gui.md` rather than changed here.

Rule 12: `PROTOCOL_VERSION` did not move and no `.breaking.md` is warranted — nothing here changes the TUI↔daemon wire or the meaning of a field behind it. The cross-version manual test is still M12's.
