...
**Status**: Implemented — M1–M6 complete. The visual half is unverified: five claims in [Testing](#testing-what-rule-4-means-here) have no automated coverage in this repo and rest on the manual smoke check, which has **not** been run (no display in this environment). Stated as an outstanding item rather than glossed.
**Priority**: Medium
**Created**: 2026-09-04
...
- **A Zoom section in the Settings sheet** — one registry row and one panel, per the `settingsContract.ts` contract, plus one general prop added to that contract (see [The Settings section](#the-settings-section)).
- **One row in the `?` shortcut overlay** (`App.tsx:621`).
- **A near-zero `App.tsx` footprint**: one import, one hook call, one array entry, and a comment. Nothing else. This is a deliberate constraint, not an aesthetic one — see [Staying out of #869's way](#staying-out-of-869s-way).
...
**Where the hook is *called from* is a second, independent correctness requirement, and getting it wrong is what shipped in the first round.** `useZoom` belongs in `DeckShell`, above the view switch, because that is the only component mounted for every screen — `DeckShell` renders `AgentOverview` *instead of* `ControlDeck`, not around it. Called inside the deck, the keys were dead on the Overview screen. The settings document moved up with it, since it is the app's rather than one screen's, and `ControlDeck` became a three-line wrapper that owns a settings state for standalone and test use.
...
Three lines of code, and this is a scope commitment rather than a style note: an import, the hook call, and one entry in the `shortcuts` array. **Measured on the shipped change: `git diff --stat` reports 6 insertions and 1 deletion**, the extra three being a comment explaining why the hook's return value is unused. The hook is called for its effects — it registers the listener — and the Settings row reaches the same level through the settings document, so `App.tsx` needs nothing back from it.

Everything else — the ladder, the key matching, the capture-phase listener, all three coalescers, the persistence and the panel — lives in new files.
...
**`SettingsPanelProps` grew a fourth prop, and the "one row and one component" claim is narrower than it was.** The panel needs to know it is in the browser preview, where there is no webview to scale. Three options were on the table: sniff `selectRuntimeMode()` inside the panel (a `window.location` read in a render, and a second source of truth for something the app already resolves once), pass the feature's own hook down (feature-specific plumbing in a shared contract, which is the thing #803 built the registry to prevent), or add the runtime mode — which the sheet already receives — as a prop for every panel. The third is the one taken, on the grounds that it is the *general* environmental fact rather than this feature's: the preview cannot scale a webview (#744) and will not be able to reach a daemon endpoint (#741) or hold a local model (#802) either, so all three tenants want exactly it. A prop only one panel could ever use would have belonged somewhere else.

**One `.settings-hint` class was added too**, for the preview's one line of prose. `docs/develop/desktop-gui.md` records that panel-level notes are "a narrow exception with **no current instance**" and that the `.settings-note` class was deleted for failing the text rule — so this is the first instance since, it is a different class rather than that one revived, and it clears the bar on the stated exception (a consequence the reader has to act on: the zoom they want is their browser's). It renders only in the preview.
...
5. **Should the `?` overlay be completed, and made platform-aware, while we are in it?** Two pre-existing problems, both left alone. It lists `App.tsx`'s five bindings and omits `AgentComposer`'s `Enter` / `Shift+Enter`, so it is not today the complete list of what the app binds. And its `⌘ K` row hardcodes the macOS glyph, so it is misleading on Windows and Linux — Greptile flagged exactly that about the zoom row this PRD adds (P2 on PR #880), which is why the new row reads `⌘/Ctrl` instead. Neutral wording is the right fix for the zoom row specifically, because either modifier genuinely works on either platform (`zoomIntentFromKey` accepts both), so there is nothing to detect; `⌘ K` is a real platform binding and would want detection rather than wording. Whether the overlay should be generated from one registry of bindings, with a platform-aware renderer, rather than a hand-kept array of glyph strings, is the question behind both — and it is not this PRD's.
...
### 2026-09-04 — Built, M1–M6, and the two things the code found that the PRD had not

Implemented in the order the milestones list, each milestone's tests written with it. The desktop suite went from **11 files / 224 tests to 14 files / 287 tests**; `cargo test-fast` is 2410 passing and the desktop crate's own Rust tests 97, up 9 from the `[zoom]` section's clamping and merge coverage. `cargo fmt --check`, the full `cargo clippy --workspace --all-targets --features e2e,e2e-live -- -D warnings` gate, `pnpm test` and `pnpm build` are all green.

**The xterm clash is worse than the audit established, and the fix is one flag.** The audit found that xterm *could* consume these keys; reading its compiled key table settled that it *does*. `@xterm/xterm/lib/xterm.js` contains `ctrlKey && !altKey && !metaKey && keyCode >= 48 && key.length === 1 → key` and `"_" === e.key && (key = C0.US)`, and it binds that handler on `this.textarea` — the `.xterm-helper-textarea`. So in the bubble phase `Ctrl -`, `Ctrl =` and `Ctrl 0` would each have typed their character into the focused agent's PTY and `Ctrl _` would have sent 0x1F. `useZoom` registers with `capture: true` and calls `stopPropagation()`, and `claims the keys before a descendant listener can see them` is the guard. Recorded here as well as above because the platform split is what makes it dangerous: macOS is excluded by that branch's own `!metaKey`, so the bug would have been invisible to a Mac reviewer.

**The panel could not reach the hook, and that is what widened the contract.** Not anticipated when the PRD was written: the keybindings own the level in `useZoom`, but the Settings row is an ordinary `SettingsPanelProps` panel whose only channel is `onSave`. Making that work meant `useZoom` adopting an externally-changed stored level — which introduced a race the PRD never mentions, because it did not exist yet. The document trails the hook by up to one persist interval, so mid-burst it holds an *older* level than the screen, and a sync effect adopting it unconditionally would drag the level backwards under a held key. The `lastPersisted` echo guard is the fix and `is not dragged backwards by a document that is still catching up` is its test. The separate contract widening (a `mode` prop) is recorded in [The Settings section](#the-settings-section).

Three smaller findings, all recorded where they belong rather than only here. `ZoomLevel`'s `Deserialize` had to be a visitor rather than `f64::deserialize().map(snap)`, because a hand-edited `level = 1` is a TOML **integer** and `f64` rejects one — and that rejection is not local, since `load_from` treats an unparseable document as "use defaults", so one plausible edit would have silently reset the user's appearance as well. `DesktopSettings` lost its `Eq` derive, because an `f64` is `PartialEq` and not `Eq`; nothing keys a map on the document, so no caller noticed. And `refitAllTerminals` iterates a snapshot with a per-entry re-check, because a `fit()` can trigger a resize that unmounts a sibling.

**Two tests were inverted rather than deleted**, following M7's precedent in PRD #745. `collapses for the real registry today, because it has one section` and `App.test.tsx`'s matching assertion both pinned the settings sheet's *collapsed* layout. The Zoom row made the registry two rows, the column came back with no change to `SettingsSheet.tsx`, and both tests now pin that — which is the claim the collapse was built to make good. The stub-driven coverage of both directions stays, so this did not become the column's only test.

**One test was dropped as unreachable rather than made to pass.** A case asserting the panel's change handler snaps an off-ladder value cannot be driven: `fireEvent.change` will not set a `<select>` to a value outside its options. The `clampZoom` in that handler stays — it is defensive, and the level goes straight to disk — but a test that cannot reach it asserts nothing, so the file records why there is none instead.

**What is not done: the manual smoke check.** M6 wrote it — six steps, in `docs/develop/desktop-gui.md` — and it has not been run, because this environment has no display. It is the only thing covering the five claims in the Testing table, and one of its steps is explicitly platform-conditional: the "nothing was typed into the PTY" check passes on macOS for the wrong reason, so it has to be run on Linux or Windows to mean anything.

### 2026-09-04 — Greptile's review: two real bugs, both now covered

Three findings on PR #880, and the two P1s were genuine defects that the suite which shipped them did not cover. Both fixes were mutation-tested — the fix reverted, the new test confirmed red, the fix restored.

**A stale keyboard write could overwrite a Settings choice, and drag the window back with it.** Hold a zoom key, then pick a level in Settings. The keyboard burst has a timer armed holding a pending level; the sync effect adopts the new stored level but left that timer alone, so 400ms later the *older* keyboard level was written over the user's choice. The second half is worse than the first: because the write moves `lastPersisted`, the next render adopted backwards and the window visibly fell back too, and the wrong level was restored at the next launch. An external adopt now cancels the pending flush — nothing needs writing in its place, since whoever changed the document has already persisted it. `drops a pending keyboard write when the document changes underneath it` is the guard.

**Closing the native window could lose a burst's trailing value.** The flush was on the React unmount alone, and closing a Tauri window tears the webview down without unmounting anything. Now flushed on `pagehide` and on a `visibilitychange` to hidden, which are the teardown signals a webview actually gives us.

**The residual gap there is stated rather than closed, and the alternative was rejected on its merits.** `save` starts an asynchronous IPC that shutdown does not await, so a hard kill inside that round trip can still lose the last change of a burst. Two things bound it: the leading-edge write means a single deliberate keypress is already on its way to disk, so what remains at risk is the tail of a *held-key* burst. Greptile's suggested fix — persist Rust-side inside `desktop_set_zoom` — was not taken, for two reasons beyond cost. It would make the keys and the Settings row write through different code, losing the property that the two cannot disagree about the level; and it would turn every keystroke into a read-modify-write of the whole document, which is more disk traffic rather than less. A genuinely durable close needs Rust-side close interception that awaits a webview round trip, which is disproportionate machinery for a zoom level.

**The P2 was valid and is fixed by wording rather than detection.** The new overlay row hardcoded `⌘`, which is misleading on Windows and Linux. It now reads `⌘/Ctrl`, which is *accurate* rather than a compromise: `zoomIntentFromKey` accepts either modifier on either platform, so there is nothing to detect. The pre-existing `⌘ K` row has the same problem and is deliberately untouched — it is a real platform binding and would want detection, which makes it part of Open Question 5 rather than a line to change here.

### 2026-09-04 — The user asked whether zoom works on every screen. It did not.

Reported against PR #880: the zoom keys appeared not to work on the Overview screen. They did not, and the cause is one this PRD's own design section had not considered — **where the hook is called from**, as distinct from what it does.

`DeckShell` renders `AgentOverview` **instead of** `ControlDeck`, not around it, so with `useZoom` called inside the deck the listener was unmounted the moment the overview came up. Every design decision in this document was about the *content* of the hook — the capture phase, the ladder, three coalescers — and none of them was wrong; the hook was simply not running.

**The symptom is easy to misread, which is worth recording separately.** A webview zoom is a property of the *webview*, not of anything React renders, so a level already applied stayed applied on the overview. Only the keys stopped responding. Anyone checking "is the overview zoomed?" by looking at it would have seen the right answer and concluded the feature worked.

Followed `reproduce-first`: extended the existing `DeckShell` block in `components/AgentOverview.test.tsx` rather than writing a parallel test, asserted at the reporter's altitude (a keystroke reaches the webview, not "a listener is registered" — the second can be true while the first is not), and added the deck case beside it as a **control**. The red was `0 calls` on the overview against `1.1` on the deck, which is what makes the attribution "the overview does not bind zoom" rather than "zoom is broken". The fix is load-bearing: moving the call back into the deck body turns the overview case red again and leaves the control green.

**The fix hoists the settings document too**, because `useZoom` needs it and it is app-wide rather than the deck's. `ControlDeck` is now a three-line wrapper owning a settings state for standalone and test use, with the body as `DeckSurface`. The alternative — an optional `settings` prop on the body falling back to its own hook — was rejected for a trap rather than for cost: it leaves two live settings instances, one rendered by nothing, so a later `save` against the wrong one writes state no screen reads. This way there is exactly one instance per tree, and it kept all 37 existing `ControlDeck` call sites untouched, which matters while #869 is still open on the same test file.

**One adjacent finding, deliberately not fixed.** `applyAppearance` is still called from inside the deck body and so has exactly the same shape. It is **latent rather than live**: `initialView` defaults to `deck` and nothing outside the tests sets it otherwise, so the only route to the overview is the rail button *from* the deck, by which time the effect has run and written `data-theme` on the document root, where it persists. It becomes a real bug the moment the overview becomes the landing screen — which is PRD #745 iteration 3's stated plan. Now that the settings state is hoisted, fixing it is moving that one effect up beside `useZoom`; it belongs to #743/#745 rather than here, and is recorded so it is not rediscovered.
