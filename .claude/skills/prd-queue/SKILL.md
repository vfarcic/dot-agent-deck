...
**When `HEAD` is `main`, that status output is empty, and the ahead count is `0`, fast-forward it, confirm it succeeded, and say so.** No prompt, no question — an up-to-date base is the default here, and the runner is told what happened rather than asked to authorise it:
...
- **The merge itself fails despite every precondition passing** — a fast-forward that would clobber a file `origin/main` has since started tracking is the concrete case, and it is reachable precisely because untracked files are not a blocker above. Treat it exactly like the three preconditions: report the git error and do **not** proceed to dispatch. **Decide on the exit status, never on the output** — git prints `Updating <old>..<new>` *after* `Aborting`, so a refusal ends in a line that reads exactly like a successful fast-forward. Verified rather than inferred, with an untracked file at a path the remote had begun tracking: exit `1`, `HEAD` unmoved, working tree untouched and no `MERGE_HEAD`, so `--ff-only` leaves nothing to undo.
