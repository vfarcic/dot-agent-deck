...
**When a precondition fails, do not touch the checkout.** That is the case the old rule was written for and it is unchanged. Say which precondition it was, in these terms:
...
- **The merge command itself fails despite every precondition passing** — a fast-forward that would clobber a file `origin/main` newly tracks is the concrete case. Treat that failure exactly like the three above: report the git error and do not proceed to dispatch. `--ff-only` never partially applies, so the checkout is unchanged and there is nothing to undo.
