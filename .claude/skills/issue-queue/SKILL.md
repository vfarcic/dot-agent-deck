...
- **Where those gates may run.** `.github/workflows/ci.yml` carries a bare `workflow_dispatch:`, so a unit can push its branch and have the full matrix run on GitHub's runners instead of this box: `gh workflow run ci.yml --ref <branch>`, then `gh run list --workflow ci.yml --branch <branch> --limit 1`. Put the *when* in the task, not an *often*, because the bare capability is a net negative — **use it when the box is loaded with other units compiling, when the change needs `build-macos` or `build-windows`, or as a final pre-PR sweep; never as the per-edit gate**, where a round trip with a 9.5-minute median replaces a 9.3–10.4s warm clippy. Tell the unit it relocates rather than removes work, that the expensive half is compiling rather than testing, and that a dispatch covers neither lane 2 nor anything it has not pushed. [`docs/develop/ci-on-demand.md`](../../../docs/develop/ci-on-demand.md) is the reference; point at it rather than restating it.
