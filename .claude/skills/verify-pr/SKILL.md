...
**This skill does not run lane 2, and NOTHING ELSE DOES EITHER — state that gap in the report.** The 24 real-agent files run in no CI job: no test that reaches a real agent runs on a runner, and no agent credential is registered on this repository (CLAUDE.md rule 5 has the decision and its two reasons; [`docs/develop/e2e-lanes.md`](../../../docs/develop/e2e-lanes.md) has the operational detail). So there is no label to apply and no workflow run to read. If the PR under review touches real-agent paths (spawn, hooks, delegate, the adapters), either run the covering tests yourself with your own credentials —
