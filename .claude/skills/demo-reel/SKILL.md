...
Any single gap is clamped to `IDLE_CAP`, which is what kills dead air while still reading as a pause.

**The final frame is held for a sized beat, not `agg`'s default.** Left alone, `agg` holds a clip's last frame for its default `--last-frame-duration` (3s in the pinned 1.9.0) *on top of* whatever tail the cast already spends on that final state — issue #365 measured PRD #341's reel ending on 5.27s of one identical frame, ~2.2s of cast tail plus `agg`'s hold. So `reel.sh` sizes the hold per clip. It asks `retime.sh --trailing` how many seconds the retimed cast runs after its last visible change, renders the cast with that tail cut out (`retime.sh --cut-tail` moves the tail's events onto the last change's timestamp, so their cursor state still lands in the final frame), and passes `--last-frame-duration max(CLIP_FINAL_DWELL, tail + CLIP_LAST_FRAME)`. The tail is folded into the hold rather than left in the cast because `agg` emits a frame only when the image changes: a tail of ticks that move nothing renders as *no* time at all, while one whose ticks move the cursor renders up to the last move, and telling the two apart takes a terminal emulator. `CLIP_LAST_FRAME` (default **1s**) is the closing beat after the tail; `CLIP_FINAL_DWELL` (default **2s**) is the least total time the final state stays on screen. The floor is for a cast that *ends* on its payoff — a test that stops recording the moment its assertion matches paints the answer at or near its last event, with little or no tail, and would otherwise show it for about one second. At the defaults, a cast with a tail of at least 1s gets nothing from the floor, so a clip with issue #365's ~2.2s tail ends on ~3.2s of final state instead of 5.27s. Both are env-overridable, in seconds of reel time (the tail is divided by `CLIP_SPEED`, which `agg` applies to the cast but not to its last-frame hold). Cards are unaffected: their hold is `CARD_HOLD`, enforced at the ffmpeg level.

A *visible change* here is any event that is not **inert**, and inert is deliberately narrower than the re-timer's `tick`: only SGR attributes (which affect later writes, not what is painted), cursor show/hide, and cursor positioning or movement — the render-loop tail. A `tick` is anything that prints no characters, which also covers erase, scroll and newline; those do change the screen, and counting them as tail would overstate how long the final state has been up and shorten its hold.
