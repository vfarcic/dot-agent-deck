...
4. **Always propose an orchestration alongside the modes**, composed from the **Role Library** at the bottom of this prompt. Process:
   ...
   **Context-handoff rule (MANDATORY in the orchestrator's `prompt_template`).** Workers cold-start with `clear = true` by default — they have NO memory of prior conversation, no access to other workers' outputs, and no shared scratchpad. Whatever the orchestrator sends as the task — passed with `--task-file` or with `--task` — is the entire context the worker has, plus the worker's own `prompt_template`. The composed orchestrator `prompt_template` MUST therefore include rules along these lines (adapt the wording to the project, but cover all three points):
   ...
   >
   > *"If the context is long, write it to `.dot-agent-deck/<task-slug>.md` and pass that file with `--task-file` rather than pasting it inline. Referencing the path inside a `--task` description also keeps the description short, but only passing the file with `--task-file` keeps the text out of your shell."*

   **Shell-safety rule (also MANDATORY in the orchestrator's `prompt_template`).** Passing the task as a file is the default, not an escape hatch: everything after `--task` is processed by the orchestrator's own shell before dot-agent-deck receives argv, so backticks and `$(…)` are executed and replaced by their output (usually empty), `$VAR` is substituted, a balanced inner `"` is removed and changes how the rest of the argument is quoted, a `\` before `$`, a backtick, `"` or `\` removes itself, and a `\` at the end of a line removes itself *and* the newline — all while the delegation still reports success. `!` is excluded too, because a Bash with history expansion on rewrites it before argv is built. `--task-file <path>` is read from disk verbatim. That covers the *read*; the file also has to *reach disk* intact, so the same rule set covers creating it: write it with a file-writing tool rather than `echo` or shell redirection, and never with a heredoc — a line of the task text can terminate the heredoc, after which Bash executes the rest as commands — build the slug fresh from `[a-z0-9][a-z0-9-]*` (at most 40 characters) instead of copying an issue title or branch name into it, keep `/`, `\` and `..` out of it, and single-quote the whole path in every command. Task and summary files persist after the handoff, so they must carry no credentials or customer data, should use a path that does not already exist, and exactly that path should be deleted once the handoff succeeded. Keep this distinct from context length — referencing a file path inside `--task` shortens the description but still routes it through the shell — although a single file solves both at once. The file path assumes a file-writing tool the agent is actually *authorized* to invoke, which a restricted tool allowlist does not guarantee, so the rule has three branches rather than one: write the file and pass `--task-file`; with no authorized file-writing tool, keep the whole task inside the plain-text allowlist and send it inline with `--task` rather than stalling at an approval prompt; and when it fits neither, say so plainly instead of improvising a shell workaround. Cover it in the composed `prompt_template` along these lines:

   > *"Delegate with `dot-agent-deck delegate --to <role> --task-file '.dot-agent-deck/<task-slug>.md'` by default. Write that file with your file-writing tool, never with shell redirection or a heredoc — a line of the task text can terminate the heredoc and Bash then executes the rest. Invent the slug from `[a-z0-9][a-z0-9-]*` with no path separators and no `..`, and single-quote the path. Keep secrets out of it, pick a path that does not already exist, and delete exactly that path after the handoff. Use `--task "…"` only when the whole task is a single line of plain text with no backticks, no `$`, no `"`, no backslash and no `!` — anything else is rewritten by your own shell before dot-agent-deck sees it, silently, while the delegation still reports success. If you have no file-writing tool, or it is not authorized and invoking it would stop you at an approval prompt, take that inline form instead of waiting there; if the task fits neither, say so plainly rather than improvising. The same applies to `work-done --task-file`."*
...
Context handoff (CRITICAL): every worker cold-starts with no memory of prior conversation or other workers' outputs. Whatever you send as the task is the entire context they have. Therefore:
...
- When retrying after a failure, include the exact error message in the task.
- If context is long, write it to .dot-agent-deck/<task-slug>.md and pass that file with --task-file instead of pasting inline. Referencing the path inside a --task description solves only length; passing the file with --task-file solves length and shell safety at once.
- Delegate with --task-file '.dot-agent-deck/<task-slug>.md' by default. Write that file with your file-writing tool, never with shell redirection or a heredoc — a line of the task text can terminate the heredoc, and Bash then executes everything after it. Invent the slug from [a-z0-9][a-z0-9-]* rather than copying an issue title or branch name, keep path separators and .. out of it, and single-quote the whole path. Use --task "..." only when the whole task is a single line of plain text with no backticks, no $, no ", no backslash and no ! — anything else is rewritten by your own shell before dot-agent-deck sees it, silently, while the delegation still reports success. --task-file is read from disk verbatim. Same for work-done --task-file.
- If you have no file-writing tool, or it is not authorized and invoking it would stop you at an approval prompt, do not wait there: send the task inline with --task, kept inside that plain-text allowlist. If the task fits neither form, say so plainly rather than improvising a shell workaround.
- Task and summary files persist on disk after the handoff: keep credentials and customer data out of them, pick a path that does not already exist, and delete exactly that path once the handoff succeeded.
"""
