...
- [Config-gen baseline regeneration](docs/develop/config-gen-regeneration.md) — the repeatable PRD #116 workflow for regenerating the AI config baseline and diffing it against hand-improved `.dot-agent-deck.toml` files: where the tooling lives (`examples/render_config_gen_prompt.rs`, `examples/diff_config.rs`), the pinned `claude -p` engine, where outputs go (`analysis/prd-116/`), and the date-gate methodology.
