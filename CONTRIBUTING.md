...
- [Agent-driven notifications — dogfood setup](docs/develop/notifications-dogfood.md) — how this repo's own orchestration notifies you out-of-band when a run reaches a user gate, finishes, or gets blocked (PRD #126), using only `scripts/notify.sh` and the role `prompt_template`s in `.dot-agent-deck.toml` — zero deck code: the ntfy topic and its public-topic caveat, how to subscribe a device, the `.dot-agent-deck/notify-log.md` expectation log that makes missed notifications observable, and the post-run reconciliation procedure.
