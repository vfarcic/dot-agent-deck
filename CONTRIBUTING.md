...
- [CI entrypoints — running a CI step locally](docs/develop/ci-entrypoints.md) — the `task ci-*` wrappers around every command CI runs (PRD #376): which `ci.yml` job owns each step, how to run one or all of them from the devbox shell, why the split is one task per step rather than one opaque `task ci`, why `ci.yml` is deliberately *not* routed through them (it is the merge gate and the measurement control), the local-pins-versus-floating-CI-toolchain skew this addresses, and the status of the unproven Semaphore pipeline in `.semaphore/`.
