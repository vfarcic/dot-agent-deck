...
3. Run `cargo xtask linkage-check` locally — it verifies the annotation matches the catalog, the function name carries the required prefix, and (for `e2e_*.rs` files) no raw `sleep` / fixed-count polling crept in. If the new ID was previously on `xtask/linkage-check/m2.allowlist`, delete that line.
