...
//!
//! PRD #176: extracted into the shared `keybindings` workspace crate so the TUI
//! binary AND the desktop GUI core resolve the SAME bindings from ONE source
//! (the user's `~/.config/dot-agent-deck/keybindings.toml` plus identical
//! defaults) — keeping shortcuts byte-for-byte identical across both
//! front-ends. The binary's `src/keybindings.rs` is now a thin re-export shim.
//!
//! This crate is intentionally free of any TUI / event-loop wiring (that is
//! Round 3). It owns:
//!
...
fn keybindings_path() -> PathBuf {
    ...
    home_dir().join(".config/dot-agent-deck/keybindings.toml")
}

/// `$HOME`, falling back to `/`. Replicates `config::dirs_home` (PRD #176): the
/// shared `keybindings` crate must not depend on the binary's `config` module,
/// and the path scheme is stable, so this 3-line helper is duplicated rather
/// than coupling the crate back to the binary.
fn home_dir() -> PathBuf {
    std::env::var("HOME")
        .map(PathBuf::from)
