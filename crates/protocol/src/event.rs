//! Hook-event and orchestration-signal payloads carried over the daemon
//! socket. Extracted verbatim from the binary's `event` module (PRD #176 M1.1)
//! so every client shares one definition.
...
impl std::fmt::Display for AgentType {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            AgentType::ClaudeCode => write!(f, "ClaudeCode"),
            AgentType::OpenCode => write!(f, "OpenCode"),
            AgentType::None => write!(f, "No agent"),
        }
    }
}
...
pub struct AgentEvent {
    ...
    /// `crate::record` pane-id capture). Lets the
