...
**A pane on the alternate screen is a third state.** Full-screen interfaces — a picker, a permission dialog, an editor — switch to a second screen that keeps no scrollback of its own, so while a pane is there the deck cannot reach the history it holds for it. Nothing is lost: leaving that screen brings every line back. A scroll there does nothing quietly, without the notice, because the history does exist — the deck just cannot see it from where the pane currently is.
