...
**For an agent like that, Agent Deck has nothing to scroll — by wheel or by key, in any mode.** This is not a setting you can change and there is no alternative chord that reaches further back: `PageUp` does nothing for the same reason the wheel does. Rather than doing nothing silently, the deck says so: a scroll that cannot land briefly overlays the pane with `This agent redraws in place and keeps no scrollback — there is nothing to scroll`, in the same reversed single-line style as the command-mode banner. It clears itself after a moment or on your next keystroke, and that keystroke is not swallowed — it reaches the agent, or runs its shortcut, exactly as it would have. A pane too narrow for the full sentence gets the short form, `Nothing to scroll — this agent keeps none`. You will only ever see it on a pane you actually tried to scroll; it never appears on its own.
