...
**For an agent like that, Agent Deck has nothing of its own to scroll, in command mode, by wheel or by key.** That is not a setting you can change: in command mode `PageUp` does nothing for the same reason the wheel does. What it does *not* mean is that nothing can reach further back — the paragraph above is the qualification and it applies here too. Press `Enter` to focus the pane and the same keys reach the agent as `ESC[5~` / `ESC[6~`, where the agent's own scrollback answers them if it keeps one. That is a different mechanism with different history behind it, not a workaround for this one. Rather than doing nothing silently, the deck says so: a scroll that cannot land briefly overlays the pane with `Nothing to scroll — this pane has no scrollback to move through`, in the same reversed single-line style as the command-mode banner. It clears itself after a moment or on your next keystroke, and that keystroke is not swallowed — it reaches the agent, or runs its shortcut, exactly as it would have. A pane too narrow for the full sentence gets the short form, `Nothing to scroll — no scrollback`. You will only ever see it on a pane you actually tried to scroll; it never appears on its own. The message reports what the deck observed about that pane and stops there — *why* a particular pane has no scrollback is the per-agent distinction this section explains, and the deck cannot establish it from the pane's output alone.

**A pane on the alternate screen is a third state, and the deck stays quiet in it** (issue #891). Full-screen interfaces — a picker, a permission dialog, an editor — switch the terminal to a second screen that by definition keeps no scrollback, and while a pane is there the deck can reach none of the history it holds for it. That history is not lost: it is on the normal screen, and leaving the alternate screen brings back every line. So a scroll that cannot land *there* is reported as nothing at all rather than with the notice above, because "this pane has no scrollback" would be false — the deck simply cannot see it from where the pane currently is.
