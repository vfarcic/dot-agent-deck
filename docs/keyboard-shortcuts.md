...
**The wheel goes to whatever the pointer is over.** It scrolls the focused pane while the pointer is inside that pane — or a mode tab's side pane when the pointer is over one. Anywhere else, the wheel does nothing: over the deck's card list, the stats bar, the tab bar, the bottom button bar or a pane's border it is dropped, the same way a click that lands on nothing is. Point at the pane you want to scroll. (The card grid has no scroll of its own — it moves only as a consequence of moving the selection.)
