...
Recent versions do not write such a path. An **installed** deck is preferred over whatever happens to be running — `~/.local/bin/dot-agent-deck`, or a `dot-agent-deck` in a directory on your `PATH` — so a deck launched from a build directory, or (since [#1140](https://github.com/vfarcic/dot-agent-deck/issues/1140)) from a copy parked somewhere scratch such as a branch build under `/var/tmp`, resolves your install and writes that instead of pinning itself. If there is no install anywhere to find, a build directory is still refused outright — `hooks install` fails with a message telling you what to install, and writes nothing rather than leaving a command that will break — while any other path is used as a last resort, with a warning in the log saying it was pinned without being vouched for. That is what keeps hooks working on a machine whose only deck is the one running, such as a desktop app starting its bundled daemon. They also repair themselves: on the next launch, a deck-owned hook whose binary is confirmed missing is rewritten to the durable path, for Claude Code's `settings.json`, Codex's `hooks.json`, Devin's `config.json` and the OpenCode plugin alike. A hook whose path still **works** is deliberately left alone even if it differs from what the deck would write — your own wrapper, or a second checkout you are deliberately using, is not something a startup should quietly repoint. That now holds for all four. Codex and Devin used to re-pin their own rules to the resolved durable path on every launch, which was not a harmless one-time correction if you launched the deck from more than one install: two checkouts, or an installed deck and a checkout, resolve *different* durable paths, so the pin never settled — it **flapped** between them, following whichever install you launched last ([#730](https://github.com/vfarcic/dot-agent-deck/issues/730) closed that). What "left alone" means in practice is worth knowing, because it is not "adopted": the still-valid rule stays exactly where it is and the launching deck's rule is added **beside** it, so that agent fires two deck hooks per event — one per install — until you remove whichever one you no longer want. Read "one per **install**" literally: this is what made #1140 visible, where a scratch copy of the deck was treated as an install and every agent ended up with two full sets of entries. Two genuine installs still do this, deliberately; a scratch copy no longer counts as one and so adds nothing. A hook of your own that merely mentions `dot-agent-deck` is never touched, on any of the four; and a handler of your own sharing one rule object with a deck command is no longer taken with it — install and uninstall remove the deck's command alone and leave yours, with its matcher, in place.
...
If that listing shows a second path pointing at a copy you parked somewhere — the #1140 shape — an upgraded deck will not add to it, but it also will not remove it while that copy is still on disk, because a hook command that still *works* is left alone by design (the paragraph above). Delete the copy and start the dashboard once: the self-heal then sees a deck-owned rule whose binary is confirmed missing and rewrites it. Editing the entry out by hand is equally fine.
