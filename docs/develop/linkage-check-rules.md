...
1. **Write the rule.** Two shapes exist, and the second is the default for anything new.
   ...
   - Its **own module**, `xtask/linkage-check/src/<rule>.rs`, exposing `pub fn run(root: &Path) -> Vec<String>` — one string per finding, no `[N]` prefix. Use this whenever the rule reads its own files: `desktop_project_boundary` (12), `git_program_literal` (13), `voice_command_registry` (14) and `site_image_refs` (16) all do. Declare the module in `main.rs`'s alphabetical `mod` list with a doc comment saying what it is for.
...
6. **Give it an opt-out only if an exception is a local judgement call.** Rules 8, 10 and 16 take a marker on the offending line (`linkage-check:allow-bare-tempdir`, `linkage-check:allow-unarmed-agent-spawn`, `linkage-check:allow-missing-image`), so the exception is declared where it is taken and review meets it in the diff that needs it. Rule 11 deliberately has none: it guards a pair of numbers that must move together, so the only correct response is to raise the constant.
