...
**PRD #882 changed who decides, and this is the single most important thing to know about invariants 2 and 3.** A PTY has exactly one window size, so every client attached to an agent sees the same grid — per-client sizing of a live TUI is unavailable, not unimplemented, because the agent lays out for one `TIOCGWINSZ` answer and emits absolute cursor positioning for that grid. The daemon therefore owns the geometry: it sizes each agent to the **last-focused client's viewer size** or, when no client has claimed focus or that client has no measured viewer of the agent, to the **smallest viewport among its attached viewers** (PRD #1105 M11, falling back to PRD #882). A client that does not decide the size pads the remainder where its pane is larger and clips where it is smaller. The layout rect is now what a client **asks for**, not what it gets.

Two consequences follow, and both are load-bearing:

- **The delta check compares against the last REQUEST, not the parser.** `resize_panes_to_layout` used to compare its target against the pane's local vt100 parser, which worked while `resize_pane_pty` set that parser synchronously — "kept in lockstep with the PTY" was true for exactly as long as one client existed (issue #883). The parser now holds the daemon's ANSWER, which legitimately differs from the target whenever another client decides the size, so a parser-based comparison would find a delta on every frame and re-send forever. `EmbeddedPaneController::requested_dims` is the comparison.
...
**The invariant did not disappear; it moved to the side that can state it.** The daemon computes the applied geometry from its registered viewers, and the policy is enforced and tested there. What remains in the widget is the rendering rule that follows from it — and `min(area, screen)` is now the **designed path** rather than a defense against an upstream failure. A pane narrower or shorter than its box is what a correctly applied policy looks like from inside a client that is not the smallest one, and since PRD #1105 a screen larger than its box, cut off at the right and bottom, is what it looks like from inside a client that is not the last-focused one. Until PRD #1105 this paragraph said the daemon enforced "the PTY is never larger than any viewer's pane"; that now holds only under the fallback.
