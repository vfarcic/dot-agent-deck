...
- **`role-set`** (reverse `generic` runs). It is read after tells 1 and 2, behind a pre-connect assertion that re-proves every listening socket in the namespace is the daemon's. Its presence clauses need positive evidence: each role listed exactly once and on the pane recorded after the detach — so a role missing, doubled or moved fails it — and each listed pane backed by a live process under the daemon in a `/proc` census. Its two absence clauses (no pane process outside the daemon's tree, no other `daemon serve` process) are read from that same census, which must contain the daemon itself, so an empty or foreign census cannot satisfy them. Its timing rests on the old TUI's startup order rather than on a pause: v0.41.0 makes its restore decision before its first frame, and by then the TUI has drawn the orchestration and been driven through tells 3 and 4. A listing or census that cannot be read is **not checked**.
...
The four tells are rule 12's and reverse asserts them. They are not enough on their own, because a delegate and two hooks never reach, say, the line that escapes a hostile id in a log record. A reverse run therefore also carries a **probe**: the minimal stimulus that reaches the branch's changed arm, with an assertion on what the branch daemon then did. It is selected by the branch name's issue number (`--probe auto`, the default), or named with `--probe`; `--probe generic` sends no stimulus of its own and adds one tell, `role-set`, and a branch no probe is known for gets `generic`. The evidence file says which. Probes run only in reverse, and an explicit probe with `--direction forward` is refused rather than dropped. `generic` runs in every endpoint mode; `discovery-fallback` needs `resolved` with `--unset-xdg-runtime-dir`; every other probe needs `sandbox-sockets`, the mode it was written against.
...
| --- | --- | --- | --- |
| `generic` | any; the [#1179 negative control](#negative-controls) | none beyond the four tells | the four tells, plus `role-set`, read after tells 1 and 2: the daemon lists each role exactly once, on the pane it had after the detach; each listed pane has a live process under the daemon; no process carrying a pane id is outside the daemon's tree; and no `daemon serve` process runs besides the daemon and its forks. It exists because tell 1 catches a second daemon but not a role set the old TUI's session restore spawned into the existing one. On the cannot-discover path it is not measured; the collateral tells are |
...
- **A real agent.** Every role is a stand-in (`sh`, `cat`), deliberately: this check is about the TUI↔daemon wire, so no *agent* credential is used or needed. (`gh release list` and `gh release download` use your GitHub credential; `--old-binary` with an explicit `--previous` calls neither.) It therefore says nothing about how a real agent behaves across the version boundary, and it is not a substitute for the lane-2 real-agent tests CLAUDE.md rule 4 asks for.
...
| `--previous` | the previous release tag. When omitted it is resolved from `gh release list`, not defaulted, and a run that cannot resolve it stops and asks for this flag — see [Which release is "previous"](#which-release-is-previous) |
| `--old-binary` | use a binary already on disk instead of downloading a release (its version is recorded, not enforced); requires an explicit `--previous`, which then only labels it |
...
| `--probe` | the reverse run's branch-specific stimulus: `auto` (default, from the branch's issue number), `generic` (no stimulus: the four tells plus `role-set`, in any endpoint mode — how [the #1179 control](#negative-controls) runs), or a probe's name; `discovery-fallback` needs `resolved` with `--unset-xdg-runtime-dir`, the rest `sandbox-sockets` |
