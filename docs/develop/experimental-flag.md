...
With the flag off the desktop shows the agent overview (with its full-window agent pane) and Settings; the rail reads **Dashboard** and **Settings**. With it on, the rail also carries Daemons, Projects, Prompts, Orchestrations and Agent Profiles, and the dashboard's **Open daemons** buttons come back.
...
| `show_desktop_deck()` | The desktop app's deck — the multi-pane terminal screen: its rail entry, the dashboard's **Open daemons** buttons, a requested deck view (shown as the overview instead), and voice's `open_deck` (offered `callable: false` and refused at dispatch) | #1198 | `graduate-desktop-deck` |
...
| `show_desktop_orchestrations()` | The desktop app's Orchestrations editor: its rail entry and command-palette entry | #1198 | `graduate-desktop-workflows` |
...
> **The five `show_desktop_*` wrappers reach the webview as one DTO field each.** The desktop crate's `desktop_features` command returns them as `DesktopFeatures` (`showDeck`, `showProjects`, `showPrompts`, `showOrchestrations`, `showAgentProfiles`); the webview reads it once into `DeckRuntimeState.desktopFeatures`, and every TypeScript gate reads `desktopFeaturesOf(runtime).show<Surface>`, so `grep -rn 'show_desktop_deck\|showDeck' desktop/` finds every site of one surface. A missing or refused answer reads as all hidden. Two things are deliberately not gated: the panels' in-deck doors (the run graph's **Edit loop**, the empty deck's **Configure agents**, and the Projects ↔ Orchestrations cross-links inside those panels), because each sits inside a surface that is itself hidden and all five fields follow the one flag; and the voice rows' own vocabulary, which stays in `commands.toml` — only whether `open_deck` is offered changes.
