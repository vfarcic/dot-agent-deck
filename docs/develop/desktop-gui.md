...
3. **Secrets never go in the document, and never in `localStorage`.** The document may hold a non-secret *reference* — which backend holds the key, or a boolean saying one is stored — and nothing more. A real credential belongs behind the `SecretStore` seam PRD #803 names (store/load/delete keyed by a stable identifier, OS keychain as the intended implementation; #802 picks the backend). There is a check, and **it is a naming tripwire, not a security boundary** — read that sentence before you rely on it. `no_settings_key_name_trips_the_credential_tripwire` in `settings.rs` fails the build on a key *name* containing `key`, `token`, `secret`, `password`, `credential`, `authorization`, `bearer` or `passphrase`, with an allowlist of full key **paths** (`voice.secret_backend`, not `secret_backend`, so an exception cannot travel to a same-named field in another section) for genuine references. What it cannot see: a field called `endpoint` or `value` whose *value* is a token, a field serde omits from the default document, anything the TypeScript DTO carries that the Rust schema does not, and any value at all — it never looks at one. Read a pass as "nobody named a field like a credential", never as "a credential cannot get in here". If your name really is a reference, add its full path to `SECRETISH_ALLOWED` with a comment saying which of the two allowed forms it is; if you are about to store real credential material, [#827](https://github.com/vfarcic/dot-agent-deck/issues/827) is the list of checks that actually have to exist first.
