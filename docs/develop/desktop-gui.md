...
3. **An older daemon.** It never reaches a project verb at all. The desktop's handshake check is exact equality on `PROTOCOL_VERSION`, which #819 moved 8 → 9, so a v9 desktop refuses a v8 daemon at the handshake with `protocol mismatch: desktop expects 9, daemon reports 8`, naming both versions; the build-stamp allowance runs after that check and does not bypass it. There is deliberately no local fallback — resolving client-side "when the daemon is local" would reinstate the silently-wrong-filesystem behaviour on the least-tested path.
...
2. **Every `localStorage` key** — `grep -rn 'localStorage\|sessionStorage' desktop/src/ | grep -v '\.test\.'`. There are **four** since #819 M6 removed `projects.v1`, every one of them written through `modeScopedKey` so a fixture visit cannot write a key live mode reads back.
...
### The four `localStorage` keys, and the one that is gone
...
**#824 says "the four `localStorage` keys" and there are again four — but not the same four.** `overview-columns.v1` was added by PRD #745 after #824 was filed, taking the count to five, and #819 M6 then removed `projects.v1` and brought it back to four. The two moves cancel in the total and not in the substance: the key that arrived is the one on the right side of the boundary, and the key that left was the one furthest from it. Both are recorded here because a count that drifts unremarked is how the next key lands unclassified.
...
The production filesystem surface of `desktop/src-tauri/src/` is **two** call sites, in one function — it was four in two until #819 M6 deleted `desktop_project_cwd()`. `grep -n 'std::fs' desktop/src-tauri/src/*.rs` finds **neither** of them outside `#[cfg(test)]`, because the probes go through `Path::is_file` and `Path::metadata` — which is worth knowing before trusting a `grep` for `fs` to have covered this; `grep -n 'is_file()\|is_dir()\|\.exists()\|metadata(\|read_dir\|canonicalize' desktop/src-tauri/src/*.rs` is the enumeration that works.

1. **Locating the bundled daemon sidecar** for **Replace daemon** — `is_executable_file` (`daemon_bridge.rs`: the `is_file` and the `metadata` inside it) and the `current_exe` sibling search that calls it, plus the `DOT_AGENT_DECK_BINARY` and `PATH` env lookups beside them. **Legitimate and permanent:** this is the client finding *its own installation*, which no daemon can tell it.
2. ~~**`desktop_project_cwd()`**~~ — two `is_file` probes for a `.dot-agent-deck.toml`. **This was the defect, and #819 M6 deleted the function.** The surface is therefore two call sites in one function, not four in two: everything left is source 1.
...
- **Remote daemons are untested rather than known-wrong.** A forwarded socket attaches and streams remote agents and their PTYs correctly. What made remote observation-only was that project resolution stayed client-side, so a remote workflow launch read the wrong `.dot-agent-deck.toml` and wrote coordinator context to the wrong host; #819 M6 moved both daemon-side, and the desktop now sends back only a path the daemon supplied or the user typed. What has not happened yet is the end-to-end confirmation against a real remote host — #819 M10 — so treat a remote launch as unverified until that run exists.
