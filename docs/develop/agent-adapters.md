...
2. It launches `codex` with `--dangerously-bypass-hook-trust` — but **only when the active `CODEX_HOME` contains no non-deck command hooks** (PRD #20 W3-Pass-2, security finding #2/M-1). Codex requires non-managed command hooks to be *trusted* before they run (an interactive `/hooks` review otherwise); because the deck **authors its own hook definition**, it vets the source — itself. The catch is that `--dangerously-bypass-hook-trust` is **invocation-global**: it trusts *every* enabled hook in that `CODEX_HOME`, not just the deck's. Codex 0.144.4 exposes no user-space *scoped* pre-trust the deck can safely use (managed/pre-trusted hooks require a root-owned `/etc/codex/{managed_config,requirements}.toml` or an MDM/cloud source; per-hook trust is a SHA-256 over the exact definition that only Codex itself should record — the app-server `hooks/list` reports each hook's `currentHash`/`trustStatus`, but forging a `trusted_hash` circumvents Codex's own trust gate). So the deck bypasses trust **only when the only thing the bypass would trust is its own vetted entry** — i.e. no third-party command hook is present in that `CODEX_HOME`. When a third-party hook *is* present, the deck does **not** bypass (it logs a warning) so a user's unreviewed hook is never silently trusted; the deck's own events then degrade to the coarse stdout classifier. (Residual: this check inspects `CODEX_HOME/hooks.json` only — project-local `<repo>/.codex`, plugin, and `config.toml`-defined hooks are not inspected; to fully auto-trust the deck's hooks alongside third-party hooks, trust the deck's hook once via Codex's `/hooks` review.)
...
The `--dangerously-bypass-hook-trust` flag is auto-injected only when the wrapper's **direct program is `codex`** AND the resolved identity is `AgentType::Codex` AND the active `CODEX_HOME` has **no non-deck command hooks** (see above) — e.g. `dot-agent-deck wrap --agent codex -- codex --model …` against a `CODEX_HOME` holding only the deck's own hooks. The wrapper appends the flag to codex's own argv, so it works for the common case (and the deck's default `codex` command). Note `dot-agent-deck wrap --agent claude -- codex` neither installs Codex hooks nor injects the flag — the codex-only trust bypass is gated on Codex identity, not just the program name.
