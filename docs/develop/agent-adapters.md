...
1. **The child took the inner PTY out of cooked mode** — it cleared `ICANON` and/or `ECHO`, measured against the pty's own `c_lflag` as `openpty` handed it over, sampled before the child ran. This is the definitive one, and it is the exact inverse of the defect this gate exists to prevent: PRD #225's prompt loss was bytes written into a line discipline that was still canonical with echo on, where the payload is echoed back and swallowed. A child reading raw keystrokes is, by construction, a program that *consumes* input rather than echoing it. Launchers never do this; a full-screen TUI does it as it initializes, before it paints. The master descriptor is what is read — a pty's termios is one shared structure, so the master's `tcgetattr` reports the discipline the slave-side child installed. It carries `session_start_origin: wrapper_interface_ready`.
2. **Output settled** — the child wrote something and then stopped for 750 ms. The fallback for an interface that stays in cooked mode (a line-oriented REPL, and the test stand-ins), and for the redirected-descriptor paths where there is no inner-PTY termios to read. The signal is the *settling*, not the first byte: a child still coming up is either silent (nothing fires) or still painting (each chunk pushes the deadline out), so what this detects is the transition from producing output to waiting for input. It carries a **different** value, `session_start_origin: wrapper_interface_settled`, because it is a guess and not an observation.
   ...
   **Fact 2 cannot tell a ready REPL from a stalled launcher, and must not be priced as if it could.** Silence says the child stopped writing; it says nothing about whether the thing that stopped is an interface waiting at its prompt or a launcher part-way through its own boot. The production launch shape is `devbox run codex-big`, whose measured wrapper→`node codex` gap is ~4 s — a launcher that prints one line and then evaluates its environment quietly for a second satisfies fact 2 while the pty is still canonical, which is PRD #225 Defect 1 exactly. So the daemon releases its wait on fact 2 (30 s spent waiting for a signal that never comes, then a blind write, is strictly worse) and keeps the post-readiness buffer over it. Only fact 1 skips the buffer.
...
**Measured on 2026-07-27 against codex-cli 0.145.0, and still true on 0.149.0: Codex does not post its native `SessionStart` when its TUI comes up — it posts it when the first *turn* starts.** A real `clear = true` delegate to a wrapped Codex worker, observed on the daemon's event broadcast: the wrapper's marked fork-time `SessionStart` at T+0, then the unmarked native `SessionStart` at **T+29.999 s** — 5 ms before the `UserPromptSubmit` that the gate's own injected prompt produced. Five later production samples landed at 31.2 / 31.2 / 31.7 / 31.7 / 32.3 s, the constancy being the tell that this was the timeout and not load. So the signal the gate waited for was a *consequence of the prompt it was gating*, which is why it never fast-pathed. Issue #243 is that fix: the wrapper's interface signal exists **before** any prompt, so the gate now releases on an observation instead of on a timeout, and the same delegate is delivered in **about a second** — fact 1 lands as soon as Codex's TUI initializes and skips the buffer; a release that comes on fact 2 instead pays the 750 ms settle plus the buffer. One consequence still stands: a test cannot wait for a genuine pre-prompt Codex `SessionStart` as a precondition for delegating — that is circular, since the event it waits for can only be caused by the prompt it is gating. Wait for the interface-ready one, or for what the pane is rendering.
...
1. the gate was released by **fact 1**, the raw-input-mode observation — fact 2's settle guess keeps the buffer;
...
Add the pure-data half too: `agent/readiness/001` pins every agent's `pre_prompt_readiness` value against the measurement it rests on, through an exhaustive `match` on `AgentType` — so a new adapter does not compile until somebody states what a fresh instance of it announces before its first prompt — `agent/readiness/002` pins that the gate's discriminator is that field and not `hook_install.is_some()`, by naming the shipped agents where the two disagree in each direction, and `agent/readiness/003` pins the `session_start_origin` truth table, i.e. which of the wrapper's markers answers the strong question the buffer skip reads.

**The three guards on the buffer skip have one test each, and a wrapper adapter must keep all three true.** `orchestration/delegate/026` pins that the weak output-settled fact still pays the buffer (a stalled launcher settles exactly like a REPL at its prompt, so the guess must not buy the skip); `orchestration/delegate/027` pins the other half end to end — a child that genuinely clears `ICANON`/`ECHO` skips the buffer, and the same skip still honours an operator-pinned `DOT_AGENT_DECK_DELEGATE_READINESS_BUFFER_MS`; `orchestration/delegate/028` pins that a marker posted for a pane the daemon did not itself spawn as a wrapper host keeps its buffer. `/027` is the one to watch when you change how a wrapper agent is launched: guard 2 reads the frozen `spawn_agent_type`, is fail-closed, and if it starts refusing honest agents the fast path dies **silently** — that test's control is the only thing in the suite that notices.
