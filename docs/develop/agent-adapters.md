...
**Measured on 2026-07-27 against codex-cli 0.145.0, and still true on 0.149.0: Codex does not post its native `SessionStart` when its TUI comes up — it posts it when the first *turn* starts.** A real `clear = true` delegate to a wrapped Codex worker, observed on the daemon's event broadcast: the wrapper's marked fork-time `SessionStart` at T+0, then the unmarked native `SessionStart` at **T+29.999 s** — 5 ms before the `UserPromptSubmit` that the gate's own injected prompt produced. Five later production samples landed at 31.2 / 31.2 / 31.7 / 31.7 / 32.3 s, the constancy being the tell that this was the timeout and not load. So the signal the gate waited for was a *consequence of the prompt it was gating*, which is why it never fast-pathed. Issue #243 is that fix: the wrapper's interface-ready signal exists **before** any prompt, so the gate now releases on an observation instead of on a timeout, and the same delegate is delivered in **~0.75 s**. One consequence still stands: a test cannot wait for a genuine pre-prompt Codex `SessionStart` as a precondition for delegating — that is circular, since the event it waits for can only be caused by the prompt it is gating. Wait for the interface-ready one, or for what the pane is rendering.
...
The **post-readiness buffer is scoped by what the gate established, not by which agent it is** — the only scoping that does not reintroduce #663. `DELEGATE_READINESS_BUFFER` (1000 ms, `DOT_AGENT_DECK_DELEGATE_READINESS_BUFFER_MS`) covers the gap between "a session exists" and "the TUI interprets `\r` as submit", and that gap is real for every readiness fact that predates #243: a native `SessionStart` (Claude fires it early in boot — at a 0 ms buffer the pointer was written into a still-booting Claude and dropped), a hookless wrapper's fork-time event, and the timeout fallback, which established nothing at all and is therefore *more* reason to wait. The one fact without that gap is the wrapper's interface-ready observation, so that is the one case the buffer is skipped for. The #199 escape hatch ("bump it and see if the prompt lands") consequently stays live for every case where the deck has *not* seen the interface, which is every case it was ever useful for; where the deck *has* seen it, the interface-ready event is on the wire and in the log, so "the agent never became ready" is answered by its absence rather than by tuning an interval.
