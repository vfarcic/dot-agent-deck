...
- **The attendance is a caller declaration**, `spawn::SpawnRequest::compose_orchestrator_context: Option<Attendance>` — `dispatch` is the one producer that passes `Some(Unattended)`. It is not inferred from "a task was supplied": the desktop's live-loop launch requires a task prompt and has its operator watching the panes, so that proxy would strip the gate the operator is present to answer. A compaction or `/clear` re-arm recovers the attendance out of the published file (the `## Unattended run` heading in the prefix ahead of `## Your task`), the same way it already recovers the task.
