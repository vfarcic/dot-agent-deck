...
**A credential the terminal WRAPPED is matched too, and the arithmetic is why this is not a footnote.** An Anthropic key is 108 characters; the harness renders at 120 columns; an `ANTHROPIC_API_KEY=` line therefore starts the value at column 19 and breaks it **102 / 6**, which splits the key's 20-character response id **14 / 6**. Byte-exact matching found neither piece, so the entire credential was reconstructable out of a panic grid while both registered patterns matched nothing — and that grid goes to nextest's run log and the raw JUnit report, where GitHub's masking of the exact secret does not reassemble line-wrapped fragments. It is not only the grid: the same value is split the same way in `full-stream.cast`, one layer lower, where a row change is the deck's own cursor-position escape rather than a newline.
...
**Neither credential outlives its step.** A `Purge the run's credentials from the runner` step sits immediately after the test command, `if: always()`, before every post-processing step, before `actions/upload-artifact`, and before the `Swatinem/rust-cache` post-action — all of which run as the same user. Position is the whole point; a cleanup after those is worth much less. It removes `~/.codex`, contains every process the run started, and removes the harness temp roots, whose isolated HOMEs each hold a copy of `auth.json` and of the Anthropic key's response id and are only self-cleaning when a test *passes*. It deliberately does not touch `DOT_AGENT_DECK_TEST_MAX_LIFETIME_SECS` — CLAUDE.md rule 14 documents why that number is load-bearing for the temp-dir reaper.

**Containment is by inheritance, and the deletion waits on a verification.** Killing by name does not work here, and the counterexample is a test in this suite rather than a hypothetical: `shell_activity_006_real_claude_bash_call_crossing_the_cap_keeps_the_badge_working` *passes by proving* that a real Claude Bash-tool shell and its `ping -c 200` child are still alive after Claude's 120-second tool cap. That shell is deliberately in its own POSIX session, so the daemon's `killpg` never reaches it; its `/bin/bash … ping …` and `ping` command lines match none of the purge patterns; and it inherited the agent's environment, Anthropic key included. It can outlive the tier by most of 200 seconds, overlapping the summary, the stripper, the upload and the cache post-action.

So the run step exports a **non-secret run marker** (`DAD_E2E_RUN_MARKER`, a run id and an attempt number) that every descendant inherits. Three mechanics carry it, and **all three are needed** — the first version of this shipped only the first and Greptile caught it on PR #805: `INHERIT_PASS` crosses `TuiDeck::launch`'s `env_clear`; `push_containment_env` crosses the *separate* `env_clear` in `spawn_daemon_serve_with_env`, which credential-bearing live tests use (`scheduler_pi_001` pushes `ANTHROPIC_API_KEY` through its `extra_env`, so a scheduler-spawned `pi` inherits it) and whose three shell-out helpers replay the same vector; and `spawn_agent` overlays rather than clears, so the in-process-daemon spawns get it for free. A guard pins the **count** of environment-rebuilding spawn paths at four, so a fifth fails a test rather than silently escaping containment. The purge step enumerates `/proc/*/environ` for it, sends TERM then KILL, and then **verifies nothing marked remains**. That verification gates the deletion: handing `rm -rf` a root a live process is writing under is the exact hazard rule 14's dead-PID floor exists to prevent, so if anything survives, the roots are left alone and the job fails. It holds the marker under a different variable name (`PURGE_RUN_MARKER`) so the needle is absent from its own environment, and skips its own ancestors so the runner's worker can never be a target. Only PIDs and `comm` are ever printed — never a command line, never an environment.

Three outcomes are hard failures now, all of them "a credential is still on this runner": a credential *file* present after removal, a *marked process* alive after SIGKILL, and a harness temp *root* present after removal. The last of those used to be a `::warning::`. Two guard tests keep the halves in step — the harness must forward the marker across `env_clear`, and the workflow must set and grep the same name — because a rename on either side would silently turn the containment check into "found nothing", which reports success and then deletes.
