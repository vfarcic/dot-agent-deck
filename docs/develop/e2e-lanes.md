...
The job also uploads `target/nextest/default/junit.xml` as `nextest-e2e-live-attempt-<n>` (issue #564), which is what survives a job-level re-run overwriting the conclusion. It carries the per-test wall clock and the ran-versus-skipped distinction that a green summary hides.
