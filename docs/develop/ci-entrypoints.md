...
Status, stated plainly: **it has run three times; the macOS block is green and the two Linux blocks have never executed a command.** A Semaphore Cloud project is connected to `vfarcic/dot-agent-deck`, and every run on 2026-08-04 produced the same pattern: `build-macos` passed on `a2-standard-4` / `macos-xcode16` in 5m45s with a cold nix bootstrap and no cache, while `build` and `security` failed with `start_time: 0` — no agent ever assigned — because the requested `(machine, os_image)` selection was refused. All three refusals used `os_image: ubuntu2404`; the Linux blocks now target `f1-standard-4` / `ubuntu2204`, with the image as the variable under test. Nothing in the Linux bootstrap or the real `cache` CLI has been observed yet.
...
- **A refused agent selection produces `start_time: 0` and no logs at all, and the list you can see is not the entitlement.** A pipeline that is valid YAML, naming a documented and correctly-spelled machine type, can still fail with **every job at `start_time: 0` and no logs whatsoever** — there is no job output to read, because no agent ever existed to produce any. `sem get job <id>` reporting `start_time: 0` means *"no agent was ever assigned"*, and that is the signature of an agent selection this organization cannot provide. The machine-type list shown in the UI and the docs is not the entitlement either: `e1-standard-4` appears in the list and was refused anyway, with the same error as a type that does not appear at all. **The only reliable probe is a job that gets an agent.** What has actually been observed here, so nobody re-derives it:

  | machine | `os_image` | verdict |
  |---|---|---|
  | `f1-standard-4` | `ubuntu2404` | refused — `start_time: 0`, *"Selected machine type is not available in this organization"* (runs 1–2) |
  | `e1-standard-4` | `ubuntu2404` | refused — identical error, despite being on the visible machine list (run 3) |
  | `f1-standard-4` | `ubuntu2204` | current choice, not yet observed |
  | `a2-standard-4` | `macos-xcode16` | **works** — `build-macos` passes on it |

  Budget one run per selection you want to rule in or out, and note that a refusal costs a full push and produces no logs at all.

- **The transferable lesson, and it is a debugging lesson rather than a Semaphore one: when the thing being refused is a *pair*, vary each element before theorising about entitlement.** Semaphore selects an agent as `(machine.type, os_image)`, but the refusal message names only the machine — *"Selected machine type is not available in this organization"* — so it reads as a verdict on one half of a two-part selection. Three runs were spent changing the machine type while `os_image: ubuntu2404` was never touched, which made `ubuntu2404` the one thing common to every refusal and left it the least-examined variable in the file. Two hypotheses were built on the machine reading before anyone checked the image, and both were wrong; the first collapsed on a five-second pricing check (the macOS machine that *works* on this org costs 6× the Linux machine being refused, so no plan tier explains it), and the second was built on the first.

  Generalised, because the shape recurs on every provider: an error message names the field the API happens to report on, **not** the field that is actually at fault. Before you theorise about accounts, plans, quotas or entitlements — the explanations that are expensive to test and comfortable to believe, because they put the cause outside your file — enumerate every element of the failing selection and confirm you have moved each one at least once. A constant across all failures is a suspect, and one that never appears in the error message is the easiest suspect in the world to miss.

One provable bug was found by re-checking the file's assumptions against current documentation instead of waiting for a log — and it was **not** the cause of any of those three failed runs, since the Linux jobs never reached any of this code: `SEMAPHORE_GIT_DEPTH` was set to `"0"` to mean "full clone", which is not a thing. Semaphore documents the variable as a shallow-clone depth defaulting to 50, the toolbox passes it straight to `git clone --depth`, and git rejects it outright with `fatal: depth 0 is not a positive number`. On a **pull-request** run that kills checkout in the prologue — the depth clone is unguarded and the following `cd "$SEMAPHORE_GIT_DIR" || exit` runs with the directory never created — while on a **branch push** it is swallowed by a "branch not found" fallback that does a full clone anyway, so it survives, noisily. The variable is gone; the default (50) is strictly more history than the GHA control has, since `ci.yml` uses `actions/checkout@v7` with no `fetch-depth`, i.e. depth 1. It remains a real latent bug that was worth fixing before a PR-triggered run existed.
