...
4. **Do not make Semaphore a required status check.** Renovate automerges cargo patch and devbox bumps on green CI (`renovate.json`); an unproven pipeline does not go in that path. This is also PRD #376 scope.
