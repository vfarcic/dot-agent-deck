...
- **The author merges after approval, or arms auto-merge.** Nothing in the ruleset constrains who presses the button (`require_last_push_approval` is `false`), and the approving maintainer is under no obligation to. Reviewer-merges is a Prow/Kubernetes convention in which a *bot* merges on `/lgtm`; the GitHub-native equivalent is auto-merge, queued by the author. **Auto-merge was enabled on 2026-08-11** (`allow_auto_merge: true`), so an author may arm a pull request and let GitHub land it when the ruleset is satisfied. The objection previously recorded here — that arming it early lets the author "never return to the pull request" — does not survive the two rules above: auto-merge waits for the required approval, so *something* still reads and judges, and `required_review_thread_resolution: true` means an armed pull request cannot merge while Greptile's threads sit unresolved. Someone must still clear them by hand. **That sentence used to say "a human still reads and judges", and that is not what happens on the ordinary path** — with `PR_REVIEW_LIVE` set, the daily `pr-review-batch.yml` sweep casts the approval unattended, and every pull request merged on 2026-09-17 was approved by `dot-agent-deck-reviewer` rather than by a maintainer. The correction is narrow and deliberately does not widen: the reviewer escalates what it cannot verify instead of approving it, so a `REQUEST_CHANGES` naming a manual obligation — rule 12's cross-version test is the recurring one — still lands on a person. What auto-merge waits for is a real approval; what it no longer implies is that a human read the diff. **That safety rests entirely on the ruleset being in force**, though, because `allow_auto_merge` is a repository setting the ruleset knows nothing about: lift the gate with an armed pull request outstanding and it lands unreviewed on the spot. [Emergency override](#emergency-override) therefore disarms before it deletes. **What auto-merge does NOT wait for needs its own sentence, because the omission has already cost a merge.** It waits for what the ruleset requires, which is not every check on the pull request page: the required contexts are `build`, `build-macos`, `build-windows`, `security` and `e2e-deterministic` (five, verified 2026-09-22 against `gh api repos/vfarcic/dot-agent-deck/rules/branches/main`). Greptile is outside that list deliberately, because waiving a finding is a legitimate approval ([step 6](#rollout)); `renovate/stability-days` is outside it too, and nothing on this page or in the ruleset records that as a decision either way. So a pull request whose only outstanding check is an unrequired one — `renovate/stability-days`, `Greptile Review`, `notify-main-red` — is **already mergeable**, and `gh pr merge --auto` merges it on the spot rather than arming: `isImmediatelyMergeable` treats `UNSTABLE`, the state a pending or failing unrequired check produces, as immediately mergeable, so the flag is dropped client-side (`pkg/cmd/pr/merge/merge.go:593` and `:826`, read at gh v2.100.0). Exit 0, no output, and `gh pr view <n> --json autoMergeRequest` reading `null` *after* the call is the tell that this is what happened. #1208 landed a lockfile update inside Renovate's stability window exactly that way on 2026-09-21 (issue #1222). Auto-merge is therefore a way of not bypassing the *review* rule and not a way of waiting for a check: where a flow genuinely needs an unrequired one, poll `gh pr checks <n>` for that context and merge when it passes.

**Never merge your own unapproved pull request.** For the owner it will succeed — the admin bypass makes it silent rather than blocked — and that silence is precisely the decay this arrangement exists to prevent. An automated flow whose last step is a merge (`/pr-create`, `/prd-full`) may arm auto-merge and hand off, since that cannot land anything unapproved — a narrower guarantee than it reads, because it equally cannot *hold* a pull request for a check the ruleset does not require, and merges immediately when that is all that is outstanding (the bullet above) — but it must never merge directly.
