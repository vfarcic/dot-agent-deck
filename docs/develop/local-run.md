...
- **Agent hook configs.** At startup the deck auto-installs its hooks into agents' user-level configs, as your everyday deck does. The binary it pins there comes from `durable_binary_path()` (PRD #381), which persists only an **installed** deck: `~/.local/bin/dot-agent-deck` or a `dot-agent-deck` in a directory on `PATH`. A `target/` build is a cargo artifact and a copy parked anywhere else — `/var/tmp/dad-branch/bin/dot-agent-deck`, say, to pin a branch build for a sandbox — is not installed, so neither is written; the resolver falls through to whichever install it can find, and refuses if there is none. Until issue #1140 only the `target/` half held, and a copy parked elsewhere was pinned into every agent's global config *beside* the installed release's entries. Note what is still not isolated: the install these tasks pin is your real one, in your real config, so the sandbox deck's hooks and your everyday deck's hooks are the same entries.
