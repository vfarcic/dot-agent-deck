...
A registered set also fails the job when the certificate has expired or expires within 24 hours, when the `.p12` does not hold exactly one valid `Developer ID Application` identity, when `codesign --verify --deep --strict` rejects the signed app, when the upload of either notarization submission fails, when Apple returns `Invalid` or `Rejected` for one — in which case the job prints `xcrun notarytool log` for the submission — and when neither verdict arrives within 75 minutes of a submission. None of these falls back to an unsigned build: once credentials are registered, an unsigned `.dmg` would mean something went wrong, and shipping it silently is the failure this is designed to avoid.

**Notarization is polled, not waited on** (issue #1325). Both submissions go through one script the job writes to `$RUNNER_TEMP/notarize.sh`: it submits without `--wait`, takes the submission id from notarytool's JSON, and polls `xcrun notarytool info <id>` every 30 seconds. A poll that fails, or that answers without a status it recognises, is logged and retried — it is not a verdict. `notarytool submit --wait` did the opposite: on v0.42.0's run 36209188544 one status request timed out (`NSURLErrorDomain Code=-1001`) about 15 minutes in, notarytool exited, the job recorded status `unknown` and failed, and Apple accepted that submission a day later. The 75-minute deadline per submission keeps both inside the job's `timeout-minutes: 180`; a poll that hangs instead of failing is bounded only by that job timeout. When the deadline passes, the error names the submission id and the `notarytool info` command to check Apple's verdict with. Re-running the failed jobs submits afresh rather than reusing that submission, because re-signing changes the bytes Apple notarizes.
...
Developer ID Application certificates are usually described as valid for five years, but **do not assume it**: the one registered as `APPLE_CERTIFICATE` reads `notAfter=Feb  1 22:12:15 2027 GMT` (checked 2026-09-26), about four months after it was created, for a reason not yet known (issue #1326). The expiry is on the certificate itself, so read it from the `.p12` (step 4 above), from Keychain Access, or from the certificate list in the developer portal. The secret cannot be read back out of GitHub.
...
**Where the warning surfaces.** A warning annotation on a green run notifies nobody. So `/tag-release` runs `.claude/skills/tag-release/cert-expiry.sh` in its first step, which reads the last release run's `desktop-sign` log and annotations and reports the certificate's `notAfter` date, plus any certificate warning or error, to whoever is cutting the next release. That covers a release cut through the skill; one dispatched another way still has only the run's own annotation.
