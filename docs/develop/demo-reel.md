...
5. **Surface to the human pre-merge.** The orchestrator presents the link at the merge gate: `Demo reel for PRD #NNN: <YouTube link>. Watch before merging.` The video is **private**, so only the channel owner can open that link until somebody deliberately flips it — which is the review step, not a formality.
