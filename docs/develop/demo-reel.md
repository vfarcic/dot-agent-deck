...
**That recording run holds your real agent credentials, and this pipeline publishes what it captured.** Eligibility requires a real agent spinning up (rule 4), which is exactly the `e2e-live` boundary — so the cast a clip is built from was written by a process authenticated as you, and the finished reel goes unlisted to YouTube with the link in the public release notes. The harness redacts registered credential values inside `full-stream.cast`, including values the terminal wrapped across rows; [`e2e-lanes.md`](e2e-lanes.md) has the mechanism and its stated limits. Treat that redaction as load-bearing rather than as housekeeping, and watch the reel before publishing it (the checks below).
