...
So the practical consequence is worth stating precisely. CI does run `status/shell-activity/001`–`004` — they are fast-tier tests in `tests/shell_activity.rs`, so every `cargo nextest run --workspace` job covers the process-table primitive, the session-id discriminator and the pane-level rising and falling edge. What no CI job covers is whether the signal still fires for a **real agent**: **a green CI run says nothing about that.** That rot is caught by running these two canaries locally, or not at all. If you are touching the scan, the monitor, or the pane primitive, run `cargo test-e2e-live shell_activity_005` and `…_007` on a machine with real Claude credentials, under `DOT_AGENT_DECK_REQUIRE_REAL_E2E=1` so a missing credential fails instead of silently passing as nextest would otherwise count it.
