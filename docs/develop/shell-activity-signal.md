...
Lane 2 *does* run in CI, per-merge on `main`, but that changes less than it sounds. Both open with `skip_unless!(common::check_claude_available())`, which wants `claude` on PATH **and** a usable `~/.claude/.credentials.json`; it never consults `ANTHROPIC_API_KEY`, which is the only credential that lane carries (#785 decision 1). So they still self-skip there — and `skip_unless!` returns normally, so nextest counts a skip as a **pass**. See [`e2e-lanes.md`](e2e-lanes.md).
