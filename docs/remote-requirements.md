...
The `$TMPDIR/dot-agent-deck-{uid}/` fallback is intended for hosts that do not set `XDG_RUNTIME_DIR` — an ordinary ssh session or a container. The endpoints live one level down, inside a directory the deck creates at mode `0700`; if it cannot make that directory owner-only — because another uid already owns it, say — it refuses to start and names the directory, the two uids and the override, rather than reporting a bare `Operation not permitted`.
...
**The directory name is itself predictable, and on a shared host that still matters.** The gain over the old spelling is that the deck never removes the directory, whereas every daemon start removed its own stale socket before binding a fresh one: the old name was free before this host's first launch, after a `/tmp` cleaner removed it, and for an instant at every start, while the directory can only be taken in the first two of those windows. Inside them another local user can create `/tmp/dot-agent-deck-{your-uid}` first, and the deck will then refuse to start until that entry is removed by them or by root. It refuses rather than misbehaving — it will not put your sockets inside a directory someone else owns — and the message names the directory and the way out. If a shared host is your situation, set the two overrides below rather than relying on winning that race.
