...
See [Daemon socket security](#daemon-socket-security) under Recommended for what the fallback does and does not protect, and for the override guidance.
...
Beyond the directory's existence, a shared host gives another unprivileged local user no read access to your endpoints or their traffic. Root is outside all of this, as it is outside every filesystem permission. v1 assumes a single-user host (the PRD's stated scope: "no multi-user access controls beyond Linux user separation"). If you would rather the endpoints sat somewhere else entirely, set `$DOT_AGENT_DECK_SOCKET` and `$DOT_AGENT_DECK_ATTACH_SOCKET` to paths inside the user's home, for example `~/.local/state/dot-agent-deck/daemon.sock` and `~/.local/state/dot-agent-deck/attach.sock`, and ensure the parent directory is mode `0700`. The socket file itself is created at mode `0600` directly by the daemon — `bind(2)` runs with a narrowed umask of `0o177` so the socket inode is never world-readable for any window, and a defense-in-depth `chmod 0600` follows. The user's process umask does not affect the socket mode, but it still affects other files the daemon and its agents create, so a restrictive umask (e.g. `umask 077`) in the unit file or login profile remains a sound default on a shared host.
