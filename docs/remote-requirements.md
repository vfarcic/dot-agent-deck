...
3. `$TMPDIR/dot-agent-deck-{uid}/hook.sock` as a last-resort fallback — the system temp dir (`/tmp` unless `$TMPDIR` says otherwise), then a per-user directory the deck creates at mode `0700`, where `{uid}` is the user's POSIX uid. Two users on the same host therefore get disjoint endpoints in directories neither can write to. The attach endpoint resolves the same way, as `$DOT_AGENT_DECK_ATTACH_SOCKET`, then `$XDG_RUNTIME_DIR/dot-agent-deck-attach.sock`, then `attach.sock` in that same per-user directory.
