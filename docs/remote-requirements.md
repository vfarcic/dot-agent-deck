...
**The directory name is itself predictable, and on a shared host that still matters.** The gain over the old spelling is that a directory persists across daemon restarts while a socket does not: the old path was open to a squatter every time the daemon was down, whereas the directory can only be taken before this host's first successful launch, or after a `/tmp` cleaner removes it. Inside that window another local user can create `/tmp/dot-agent-deck-{your-uid}` first, and the deck will then refuse to start until that entry is removed by them or by root. It refuses rather than misbehaving — it will not put your sockets inside a directory someone else owns — and the message names the directory and the way out. If a shared host is your situation, set the two overrides below rather than relying on winning that race.
