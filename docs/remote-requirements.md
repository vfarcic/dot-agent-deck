...
3. `$TMPDIR/dot-agent-deck-{uid}/hook.sock` as a last-resort fallback — the system temp dir (`/tmp` unless `$TMPDIR` says otherwise, and `/tmp` again when it is set but empty), then a per-user directory the deck creates at mode `0700`, where `{uid}` is the user's POSIX uid. Two users on the same host therefore get disjoint endpoints in directories neither can write to. The attach endpoint resolves the same way, as `$DOT_AGENT_DECK_ATTACH_SOCKET`, then `$XDG_RUNTIME_DIR/dot-agent-deck-attach.sock`, then `attach.sock` in that same per-user directory.

See [Daemon socket security](#daemon-socket-security) under Recommended for what the fallback does and does not protect, and for the override guidance. Step 2 works on any host that sets `XDG_RUNTIME_DIR`, but macOS does not set it for you, so a Mac lands on step 3 unless you set one of the two variables yourself — see [macOS as a remote host](#macos-as-a-remote-host).
...
The `$TMPDIR/dot-agent-deck-{uid}/` fallback is intended for hosts that do not set `XDG_RUNTIME_DIR` — an ordinary ssh session or a container. The endpoints live one level down, inside a directory the deck creates at mode `0700`. If another uid already owns an entry at that name, the deck does not use it and does not refuse to start either: it binds in a relocated sibling instead (below). If it cannot make the directory owner-only for any other reason, it refuses to start and names the directory and the override, rather than reporting a bare `Operation not permitted`.

What that fixes is availability, not confidentiality. The flat `/tmp/dot-agent-deck-{uid}.sock` spelling sat directly in a world-writable sticky directory, so another local user could create an entry at that exact path; the daemon's `bind(2)` then failed with `EADDRINUSE` and the deck could not remove the entry either, because a sticky directory only lets the entry's owner unlink it. The deck's own endpoint was unusable until the squatter or root cleared it. Inside a directory at mode `0700` that an unprivileged uid can neither write to nor traverse, there is no entry for them to create. Confidentiality was never the gap: the socket inode has always been mode `0600`, and under `$XDG_RUNTIME_DIR` (mode `0700`, the user's own) no foreign uid could create the entry in the first place — so a desktop session, which is where that variable is normally set, was never reachable this way. The exposure was bounded by the directory, not by the fallback existing.

**The directory name is itself predictable, so the deck does not depend on winning it.** Another local user can create `/tmp/dot-agent-deck-{your-uid}` before your first launch, or after a `/tmp` cleaner removes it. The deck will never put your sockets inside a directory someone else owns. Instead it creates a sibling directory, `/tmp/dot-agent-deck-{your-uid}.` followed by 16 random hex digits, at mode `0700` with a single `mkdir` that fails if anything is already there, and binds both endpoints inside it. Nobody can create that name in advance, because nobody can predict it, and once it exists it is yours. Hooks, `daemon status`, `daemon stop` and the TUI all find it again with no configuration: they list the temp directory for siblings with that prefix that belong to you and are owner-only, and ignore every other entry, including any another user made. The daemon logs a warning naming both directories. The choice is sticky: once relocated, later starts keep using the relocated directory for as long as the usual one does not exist, so a squatter removing their entry cannot make two starts disagree about where the deck is. To return to the usual location, stop the deck and remove every `/tmp/dot-agent-deck-{your-uid}.*` directory. An older deck binary does not know about the relocated directory, and finds a relocated daemon through the compatibility alias below. Failing that, a binary from a release that already uses the per-user directory cannot start a second daemon while the directory is still taken, because its own daemon cannot bind inside it either; it fails with the message naming the directory instead. If you would rather choose the location yourself, set the two overrides below.

**The old flat spellings are still bound, as a temporary compatibility alias.** So that a deck binary from before this change — a stale install earlier on `PATH`, or an environment pinned to an older release — finds a newer daemon and shows its version-mismatch prompt instead of silently starting a second daemon, a daemon on this fallback also binds `/tmp/dot-agent-deck-{uid}.sock` and `/tmp/dot-agent-deck-attach-{uid}.sock` beside its own endpoints, and removes them when it stops. The alias is best-effort: if an entry the deck does not trust sits at either path — one another user created, say — or another daemon is already listening there, the daemon logs a warning and carries on at the per-user directory. Someone squatting the old spelling can therefore stop an *older* client from finding your deck, but cannot stop the deck from starting. The alias will be removed in a later release.

Mode `0600` on the socket keeps *other* unprivileged users out. It has never kept out root, or another process running as you — an agent you launched can reach the daemon's endpoint, which is by design, since that is how hooks report. Connections are also checked in the other direction: a client verifies that the process listening at the endpoint runs as you, and refuses it otherwise.

Beyond the directory's existence, a shared host gives another unprivileged local user no read access to your endpoints or their traffic. Root is outside all of this, as it is outside every filesystem permission. v1 assumes a single-user host: there are no multi-user access controls beyond the Linux user separation the OS already gives you. If you would rather the endpoints sat somewhere else entirely, set `$DOT_AGENT_DECK_SOCKET` and `$DOT_AGENT_DECK_ATTACH_SOCKET` to paths inside the user's home, for example `~/.local/state/dot-agent-deck/daemon.sock` and `~/.local/state/dot-agent-deck/attach.sock`, and ensure the parent directory is mode `0700`. The socket file itself is always created owner-only (`0600`), and is never briefly readable by anyone else while it is being created. Your own umask cannot weaken that — but it does still affect other files the daemon and its agents write, so a restrictive umask (e.g. `umask 077`) in the unit file or login profile remains a sound default on a shared host.
