...
The deck therefore holds a `clear = true` task for a short **readiness buffer** after the replacement signals its session start (and after the fallback wait expires, for agents that never signal at all). The default is 1000 ms: the spawn-time path's 500 ms, which was tuned for a warm pane, doubled because a respawn is a cold start. Nothing about this is configured per role; the only effect you should notice is that a `clear = true` delegation takes about a second longer to appear in the worker's pane than a `clear = false` one. Two cases wait longer than that, and both are described below — a worker the deck watched take over its terminal waits five seconds, and one that announces nothing at all waits eight.
...
**Codex is different, and faster.** A Codex worker runs under the deck's own wrapper, which hosts its terminal — so rather than waiting for a signal, the deck *watches* the replacement's interface come up and delivers as soon as it has. A `clear = true` delegation to Codex used to take about 31 seconds to appear in the worker's pane (Codex announces its session when its first turn starts, i.e. only *after* a task arrives, so the deck had nothing to wait for and paid the full 30-second fallback every time), and now takes a few seconds. There are two things the deck can see, and they are not worth the same: watching the worker take the terminal out of line-editing mode — what a full-screen interface does as it starts — is proof that the worker, rather than a launcher, now owns the terminal, so that is what the deck releases on. It is not proof that the worker will accept the task yet, which is a distinction worth stating plainly because the deck got it wrong first: a full-screen interface switches the terminal over at the *start* of its own startup, about a tenth of a second in, and goes on ignoring keystrokes until it has finished drawing itself. Delivering on that instant loses the task as surely as delivering too early ever did. So the deck still holds the task over that signal, for longer than the buffer below — five seconds, measured against how long a real Codex takes to stop dropping input, not guessed. Merely watching its output go quiet is not proof of even that much: a launcher stalled part-way through its own startup goes quiet exactly like an interface waiting at its prompt, and if you launch Codex through something like `devbox run`, that is exactly what happens first — the launcher prints a line, then spends two to four seconds working out its environment before Codex is started at all. So the deck does not deliver on silence straight away. It keeps waiting for the real interface to turn up behind the quiet launcher — for as long as its readiness window lasts, which is the same half-minute it used to spend waiting for a signal that never came — delivers the moment the interface appears, and falls back to delivering on the silence, with the buffer still applied, only if it never does. In practice a healthy Codex worker gets its task a few seconds after its interface appears, against the half-minute it used to take, and never sees the launcher wait at all. What the willingness to keep waiting buys is that a slow launcher is no longer handed a task it will swallow, and the case that needs it most is a cold one: on a first run in a fresh project, where `devbox` installs packages before it works anything else out, the interface took **sixteen seconds** to appear. A window that gave up sooner wrote the task into the launcher, where it sat in the terminal's input queue until Codex started and was then read as ordinary typing rather than as a submission — the task visible in the composer, unsent, with nothing to say so. The worst case is a wrapped worker that never presents a full-screen interface at all, and it costs the half-minute this section opened on: no worse than before any of this, and the reason a longer wait is the safe direction to be wrong in. If you saw a Codex worker sit apparently idle for half a minute after every delegation — long enough to make you re-run the delegation, which then superseded the worker that was about to come good — that was this.

**OpenCode is different in the other direction.** It announces nothing at all before a task arrives, so there is genuinely nothing to wait for; the deck no longer spends 30 seconds finding that out and holds the task for **eight seconds** instead. That is still a large speedup — a `clear = true` delegation to OpenCode used to take about thirty seconds to appear, and now takes under ten — but be clear about the trade, because it is the sharpest one on this page: with the wait gone, that interval is the *only* thing standing between the task and a still-booting OpenCode. The eight seconds is measured rather than picked. Replaying the deck's own write against a real OpenCode on a terminal, the task is swallowed outright — not left visible in the input box, just gone — until the moment OpenCode paints its `Ask anything...` prompt, and delivered every time after it. That moment is two and a half seconds in on an idle machine and four and a half on a busy one, so eight is a little under twice what a busy machine was measured to need. On a machine loaded far past that — sixteen cores with sixty-four busy processes on them — it took twelve seconds, and eight would not have been enough. So treat it as a mitigation rather than a guarantee: if OpenCode tasks go missing on your machine, raise `DOT_AGENT_DECK_DELEGATE_READINESS_BUFFER_MS`, and please report it. Unlike Codex, there is nothing here for the deck to watch instead — OpenCode emits no signal before its first task by construction — so an interval is the ceiling until that changes.
...
Values above `30000` are capped, and `0` disables the wait entirely (the pre-fix behaviour — useful only for reproducing the problem). It applies wherever the deck has *not* watched a wrapped worker take the terminal out of line-editing mode — which is every agent except a wrapped one, plus every case where the wrapper saw only the weaker signal — and it applies to a scheduled task's first prompt as well as to a delegation. **Setting it explicitly also puts a floor under the fast path:** if you have raised this because prompts were going missing on your machine, Codex keeps waiting that long too rather than skipping ahead. That is deliberate — you know something about your machine that watching one worker start does not refute. Please also report it: a machine that needs more than a second is exactly the evidence [#243](https://github.com/vfarcic/dot-agent-deck/issues/243) needs to size this per agent.
