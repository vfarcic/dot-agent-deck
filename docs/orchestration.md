...
**Codex is different, and faster.** A Codex worker runs under the deck's own wrapper, which hosts its terminal — so rather than waiting for a signal, the deck *watches* the replacement's interface come up and delivers as soon as it has. A `clear = true` delegation to Codex used to take about 31 seconds to appear in the worker's pane (Codex announces its session when its first turn starts, i.e. only *after* a task arrives, so the deck had nothing to wait for and paid the full 30-second fallback every time), and now takes about a second. There are two things the deck can see, and they are not worth the same: watching the worker take the terminal out of line-editing mode — what a full-screen interface does as it starts — is proof that it consumes keystrokes, so the buffer below is skipped. Merely watching its output go quiet is not: a launcher stalled part-way through its own startup goes quiet exactly like an interface waiting at its prompt, so the deck delivers on that too but keeps the buffer over it. If you saw a Codex worker sit apparently idle for half a minute after every delegation — long enough to make you re-run the delegation, which then superseded the worker that was about to come good — that was this.
