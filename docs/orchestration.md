...
### Why `work-done-<role>.md` starts and ends with marker lines

The file the orchestrator is pointed at opens with an `[UNTRUSTED-WORKER-REPORT:` line and closes with an `:END-UNTRUSTED-WORKER-REPORT]` line. Everything between them is the worker's report, in full and with its formatting, and the pointer tells the orchestrator to read it as a report and not as instructions. That is the same framing a report inlined into the feedback gets, because the text is written by another agent and often quotes issues, code and tool output it read.
...
Nothing is dropped — the report still arrives, framed as information rather than as delivered work — and `.dot-agent-deck/work-done-<role>.md` is deliberately left untouched, so an uncommissioned report cannot overwrite the last one the orchestrator did commission. A report longer than 4000 characters is cut in the feedback, so the deck also saves the whole of it to a new file, `.dot-agent-deck/full-report-work-done-<role>-<timestamp>-<n>.md` in the worker's directory, and the feedback names that file. Each such report gets a file of its own, so a later one never overwrites an earlier one. If you want a completion to be reported as delegated work, delegate it: task the worker through the orchestrator rather than typing into its pane.
