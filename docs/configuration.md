...
For the full reference and more examples, see [Workspace Modes](workspace-modes.md) and [Orchestration](orchestration.md). `default` and `extends` only matter to a project that defines **several** orchestrations; both are explained under [More than one orchestration](orchestration.md#more-than-one-orchestration).
