...
/**
 * Which stored selection would put THIS deck in force (PRD #1105 M6) — or
 * `undefined` when this document cannot name it unambiguously.
 *
 * # What this is for
 *
 * The overview merges every observed deck's agents onto one screen, while a
 * tile's terminal is always the *selected* deck's. Opening a non-selected
 * deck's agent therefore has to move the selection first, and moving it means
 * writing a token — so something has to map a deck the fleet describes back to
 * a row this document holds.
 *
 * # The mapping is by LABEL, and that is exact where it answers at all
 *
 * A deck's identity on the wire is `connection.deckId`, an opaque
 * `deck-<16 hex>` minted from `EndpointIdentity::wire_id()`, and nothing on
 * this side can invert a hash. What both sides *do* derive the same way is the
 * label: `Endpoint::describe()` renders a remote deck as `user@host[:port]`,
 * and {@link describeEndpoint} derives it from a row byte for byte — which is
 * why the Deck selector's own menu is built out of it.
 *
 * The connectable set is `[the local deck] ∪ [every row with a socket path]`,
 * so a deck in the fleet is either the local one or exactly one of those rows.
 * It follows that a label matching **exactly one** row identifies that deck
 * with certainty: the target's own row is necessarily among the matches, and a
 * single match leaves nothing else it could be.
 *
 * # It refuses rather than guesses, and the refusal is the safe direction
 *
 * The label is deliberately not unique — two rows differing only in socket
 * path, identity file or jump host describe identically — and there the
 * matches are two and this returns `undefined`. Guessing would select the wrong
 * machine, and agent ids are per-daemon monotonic integers, so the pane would
 * then attach *a different agent of the same id* on a deck the user did not
 * ask for. A pane whose terminal does not attach is a visible nuisance; a pane
 * showing somebody else's agent under the right name is not visible at all.
 *
 * Note what is NOT a failure case: two rows that collide this way are already
 * two identical entries in the Deck selector, so this is exactly as precise as
 * the control it automates.
 */
export function selectionForDeck(
  section: EndpointSettingsDto | undefined,
  deck: { deckKind?: "local" | "remote"; socketPath?: string },
): DeckSelection | undefined {
  // Defaulted to local for the same reason `deckKind` itself is: a fixture or
  // an older reply that says nothing is a deck on this machine, which needs no
  // row and is always selectable.
  if (deck.deckKind !== "remote") return LOCAL_DECK_SELECTION;
  if (!deck.socketPath) return undefined;
  const matches = (section?.remote ?? []).filter((row) => describeEndpoint(row) === deck.socketPath);
  return matches.length === 1 ? { kind: "one", id: matches[0].id } : undefined;
}
