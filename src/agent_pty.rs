...
/// Issue #550: absolute path of the **main worktree** of the repository the
/// pane's working directory belongs to — the checkout that outlives every
/// linked worktree cut from it. Injected by [`spawn`] into every child it
/// starts, resolved by [`crate::worktree_owner::main_worktree_of`] from the
/// pane's own cwd.
///
/// A delegated agent working in a linked worktree has nowhere durable to leave
/// a report: a worktree is temporary — closing a dispatched unit's tab removes
/// its copy, and `dot-agent-deck worktree reclaim` removes one whose branch has
/// merged (`worktree_reclaim`'s three-part gate). Today the
/// path has to be hand-written into the task by whoever delegates, and
/// forgetting produces a *stall* rather than an error — the agent has nothing
/// to resolve and correctly declines to guess. This makes the deck answer it
/// once, at spawn, so a role prompt can say "write findings to
/// `$DOT_AGENT_DECK_MAIN_WORKTREE/…`" and no task has to carry the path.
///
/// **Absent rather than guessed**, which is the property worth relying on: the
/// variable is set only where the main worktree is genuinely resolvable, and
/// [`spawn`] scrubs any inherited one first, so a consumer can distinguish
/// "the deck could not work this out" (unset) from "the deck says it is here"
/// (set). Unset covers a cwd outside any repository, a bare repository, and
/// the layouts `main_worktree_of` documents as unrecoverable. Purely
/// additive — nothing in the deck reads it, and no consumer is obliged to.
///
/// Reaches a wrapped agent too, without a second injection site. A
/// Wrapper-strategy agent (Codex) runs under `dot-agent-deck wrap`, which opens
/// an inner PTY of its own — but the wrapper is itself the child [`spawn`]
/// starts, and it neither clears nor scrubs the environment it passes on, so
/// the inner agent inherits this exactly as a natively-spawned one is handed
/// it.
///
/// Named for git's own term rather than `DOT_AGENT_DECK_ROOT`, which reads as
/// the deck's own install or storage root: every other name in this namespace
/// (`STATE_DIR`, `LOCK_DIR`, `REMOTES_DIR`) is something the deck owns, and
/// `<TOOL>_ROOT` conventionally means where the tool lives. `main worktree` is
/// what git calls this and stays literally true in an ordinary checkout, which
/// is also a main worktree.
pub const DOT_AGENT_DECK_MAIN_WORKTREE: &str = "DOT_AGENT_DECK_MAIN_WORKTREE";
...
pub fn spawn(opts: SpawnOptions<'_>) -> Result<AgentPty, AgentPtyError> {
    ...
    // Issue #550: the scrub is what makes the injection below fail CLOSED.
    // Every other variable here is re-supplied unconditionally, so removing it
    // is only hygiene; this one is re-supplied *only when it resolves*, and an
    // inherited value is exactly the confidently-wrong path the feature exists
    // to avoid handing an agent. A daemon launched from inside a deck pane in
    // repository A that then spawns a pane whose cwd is in repository B — or
    // in no repository at all — would otherwise tell B's agent to write its
    // durable report into A.
    cmd.env_remove(DOT_AGENT_DECK_MAIN_WORKTREE);
    ...
    // Issue #550: hand the child the main worktree of whatever repository its
    // own cwd belongs to, so an agent working in a linked worktree knows where
    // the durable checkout is without the task having to spell it out.
    //
    // Resolved from `opts.cwd` — the pane's cwd, not the daemon's — because
    // that is the repository the agent actually works in; panes under one
    // daemon routinely sit in different worktrees, and a value computed once
    // per process would be right for at most one of them. Falls back to the
    // daemon's own cwd only when the pane did not pin one, which is the
    // directory the child inherits anyway.
    //
    // Applied AFTER `opts.env` so the `any(...)` guard sees the caller's keys,
    // and gap-filling like `DOT_AGENT_DECK_SOCKET` rather than overriding like
    // the lifetime tag below: a caller-supplied value is an explicit statement,
    // not a guess, so it wins (and a test can pin one without a git repo).
    // Nothing resolvable means nothing set — see the scrub above, which is what
    // makes "unset" mean "unresolvable" instead of "whatever the daemon
    // inherited".
    if !opts
        .env
        .iter()
        .any(|(k, _)| k == DOT_AGENT_DECK_MAIN_WORKTREE)
    {
        let base = opts
            .cwd
            .map(PathBuf::from)
        ...
        if let Some(base) = base
            && let Some(main_worktree) = crate::worktree_owner::main_worktree_of(&base)
        {
            cmd.env(DOT_AGENT_DECK_MAIN_WORKTREE, main_worktree.as_os_str());
        }
    }
...
mod spawn_tests {
    ...
    // ---------------------------------------------------------------------
    // Issue #550 — `DOT_AGENT_DECK_MAIN_WORKTREE`. Every assertion below reads
    // the value back out of the CHILD's real environment rather than out of
    // the builder, because what a consumer relies on is what the agent
    // process actually sees. `worktree_owner`'s own tests cover the git
    // layouts; these cover the injection: right value, right seam, and — the
    // property most worth having — genuinely ABSENT rather than stale when it
    // cannot be resolved.
    // ---------------------------------------------------------------------

    /// Spawn a child with `cwd` and report what it saw for
    /// `DOT_AGENT_DECK_MAIN_WORKTREE`, as the literal `<unset>` when the
    /// variable is absent. Goes through [`spawn`] rather than the registry
    /// because that is the seam the injection lives at, and the one every
    /// launch path funnels through.
    fn child_observed_main_worktree(
        cwd: &std::path::Path,
        extra_env: Vec<(String, String)>,
    ) -> String {
        let dir = tempfile::tempdir().expect("create tempdir");
        let out = dir.path().join("main-worktree.txt");
        let pty = spawn(SpawnOptions {
            command: Some(&format!(
                "sh -c 'printf \"%s\" \"${{DOT_AGENT_DECK_MAIN_WORKTREE:-<unset>}}\" > {}'",
                out.display()
            )),
    ...
    fn git_fixture(dir: &std::path::Path, sandbox: &std::path::Path, args: &[&str]) {
        ...
        assert!(
            out.status.success(),
            "fixture precondition: `git {args:?}` failed: {}",
            String::from_utf8_lossy(&out.stderr)
        );
    }

    /// A checkout with one commit plus a linked worktree cut from it, returned
    /// as `(scratch, checkout, linked)`. The linked worktree is a SIBLING of
    /// the checkout, matching how this repo's own tooling places them, so
    /// resolving to the checkout cannot be an accident of nesting.
    fn checkout_and_linked_worktree() -> (tempfile::TempDir, PathBuf, PathBuf) {
    ...
    fn canon_str(p: &std::path::Path) -> String {
    ...
    /// The case the feature exists for: a pane whose cwd is a linked worktree
    /// is handed the MAIN checkout — the one that survives the worktree being
    /// removed when its branch merges.
    #[test]
    fn spawn_injects_the_main_checkout_for_a_pane_in_a_linked_worktree() {
        let (_scratch, checkout, linked) = checkout_and_linked_worktree();

        let observed = child_observed_main_worktree(&linked, vec![]);

        assert_eq!(
            canon_str(std::path::Path::new(&observed)),
            canon_str(&checkout),
            "a worker in a linked worktree must be told where the durable checkout is; \
             being told its own worktree would be the stall dressed as a success"
        );
    }

    /// An ordinary checkout is itself a main worktree, so the variable is set
    /// there too. That is what lets a role prompt read it unconditionally
    /// instead of branching on how the pane was placed.
    #[test]
    fn spawn_injects_the_checkout_itself_for_an_ordinary_pane() {
        let (_scratch, checkout, _linked) = checkout_and_linked_worktree();

        let observed = child_observed_main_worktree(&checkout, vec![]);
    ...
    /// The fail-closed half, and the property most worth asserting: where the
    /// main worktree cannot be resolved the variable is ABSENT, so a consumer
    /// can tell "the deck does not know" from "the deck says it is here". A
    /// guessed path would be written into by whatever agent trusted it.
    #[test]
    fn spawn_leaves_the_main_worktree_unset_where_it_cannot_be_resolved() {
        let scratch = crate::test_temp::tempdir().expect("scratch tempdir");
        ...
        assert!(
            !crate::worktree_owner::git_at(&plain)
                ...
                .status
                .success(),
            "fixture precondition: {} unexpectedly resolves to a git dir",
            plain.display()
        );

        let observed = child_observed_main_worktree(&plain, vec![]);

        assert_eq!(
            observed, "<unset>",
            "unresolvable must mean absent, never a guess"
        );
    }

    /// The scrub is what makes "unset" mean "unresolvable" rather than
    /// "whatever this process inherited". A daemon launched from inside a deck
    /// pane carries that pane's value; a pane it later spawns in an unrelated
    /// directory must not be told to write its durable report into the
    /// daemon's repository.
    #[test]
    fn spawn_scrubs_an_inherited_main_worktree_it_cannot_re_resolve() {
        let _g = ENV_TEST_LOCK.lock().unwrap_or_else(|p| p.into_inner());
        let scratch = crate::test_temp::tempdir().expect("scratch tempdir");
        let plain = scratch.path().join("not-a-repo");
        std::fs::create_dir_all(&plain).expect("create a non-repo dir");

        // SAFETY: serialized by ENV_TEST_LOCK; the prior value is restored
        // before the lock is released.
        let prior = std::env::var(DOT_AGENT_DECK_MAIN_WORKTREE).ok();
        unsafe {
            std::env::set_var(DOT_AGENT_DECK_MAIN_WORKTREE, "/somebody/elses/checkout");
        }
        let observed = child_observed_main_worktree(&plain, vec![]);
        unsafe {
            match prior {
                Some(v) => std::env::set_var(DOT_AGENT_DECK_MAIN_WORKTREE, v),
                None => std::env::remove_var(DOT_AGENT_DECK_MAIN_WORKTREE),
            }
        }

        assert_eq!(
            observed, "<unset>",
            "an inherited value is exactly the confidently-wrong path this must never \
             hand an agent — the scrub has to run even though nothing replaces it"
        );
    }

    /// Gap-fill, not override: a caller-supplied value is an explicit
    /// statement rather than a guess, so it wins — the same rule
    /// `DOT_AGENT_DECK_SOCKET` follows, and what lets a respawn replay a
    /// pinned `spawn_env`.
    #[test]
    fn spawn_lets_a_caller_supplied_main_worktree_win() {
        let (_scratch, _checkout, linked) = checkout_and_linked_worktree();

        let observed = child_observed_main_worktree(
            &linked,
            vec![(
                DOT_AGENT_DECK_MAIN_WORKTREE.to_string(),
                "/pinned/by/the/caller".to_string(),
            )],
        );

        assert_eq!(observed, "/pinned/by/the/caller");
    }
