...
// PRD #176 M1.1: the wire-shaped `AgentRecord` / `TabMembership` types moved to
// the `protocol` crate. Re-exported here so existing `crate::agent_pty::…` call
// sites (and the daemon-side validation helpers below) compile unchanged. The
// validation logic (`validate_tab_membership`, `is_valid_display_name`, …) and
// the PTY registry stay in this module.
