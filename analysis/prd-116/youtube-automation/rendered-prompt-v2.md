...
1. **Discover the project at `/home/vfarcic/code/youtube-automation`.** Read its build/package files, task runners, scripts, and CI/CD configs to identify the real toolchain. Do NOT assume — derive everything from what's actually in the repo. Probe for:
...
6. Write the approved config to `/home/vfarcic/code/youtube-automation/.dot-agent-deck.toml`. If an orchestration was kept, include `[[orchestrations]]` alongside `[[modes]]` in the same file.
