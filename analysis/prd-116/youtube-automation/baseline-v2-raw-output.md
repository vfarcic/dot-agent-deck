The project defines role-specific agent launchers in devbox.json (Opus for orchestrator/coder, GPT-5.5 for reviewer/auditor, Sonnet for release), making multi-agent orchestration a natural fit. The code-review orchestration chains coder → reviewer & auditor (parallel) → mandatory human gate → release with strict context-handoff discipline. The dev mode pairs `git status -s` persistence with reactive rules for `go test` and `git` commands, and integrates with prds/ and coordination skills for PRD-driven development.
...
[[modes.rules]]
pattern = "git\\s+(log|status|show|diff)"
watch = false
