...
The pilot's most important finding (C4) was that a delta is only a user signal if the relevant prompt/role capability **existed when the user last edited the config**. The config-gen prompt asset (`assets/config_gen_prompt.md`) was created on **2026-04-27** (commit `3b83478`) and remained **unchanged throughout the entire analysis window** — from that date through the latest user-config edit studied here (`dot-agent-deck`, 2026-06-21) and the Phase-2 baseline captures. So across that window the *entire prompt as it stood pre-Phase-3* — orchestration-proposing, the role library, the `reactive_panes` guidance, the consolidated-rules guidance, the release-role guidance — was in effect, which is the state the date gate relies on. (Phase 3 of this PRD subsequently edits the asset to land P1–P4; those edits post-date every config and every baseline the date gate compares, so they do not affect this reasoning. The orchestration *engine*, PRD #58, landed `99a4a03` on 2026-04-21; the prompt began *proposing* orchestrations on 2026-04-27.)
