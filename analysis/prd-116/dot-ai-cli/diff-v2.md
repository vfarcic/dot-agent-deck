...
- **Baseline** (regenerated): `analysis/prd-116/dot-ai-cli/baseline-v2.toml`
...
- **both**: `git status -s` (B name=Some("Status") watch=yes; U name=Some("Git status") watch=yes)

#### `[[modes.rules]]`

- **B-only**: `task\s+(test|build|build-all|fetch-spec|checksums)` (watch=no)
- **B-only**: `git\s+(status|log|diff|show)` (watch=no)
- **B-only**: `go\s+(test|vet|build|fmt)` (watch=no)
