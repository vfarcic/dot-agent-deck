...
- **Asserts:** a tab started with no typed name runs under its canonical orchestration name, and a second tab typing that name in the same directory is refused (the `unwrap_or(name)` fallback the tab label and the form's snapshot apply); the same resolved title in a different directory starts; and once the holder's only pane has exited — its role-map entry still standing, since only a pane close unregisters one — the refused tab's start succeeds. Verified load-bearing: a check that counted a registered pane as a holder without asking the registry whether it is live leaves the title unclaimable and fails the last step.
- **Does not assert:** a daemon restart, which loses the title record with the role maps beside it; the path equivalence of the cwd half beyond a trailing separator (unit-tested in `src/state.rs`); the TUI.
- **Platform coverage:** mac+linux (unix-only — PTY stand-ins over a Unix socket).
