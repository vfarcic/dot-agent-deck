...
- **Asserts (issue #833 extension):** the card's *preferred* title source, `ui.display_names`, is defended at the seams that WRITE it rather than at the render seam, for the two of its writers that carry a producer-supplied string into this crate. (1) Rename: `pane::RenameOutcome::applied` — the gate the dashboard dispatch loop mirrors into that map — refuses a name whose only planted character is a `U+202E` bidi override, which the byte-level `is_valid_display_name` check admitted because the override encodes as three UTF-8 bytes each above `0x20`; running that refusal through the same three-arm mirror the loop applies leaves the card titled as it was, with no control character or bidi override in any cell. (2) Session restore: `ui::mirror_saved_pane_name` — the one way the four `run_tui` restore sites now mirror `SavedPane.name` into `ui.pane_names`, from which the dashboard loop fills `ui.display_names` — refuses the same two hostile names, so neither map gains an entry and the restored card falls back to the session's own scrubbed name; before it, those sites replayed the name through `rename_pane` (which rejects it and stores nothing) and then inserted the raw string anyway. Control on both: `  café-агент-日本語  ` is still Applied and `  example-beta  ` still restores, each trimmed to exactly what the controller put on `Pane.name`.
- **Does not assert:** the third writer of `ui.display_names` — the daemon's `list_agents` echo of `AgentRecord.display_name`, which is scrubbed at the wire boundary and covered by `session/live/007`; the fourth — a new-pane spawn, gated by `agent_pty::resolve_display_name`; an orchestration role pane's title, which is inserted raw from `role.name` in the project's own `.dot-agent-deck.toml` and deliberately left so (that file also supplies each role's `command`, so reading it is already consent to execute what it names); the title's display-column fit budget (`truncate_styled_segments`), which is `dashboard/pane/015`.
- **Platform coverage:** mac+linux+windows.

##### dashboard/pane/014 — Hostile `tool_name` / `tool_detail` on the hook socket are scrubbed and clamped before they can reach a card's tool line (issue #833).
- **Layer:** L1 (in-process `AppState::apply_event` + ratatui `TestBackend` via `render_card_grid_to_buffer`).
- **Agent:** none (four synthetic `ToolStart` hook events, two of them hostile).
- **Asserts:** a `ToolStart` whose `tool_name` carries ESC and NUL and whose `tool_detail` carries a `U+202E` right-to-left override, a DEL and a CR/LF is stored on `active_tool` scrubbed (`Ba[31msh` / `src/gnidaer rm -rf /`), and a 100 KiB detail is stored clamped to `untrusted_text::MAX_TOOL_TEXT_BYTES` on a character boundary with a trailing `…`; rendering all four cards draws four `Working` badges, puts no control character or bidi override in ANY buffer cell, and still shows both the hostile card's readable tool line and both neighbours'. Before the fix `apply_event` stored both strings verbatim on `active_tool` AND in the `recent_events` journal that `ui::recent_tool_lines` draws the card's tool line from, while the SAME two strings arriving over the daemon's `list_agents` echo were already scrubbed and clamped (`session/live/007`) — one field, two routes, one of them unprotected.
- **Does not assert:** `user_prompt` at the same ingest, which is scrubbed on neither route and is outside #833; the `daemon status` JSON document and the desktop DTO, two further readers of `active_tool` that this ingest-time scrub also covers but which are not rendered into a terminal cell here.
- **Platform coverage:** mac+linux+windows.
...
- **Does not assert:** the close-flow consequence of a stable-key generation change — that seam is `prompt/close-confirm/009`, which pins it directly. Issue #317 closed the gap this line used to describe: close confirmation armed on the session id alone (`CloseTarget::Session`), and because Pi reuses `{pane_id}-session` across respawns the armed target stayed resolvable after a generation change, so confirmation could act on whichever generation currently occupied the pane. It now arms on the generation (session id plus `agent_id`) and a refreshed identity reads as vanished. The #284 identity refresh this test pins is what made that detectable — the pre-fix stale `pi-agent-2` identity would have concealed the change — but the fix itself lives at the arm/resolve seam (`CloseTarget` / `resolve_close_plan`), not here. This test also does not assert the initial spawn-time placeholder → first-respawn transition (`status/agent-event/005`), socket transport, or rendered card history.
- **Platform coverage:** mac+linux+windows.
...
##### status/supersede/016 — A card a reconnect seeded from the daemon's snapshot is superseded exactly where the daemon's own session is (issue #804).
- **Layer:** L1 (two in-process `AppState`s, a daemon one built through `apply_event` and a TUI one seeded from the daemon's `live_session_for` answer through `seed_hydrated_session`; no socket).
- **Agent:** none (synthetic Pi generations; the incumbent declares a `history-only` live target so `pane_writable`'s pick is visible).
- **Asserts:** right after the reconnect the seeded card's `last_activity` equals the daemon's quiet instant and the `ListAgents` join answers identically on both sides; a frame from a different agent, applied to both, retires the reconnected card exactly when it retires the daemon's (stamped 30 minutes after the quiet instant it does, 30 minutes before it does not), and `pane_writable`, `pane_session_id` (by the owning agent) and `live_session_for` for both agents then agree between the two states. Against the unfixed seeding the first run diverges: the TUI keeps a card the daemon retired.
- **Does not assert:** the daemon's `ListAgents` handler over a socket (`session/live/002`, `/003`); the daemon's registry-ownership ground for supersession (`generation_disowned`, issue #454), which a client state has no oracle for, so the daemon here has none either; a sub-millisecond disagreement, which the tests avoid by stamping whole milliseconds — the wire carries milliseconds, so a frame within the same millisecond as, and before, the daemon's instant is judged against the truncated value.
- **Platform coverage:** mac+linux+windows.

##### status/supersede/017 — The reconnect overlay takes only fresher evidence: a card the upsert keeps is overlaid only by a newer, non-future snapshot (on a tie it adopts only a missing live target), and a minted card changes no newest-wins pick (issue #804, PRD #1223).
- **Layer:** L1 (in-process `AppState`; events applied through `apply_event` before and after `seed_hydrated_session`).
- **Agent:** none (synthetic Pi generations; the snapshot declares `history-only` and a distinctive tool count so every pick's choice is visible).
- **Asserts:** with a `SessionStart` from the same agent that reached the pane before hydration, the pane keeps exactly ONE card (PRD #1223's upsert) in every case. When the snapshot's instant is strictly newer than that card's and not in the future, the card takes the snapshot's `last_activity`, `status`, tool count and `history-only` live target, and `pane_session_id`, `pane_writable`, `agent_writable` and `live_session_for` all answer from it. When the snapshot only TIES the card and the card declares no live target, the card adopts the snapshot's `history-only` live target (one carrier event, and `writable`, `pane_writable` and `agent_writable` report history-only) and nothing else: the join answer differs only in `live_target`, and `status`, tool count and `last_activity` are the card's own. When the snapshot TIES a card that declares its own live target, or is newer but stamped in the future, the card is left exactly as it was: same join answer, same live target, no carrier event pushed, same `last_activity`. For a freshly minted card: a future-stamped session from another agent that beat the minted card still beats it when the snapshot is stamped further out; a newer paneless report from the same agent keeps `agent_writable` on the seeded card; a paneless report that only TIES the snapshot leaves the minted value rather than creating a tie (this case took over the tie coverage the #804 version's case (a) gave the minted branch); and a re-seed from a snapshot older than a frame the card has since seen does not move `last_activity` backward. Checked by mutation with PRD #1223's rewrite: removing the kept card's overlay, its strictly-newer half or its not-in-the-future half, or making the minted branch's newer-than-evidence comparison non-strict, turns this test red; so does removing the tie's live-target adoption, dropping its has-no-live-target guard, or widening it to the whole overlay.
- **Premise changed with PRD #1223:** as written for issue #804, cases (a) and (b) asserted a SECOND card, `pane-<id>`, minted beside the pre-hydration `SessionStart`'s card, and that every newest-wins pick chose it. PRD #1223's upsert removed that duplicate on purpose, so one agent on one pane has one card. The property the cases protected (the fresher evidence wins on reconnect) is kept, and is now asserted against the single card. The duplicate was not lost by accident.
- **Does not assert:** the readout (`session/live/016`); agreement with the daemon's own decisions (`status/supersede/016`); a future-stamped snapshot against a successor (`status/supersede/018`); the upsert itself from the placeholder side (`state::tests::placeholder_for_an_already_announced_generation_keeps_its_card`).
- **Platform coverage:** mac+linux+windows.

##### status/supersede/018 — A future-stamped snapshot cannot pin the reconnected card against an honest successor (issue #804, Greptile P1 on PR #1225).
- **Layer:** L1 (in-process `AppState`; `seed_hydrated_session`, then one frame through `apply_event`).
...
- **Asserts:** `process_table(&[own_pid])` enumerates the machine's processes and `descendants()` finds a real grandchild of the test process (spawned on pipes, detached via `setsid()`) as a descendant of the test's own pid; the found entry reports no controlling terminal, session-leader true, its full argv (a uniquely marked command line, reached through `CommandLine::read()` and required to be `Read` — since issue #862 the sample reads a command line only for the detached descendants of the roots it is given, and this test's own pid is that root), and a session id differing from the test process's own (checked independently via `libc::getsid(0)`) — the real-process proof that `getsid` reads what the M2 discriminator's load-bearing condition assumes it reads. On Windows, `process_table()` returns `None` with roots and without them (no process-enumeration backend exists there — same contract as `foreground_pgid`).
...
##### status/shell-activity/008 — A process-table sample reads a command line for the session-boundary descendants of its own roots and for nothing else on the machine (issue #862).
- **Layer:** L1.
- **Agent:** none (the same real `sleep` grandchild `status/shell-activity/001` spawns and `setsid()`s — no agent involved).
- **Asserts:** with the test's own pid named as the sample's root, `process_table(&[root_pid])` returns `CommandLine::Read` carrying the uniquely-marked full argv for the real `setsid()`'d grandchild; `CommandLine::NotSampled` for `mid`, a real child of the test in the test's OWN POSIX session (so the scoping is by session, not merely by descendancy); and `NotSampled` for every other row in the table, asserted by enumerating the rows that carry a command line and requiring the grandchild to be the only one. Also asserts `shell_tool_candidates()` reports exactly that one pid, since the scoping stops **at** the session boundary rather than descending past it — a process below the grandchild would inherit its session and must still not be read, which is what keeps a Bash-tool call's whole build tree out of the argv phase. Then, with **no** roots, `process_table(&[])` returns `NotSampled` for every row including the grandchild — which is what makes the previous claims a property of the roots rather than an accident of this process tree.
...
- **Does not assert:** the marker file's *content* or format (the ownership gate never parses the document; the unit tests in `src/issue_dispatch_run.rs` cover the write's placement, tree-cleanliness and idempotency, and `011` covers its length); the `reclaim` removal path for this fixture (`002`); retro-marking of pre-existing worktrees, which is deliberately not done at all.
- **Platform coverage:** mac+linux.

##### worktree/reclaim/011 — A zero-byte ownership marker — the residue a torn `std::fs::write` leaves — must not resolve to ownership, so the worktree falls to `ask` rather than to the unattended `remove`.
- **Layer:** fast synthetic real-binary-subprocess integration (as `worktree/reclaim/001`), with `010`'s addition: the subject worktree is created by calling the production `issue_dispatch_run::create_worktree` in-process, so the marker arrives from the creation path and truncating it is the only thing that changes between the two measurements.
- **Agent:** none (one worktree on a canned `MERGED` PR, clean, measured twice — once with its marker intact, once after that marker is truncated to zero bytes in place).
- **Asserts:** a **fixture precondition** that the creation call returned `WorktreeCreation::Created` and that the marker it wrote starts out a non-empty file; a **control** that with that intact marker the entry reads `owned: true` / `verdict: "remove"` — without it, "foreign after truncation" could equally mean the worktree was never owned at all; a second **fixture precondition** that the truncated marker is still a zero-byte *file* on disk, ruling out "the marker was simply removed", which is the already-covered plain foreign case (`005`); then that the same entry now reads `owned: false` and `verdict: "ask"`; and finally that a bare `worktree reclaim` leaves the directory on disk, so the assertion reaches the deletion path and not only the report. Pins issue #946: the ownership gate was `is_file()` alone, so a failed marker write — `std::fs::write` is `File::create` + `write_all`, and `File::create` truncates before the first byte lands — left a file that read as `Ours`, sending a merged, clean worktree to `Verdict::Remove`, the branch with no confirmation, in exactly the case whose warning promised the operator one.
- **Does not assert:** the write-failure path itself, or which `MarkerWriteError` arm a given failure carries (`src/worktree_owner.rs`'s unit tests stage a real `ENOSPC` through `/dev/full` and an unremovable leftover through a read-only metadata dir); a *partial but non-empty* marker, which still reads as a claim by design — closing that would take the temp-and-rename dance the module argues against.
- **Platform coverage:** mac+linux.
...
##### prompt/close-confirm/009 — A respawn under Pi's STABLE producer key closes nothing, because the armed target is a generation and not a session id (issue #317).
- **Layer:** L2 (real-binary PTY plus two synthetic `SessionStart` frames delivered through the real daemon hook socket under one pane-derived producer key).
- **Agent:** none (continued `cat` pane; the hook frames give the same pane two successive agent generations in rendered state).
- **Asserts:** after Ctrl+W arms a card whose session id is `{pane_id}-session`, a second `SessionStart` under that SAME id differing ONLY in `agent_id` takes the pane; confirming surfaces `Nothing closed`, retains the card, and leaves the daemon agent alive rather than tearing down the replacement generation.
- **Why it exists:** `prompt/close-confirm/005` proves the vanishing case through a session id that goes away, which is the easy half — the key stops resolving and the close cannot land anywhere. Pi's producer key is derived from the pane and is stable across respawns, so it never stops resolving, and an armed target keyed on the id alone resolved straight onto whichever generation currently occupied the pane. The fix arms on the generation (session id plus `agent_id`), which is only detectable because #284 refreshes `agent_id` in place on the surviving entry.
- **Does not assert:** the state seam's own supersession rule (`status/supersede/005`); tab identity binding (`mouse/tabstrip/003`); that a `None` → `Some` identity transition is treated as the same card (a deliberate agreement with the state seam, unit-tested at `resolve_close_plan`).
- **Platform coverage:** mac+linux.
...
##### prompt/pane-input/034 — An armed native-seed PTY-injection fallback is refused, with no bytes written and the seed dropped, when the pane changed hands inside the grace (issue #617, finding 6).
...
- **Agent:** none — a `sh -c 'read _line'` predecessor that leaves by exiting when one unsubmitted line ends its `read`, and a `/bin/cat` successor inheriting its `DOT_AGENT_DECK_PANE_ID`. The grace is passed as a PARAMETER (1500 ms), so the 15 s default and its second-granularity `DOT_AGENT_DECK_SEED_FALLBACK_SECS` env var are both out of the picture; it is an order of magnitude larger than `prompt/pane-input/035`'s because this hand-over waits on a child exiting and its reader thread seeing EOF rather than on a synchronous `close_agent`.
- **Asserts:** four things. `arm_seed_fallback` is a fire-and-forget `tokio::spawn` that returns nothing, so the refusal is read from the only place the code publishes it: a thread-scoped `tracing` subscriber at WARN must capture `seed fallback: identity gate refused the PTY injection` naming `WrongSession`, which also proves the armed task actually fired. Then the refused seed must be DROPPED rather than restored to the record it was stashed on — `take_pending_seed_fallback_for(pane, original)` is `Absent` — since nothing re-arms this fallback, so a restored seed is one no delivery path will ever come back for. Then it must not have been re-filed under the pane's NEW occupant either — `take_pending_seed_fallback_for(pane, successor)` is `Absent` — where it would be pullable by that occupant's own `get-seed`, re-opening the mis-delivery by the native route after the injection route refused it. Then the seed's bytes must be absent from the successor's scrollback, barriered by an `Applied` authorized write asserted visible first.
- **The seed the fallback finds at fire time is the DEPARTED agent's OWN, and that is what keeps this test on the WRITE-level half of the property.** The predecessor leaves by exiting rather than through `close_agent`, so its record survives flagged `exited` and the seed stashed for it is still that agent's own seed when its timer fires; `spawn_agent`'s duplicate-pane-id check skips exited entries, so the successor can claim the same pane id, and the successor deliberately stashes nothing. A write-level refusal needs a predecessor whose record — and so whose own seed — outlives it, which `close_agent` leaves neither of: since the take became identity-scoped (issue #617, Greptile P1 on PR #919) a seed stashed for the pane's NEW occupant is never taken by a departed agent's fallback, so a fixture built on one is refused at the take instead and is `prompt/pane-input/035`'s. The successor's own fallback is deliberately NOT armed, so the only thing that could type this seed into its PTY is the original's armed task.
- **Why it exists:** nothing in either tier called `arm_seed_fallback` at all before this — the three tests that name the grace (`e2e_pi_live.rs`, `e2e_pi_worker.rs`, `e2e_delegate_work_done_chain.rs`) are all lane 2 and all set `DOT_AGENT_DECK_SEED_FALLBACK_SECS=600` precisely so the fallback does NOT fire. The registry's `take`/`set` arbitration was unit-tested; the write was not. Measured: with the pre-lock identity gate in `write_guarded` temporarily disabled, this test fails at its first assertion with `captured log = ""` — the injection is delivered instead of refused — so the gate is what it discriminates.
- **Does not assert:** the NATIVE `get-seed` pull that normally wins the race (`take_pending_seed_native` and its flag are unit-tested beside it); the leave-it-alone branch taken when the pane's seed belongs to the successor (`prompt/pane-input/035`); the two production callers that arm it; the `Ambiguous` partial-write branch.
- **Platform coverage:** mac+linux (the `/bin/cat` byte target and the `sh` predecessor are POSIX).

##### prompt/pane-input/035 — A successor's own stashed seed survives a departed agent's expiring PTY-injection fallback and is still pullable by the successor (issue #617, Greptile P1 on PR #919).
- **Layer:** fast integration, in `src/agent_pty.rs`'s own `mod spawn_tests` (the real `arm_seed_fallback` against registry-owned PTYs; no daemon, no pi, no extension, no LLM and no `e2e` feature gate).
...
##### prompt/pane-input/036 — A card drawn from the command is not an agent that can read: the seed waits for a producer to announce a conversation, and the 10-second slow path still delivers for one that never will (issue #1005).
- **Layer:** L1, in `src/ui.rs`'s own test module (the production `process_pending_seed_prompts` driven over a recording pane controller and a real `AppState::apply_event`), so it runs under `cargo test-fast` and therefore in the `build` job.
- **Agent:** none (a synthetic Claude Code card-surfacing `SessionStart` and a synthetic genuine one).
- **Asserts:** a pane whose ONLY session is the daemon's card-surfacing `SessionStart` — `session_id` is the pane id, no `agent_id`, `agent_type` resolved from the command by `spawn::surface_spawned_pane` — gets NO write across two passes even with the 500 ms `SPAWN_TIME_READINESS_BUFFER` already elapsed, and the seed is HELD rather than dropped; the agent's own `SessionStart` then releases exactly one write, carrying the seed itself and naming that generation. Separately, and in both the card-surfaced and the no-session-at-all shapes, a seed older than 10 s IS delivered by the `timeout_ready` slow path.
- **Why the second half is here.** Narrowing the fast path onto `pane_hook_session_id` makes the 10-second slow path the ONLY delivery a producer that announces nothing ever gets — opencode's cold boot (#146), Pi's `NoSignal`, `scheduler/spawn/005`. A change that fixes `claude` and strands those is the stated failure mode to design against, so it is pinned beside the narrowing rather than left for whoever breaks it. It is a REGRESSION GUARD, not a reproduction: today's over-eager gate delivers these too, so this half is green before the fix as well as after.
- **Does not assert:** that `spawn::surface_spawned_pane` stamps the origin marker (replayed here, pinned by `surface_spawned_pane_emits_session_start_for_attached_tuis` in `src/spawn.rs`); that a provisional start does not ESTABLISH the pane's generation (`prompt/pane-input/033`); the end-to-end ordering through the real binary (`prompt/new-pane/017`); or the daemon-owned delivery paths, which gate on `state::wait_for_session_start` instead.
- **Platform coverage:** mac+linux+windows.

##### prompt/pane-input/037 — The shared readiness predicate, and the orchestrator remit that reads it, both wait for a conversation (issue #1005, retargeted by #1012).
- **Layer:** L1, in `src/ui.rs`'s own test module (the production `spawn_time_agent_ready` called directly, and `deliver_orchestrator_prompt` over a recording pane controller).
- **Agent:** none (a synthetic Claude Code card-surfacing `SessionStart` and a synthetic genuine one).
- **Asserts:** `spawn_time_agent_ready` is false for a pane with no session at all, and still false once the daemon's card-surfacing `SessionStart` has resolved an agent type off the command (asserted as a precondition, alongside `pane_hook_session_id` being `None`) — only the producer's own announcement opens it. Then, for a start-role pane in that same card-only shape and with the 500 ms `SPAWN_TIME_READINESS_BUFFER` pre-spent, `deliver_orchestrator_prompt` puts no bytes in the pane across two passes and does not consume the remit; the agent's own `SessionStart` releases exactly one write, carrying the remit itself and naming the generation it entered.
- **Why the predicate is pinned directly, and why this entry changed.** Issue #1005 narrowed the PREDICATE and each delivery site is one of its callers, so the first half used to reach it through a third caller, `process_pending_dispatches`. Issue #1012 removed that caller: no production code had pushed to its queue since PRD #93 round-5 moved delegation dispatch into the daemon, so the `retain` drained an always-empty `Vec` every frame. Asserting `spawn_time_agent_ready` itself is what that half was really about, and unlike a per-caller fixture it keeps standing for a caller added later. The orchestrator site's buffer is pre-spent in the fixture so the only thing under observation there is the readiness gate.
- **Does not assert:** the seed path (`prompt/pane-input/036`), which is the predicate's other live caller and covers the 10-second `timeout_ready` slow path this predicate never gates; confirmation, retry or deadline behaviour on the orchestrator path (`prompt/pane-input/023`–`/030`); the daemon-owned delegate path, which gates on `state::wait_for_session_start` instead.
- **Platform coverage:** mac+linux+windows.

##### prompt/pane-input/038 — An erase burst of exactly one `DEL` per payload character removes the payload from a REAL interactive Claude prompt and leaves the word before it untouched (issue #876).
- **Layer:** L2 (PTY-attached real binary, runtime-skipped when Claude CLI/credentials are unavailable; flaky-tolerant lane-2 tier).
- **Agent:** REAL interactive Claude Code on `claude-haiku-4-5-20251001`, with onboarding/project trust seeded and `--allowedTools Bash Read`; no `-p`.
- **Asserts:** the one premise issue #876's drain rests on, against the agent whose editor actually has to honour it. After the real Claude pane registers and the genuine interactive prompt renders, a sentinel word and a stand-in payload are typed — the exact state an `Ambiguous` guarded submit leaves in an input box — and then ONE write carrying exactly `payload.len()` `DEL` bytes is sent, followed by a third sentinel. The live prompt must then read `<kept> <tail>` CONTIGUOUSLY, which is what makes the count exact in both directions: one erase short wedges a leftover payload character between the two words, one too many eats the space and then the word the user typed. A second phase then types 600 filler characters — between the ~524 bytes a real idle prompt encodes to and the 1024-byte cap — and erases exactly that many in ONE write, asserting the prompt collapses back to the same two words: the short phase pins the count, this one pins the SHAPE, since a burst that size is what a TUI classifying bulk input heuristically would be most likely to read as a paste and insert as text. Returning to command mode leaves the pane visible and the same daemon-side agent record present. The byte is the same one the daemon writes: the deck forwards `KeyCode::Backspace` as `0x7f` (`ui::keyevent_to_bytes`) and `agent_pty::PANE_ERASE_BYTE` is `0x7f`.
- **Verified load-bearing:** sending `payload.len() - 1` erases turns it red against the real Claude UI, whose prompt then renders `❯ erase_keep_4c71 eerase_tail_9e33` — the single leftover character an absence-only assertion could not have seen.
- **Why a lane-2 test rather than a deterministic one:** the drain's arithmetic is unit-tested against a fault-injecting writer and its *effect* against the kernel's canonical-mode line discipline (`agent_pty::erase_bytes_undo_exactly_our_write_and_leave_the_users_draft`), but neither is an agent TUI implementing its own editor. `\x7f` not being honoured is the one failure mode that would make the daemon record a drain it did not achieve, so it is pinned where it can only be pinned — on a live agent.
...
##### tabs/orchestration/013 — A dispatched orchestration's run-identifying tab label reads its canonical NAME first under the tab strip's trailing-ellipsis truncation (issue #960).
- **Layer:** L1 widget (in-process `TestBackend` through the production `render_tab_bar_to_buffer` → `render_tab_strip` → `fit_tab_labels` path; no subprocess, no PTY, no LLM).
- **Agent:** none — the input is a label string, which is all the renderer sees.
...
##### hooks/ingest/005 — A producer-supplied id or raw line cannot forge, overwrite or reorder a line in the daemon's log (issue #1082).
- **Layer:** L1 (the real `run_hook_loop` driven against a real Unix hook socket in-process, with a real `tracing` subscriber capturing what it writes; no PTY, no deck binary, no agent).
- **Agent:** none (one message of every `DaemonMessage` verb, a raw `AgentEvent`, an event whose `event_type` is a typo, and a line that is not JSON — all on one connection, all carrying ids built from LF, CR, ESC+CSI, U+0085 and U+202E).
- **Asserts:** no line the daemon wrote contains a control character or a Unicode bidi format character, while the readable words between them survive — escaping preserves the evidence rather than dropping it. It also asserts, by message text, that the hostile ids actually REACHED each swept site (the provenance refusal, all seven verbs, the two unknown-pane warnings, the `Received event`, unrecognized-`event_type` and foreign-`SessionStart` warnings, and the malformed-line branch), so a site quietly dropped from the sweep fails here rather than passing vacuously. Offending lines are collected, not asserted one at a time, so reverting one call site names that call site.
...
- **Layer:** L2 (three real `cargo build`s into one shared scratch `CARGO_TARGET_DIR`, pinned to the rustc host target and capped at half the machine's cores, then plain subprocess runs of each produced binary — no PTY). Issue #928: nextest schedules it at the FRONT of the queue (`priority` in `.config/nextest.toml`) rather than at its natural binary-name position — it is the tier's longest test, and the offset at which it used to start set the whole `e2e-deterministic` job's wall clock.
- **Agent:** none.
...
- **Asserts:** starting from the owner's two-viewer shape, where the focusless fallback has the real narrow TUI demonstrably cap the shell below the desktop's 153 columns, detach that TUI through the product's user-facing Detach flow while the desktop-shaped attach remains. The daemon settles at 22x153 and, independently, the shell's own `stty size` reports 22 rows and all 153 columns. This is the half that makes the fallback reversible: without it, an agent would ratchet down to the smallest pane that ever looked at it and stay there, which is the sticky behaviour issue #883 reported and this PRD replaces.
- **Does not assert:** last-focused-client selection; how quickly the shell grows (the assertion is a settle, not a latency bound); release on a client that *crashes* rather than detaching cleanly — the daemon-side guard covers both paths identically because it runs on the attach task's drop, and the unit test `releasing_a_viewer_grows_the_agent_back` pins the registry behaviour directly.
- **Platform coverage:** mac+linux.
...
- **Asserts:** `request_from_socket`, driven on a worker thread and awaited via `mpsc::recv_timeout` at 15s (comfortably above the production 5s bound the fix added), returns before the 15s bound and the returned value is exactly `None` — a timed-out/unbounded daemon must fold into the same "no seed" bucket as a daemon that closes without replying, not a distinct outcome. The bug it was written against: `request_from_socket` had no read/write bound of its own and relied entirely on the daemon closing the connection, so against this stub it hung forever. A `RecvTimeoutError::Timeout` is therefore that regression coming back, and fails the test with an explicit panic message rather than hanging until nextest's own timeout.
- **Does not assert:** the exact timeout duration the fix settled on (only that it is comfortably under 15s); `SocketReply`'s three-way outcome (only `request_from_socket`'s two-way `None` collapse is exercised here, which folds `NoReply` and `Unreachable` into the same `None` — within `error/socket`, `004` and `007` assert `SocketReply::Line`, `006` asserts `SocketReply::NoReply`, and none of `001`–`008` asserts `SocketReply::Unreachable`); real daemon behavior; Windows named-pipe timeout semantics.
- **Platform coverage:** mac+linux (Unix-domain socket).
...
- **Asserts:** `request_from_socket` returns `Some("{\"seed\":\"abc123\"}")` — the exact reply line, unmodified — proving the timeout bound added for `error/socket/003` does not fire against a daemon that is merely slow. It passed both before and after that fix landed; it is a correctness control, not a timing measurement, and the delay is deliberately far from the 5s bound to avoid flaking under scheduler jitter.
...
##### error/socket/005 — `request_from_socket` returns within a bounded wait against a peer that dribbles one non-newline byte just before each per-read timeout, because the total-operation deadline fires even though every byte resets the per-read idle timeout.
...
- **Asserts:** `request_from_socket`, driven on a worker thread and awaited via `mpsc::recv_timeout` at a 15s ceiling — comfortably above the operation-level deadline the fix added, and comfortably inside the 20s the drip keeps running — returns before the ceiling. The bug it was written against: the per-read idle timeout added for `error/socket/003` re-armed on every byte received, so each dribbled byte restarted it before it could fire and the call never returned, even though the peer never went silent for as long as one read; the fix answered that by counting the same 5s down as a **total-operation** deadline as well, which fires regardless of how the peer paces its bytes. A `RecvTimeoutError::Timeout` is therefore that regression coming back, and fails the test with an explicit panic rather than hanging until nextest's own timeout. Deliberately does not pin the exact deadline value, so it keeps passing under any sane operation-level bound.
- **Does not assert:** the exact operation-level deadline the fix settled on; the reply-length cap (a separate, deliberately out-of-scope follow-up); any other caller of the shared `request_from_socket_inner` code path — the one that carried the unbounded read — with a different timeout value; this test exercises the choke point itself, so any future caller inherits the same coverage.
- **Platform coverage:** mac+linux (Unix-domain socket).
...
##### error/socket/013 — A request write that breaks with bytes already gone is `SocketReply::NoReply`, not `SocketReply::Unreachable`.
- **Layer:** L1 (`src/hook.rs`'s `#[cfg(test)] mod tests`; same synthetic stub-daemon setup as `error/socket/003`–`007`, over a real temp Unix socket — no PTY, no daemon binary, no real agent).
- **Agent:** none (a `std::thread` stub daemon that accepts one connection and then never reads a byte from it, holding it open until the client has returned).
...
- **Agent:** REAL interactive OpenCode (no `opencode run`, no stand-in) as the `clear = true` `coder` role, on whatever `common::opencode_test_model()` resolves — `anthropic/claude-haiku-4-5-20251001` by default since issue #922, overridable per host with `DOT_AGENT_DECK_OPENCODE_TEST_MODEL`. That accessor is the single source of truth and is deliberately not restated as a literal here: this line said `openrouter/openai/gpt-4o-mini` for two defaults after that stopped being true. The deterministic orchestrator role invokes the genuine delegate CLI.
...
- **Could not pass at all between an unknown OpenCode release and 2026-09-08 (issues #878/#921), and it failed in the FIRST assertion above rather than in anything it exists to cover.** The readiness needle was the literal `"Ask anything..."` (ASCII `2e 2e 2e`) while OpenCode paints `Ask anything…` (one U+2026, `e2 80 a6`); the wait underneath is a plain `contains`, so it burned its full 120 s on a run where OpenCode had booted and was visibly at its composer, then blamed credentials. Repaired by matching `Ask anything` and leaving the decoration out — see `RealDelegateCase::input_ready_needles`, which explains why pasting in the new glyph was the wrong repair — and the diagnostic no longer asserts a cause. **This is CLAUDE.md rule 5's stated cost, observed:** nothing in CI runs this lane, the trigger was an upstream UI string rather than any change here, and the sibling Claude arm `/014` was unaffected because its needle (`? for shortcuts`) carries no decorative glyph.
- **Also asserts, since issue #243:** that the pointer is SUBMITTED within 20 s of the delegate being released. OpenCode declares `PrePromptReadiness::NoSignal` (measured in #146), so before the fix every `clear = true` delegate to it sat out the full 30 s `SESSION_START_WAIT_TIMEOUT` waiting for an event that cannot arrive and only the fallback delivered — under which every other assertion here still held, which is why the bound is the one thing that tells the fixed path from the broken one. Justified from both ends in `OPENCODE_DELEGATE_TO_SUBMIT_BUDGET`: a full 11 s under the ~31 s the pre-fix path burned (the figure `orchestration/delegate/030` measures in virtual time), and generous above the deck's own contribution — measured at **257 ms**, since `delegate→submit ≈ buffer + 0.26 s` — plus the 8000 ms `NO_SIGNAL_READINESS_BUFFER` this test now pins. **The caveat is discharged and the bound stays 20 s.** "Nobody has measured how long a REPLACEMENT OpenCode needs to consume a pointer handed to it 1 s after spawn" was answered on 2026-08-26 across 176 real-agent runs: the requirement is one observable boundary, the `Ask anything` composer paint (quoted without its trailing ellipsis on purpose — that glyph is not stable across OpenCode releases, see #878/#921), at 2.5 s idle / 4.5 s contended / 12 s at 4x oversubscription. At the shipped buffer this leg costs ~8.3 s of the 20 s, leaving 11.7 s of margin; do not widen it, since past ~25 s it stops separating a prompt delivery from the dead wait. A slower box gets the operator override, not a bigger budget.
...
- **Platform coverage:** mac+linux (unix-only PTY/UDS; local real-agent tier).
- **Cost note:** one short cheap-model worker turn per observation (Haiku at the current default), plus the one-turn model probe `check_opencode_available` runs as its preflight since issue #922.
...
- **Asserts:** the precondition that the close IS still in flight when the delegate lands (`pane_close_in_flight`), so the test cannot silently degrade into an ordinary post-close delegate; then that the worker pane gets a live agent again, that agent physically receives the `worker-task-coder.md` pointer, and the role is STILL registered in `pane_role_map` afterwards — the last one because without it the delivery succeeds while the NEXT delegate is rejected with `reached no worker for role(s)`, which is the permanent breakage #606 reports. Since issue #960 it also asserts the RE-CREATED pane's rebuilt `TabMembership` — its `display_title`, `role_index` and `orchestration_id` — because `PaneRecreateIdentity` hardcoded `display_title: None`, so a re-created worker silently dropped its tab's run-identifying label. That loss is invisible at first (`partition_hydrated_panes` keeps the first non-`None` title it finds, and the orchestrator pane still has one) and lands only once every title-carrying pane has exited or been re-created this way — which is why the fixture now carries a title distinct from the orchestration name, and why the assertion is on the re-created pane rather than on the bucket. Verified load-bearing: reverting the recreate, or only the settle-wait that precedes it, turns it red; reverting the title carry-forward alone fails the membership assertion at `left: (None, 1, Some("recovery-instance-1"))`.
- **Does not assert:** what the TUI paints for the recovered card (the daemon-side re-creation is what is under test; live surfacing of a daemon-spawned pane is PRD #222's); the close's own outcome, which is awaited only so the test does not outrun it; the reattach that would actually reveal a dropped title (`orchestration/dispatch/005` drives the hydration round trip, and the identity rules of the sibling lookup are unit-tested in `src/state.rs`); and, since issue #1114, either of the two OTHER ways a `clear = true` delegate reaches the same end state — landing before `close_agent` removed the record (`orchestration/delegate/032`) or meeting a close that outran the settle window (`/033`). This test enters the window through the real `StopAgent` over the socket, which is why it cannot aim at either.
...
##### orchestration/delegate/031 — An AMBIGUOUS delegate task pointer keeps its payload record only while its bytes are STILL IN the worker's input box; erased back out, the next delegation's identical pointer is admitted.
- **Layer:** L1 (in-process production task-file resolution and pointer composition, the detailed guarded submit path against three real `/bin/cat` PTY panes — two of them driven through a FAULTED writer so the `Ambiguous` outcome is produced by the production classification rather than supplied to it — and the production settle decision `settle_one_shot_payload_record` driven with each outcome that leaves a record).
- **Agent:** none.
- **Asserts:** the production pointer (`resolve_delegate_task_body` + `compose_delegate_prompt`, task file written into a real tempdir) is delivered into three worker panes. Two are genuinely `Ambiguous` — the whole payload accepted, the submit CR refused — differing in ONE thing: whether the writer recovers in time for issue #876's erases. On the pane whose erases could not land the pointer bytes are still in the input box (asserted on the writer's byte log: exactly the payload, not one erase), the payload record survives, and once the user types the next delegation's pointer is refused as `RefusedUserInput` — the outcome that publishes a `DeliveryNotice` on the worker's card — instead of appending the leftovers to the user's unsent draft and submitting both as one turn. On the pane whose erases DID land the log carries one erase per payload byte, no record is kept, and the next delegation's identical pointer is `Applied`: refusing there would cost a whole delegation to guard a pane that is already clean. A third pane runs the sequence with `Applied` as a control and is likewise admitted. Separately asserts the two preconditions the branch rests on: two different task bodies compose the SAME fixed one-liner (so a later delegation's bytes are byte-identical by construction), and the pointer is plain printable ASCII (so an exact undo is provable). `dispatch_one_owned`'s half of issue #715, and #876's residual on the same path.
- **Verified load-bearing:** stubbing `drain_stranded_payload` to abstain unconditionally turns the drained pane's erase-log assertion red (`left: 56, right: 112`); the stranded pane and the `Applied` control stay green, which is what makes the two branches independent rather than one assertion in two costumes.
- **Does not assert:** that `Ambiguous` still sets `dispatch_one_owned`'s `delivered` flag, so the commission stays owed and the silence watch stays armed (unchanged by #715/#876 and deliberately NOT folded into this decision — the flag answers "did this reach the worker?", the record answers "is the input box empty again?"); that the refused later delegation's `DeliveryNotice` and commission release actually fire (`RefusedUserInput`'s own arm, reached here only as an outcome value); that a real agent TUI honours the erases (the kernel line discipline stands in for one in `agent_pty`'s `erase_bytes_undo_exactly_our_write_and_leave_the_users_draft`, and no fault seam for a partial PTY write exists at the e2e layer); and the `DeliveryNotice` the daemon publishes when bytes ARE left behind.
- **Platform coverage:** mac+linux.

##### orchestration/delegate/032 — A `clear = true` delegate landing BEFORE the close removed the pane's record does not destroy that record, and the role comes back (issue #1114, the wider of its two entries).
- **Layer:** fast integration (the real `handle_delegate_with_state` against daemon-owned PTYs, a real registry, and the real `StopAgent` sequence — assembled statement by statement so the delegate can stand INSIDE it; no LLM and no `e2e` feature gate).
- **Agent:** none — plain `cat` stand-ins for both roles, because what is under test is the daemon's own lifecycle ordering rather than anything an agent does. The replacement's readiness signal is injected over the hook socket, as the rest of the fast delegate suite does for a stand-in.
- **Asserts:** two preconditions that keep this scenario distinct from `orchestration/delegate/022`'s — the pane reads as mid-close AND its registry record is still PRESENT, which is the window between `begin_pane_close` and the `spawn_blocking` hop that runs `close_agent`. Then that the in-flight close still finds the agent it was taking apart (an `Err(NotFound)` there is the delegate having lifted the record out from under it — `respawn_agent_for_pane_declared` removed the entry and only THEN called `spawn_agent`, which refuses a pane held for cleanup); that the worker pane gets a live agent again; that it physically receives the `worker-task-coder.md` pointer; and that the role is STILL in `pane_role_map`, without which the NEXT delegate is rejected with `reached no worker for role(s)`.
- **Verified load-bearing:** the pre-fix build fails the close-outcome assertion at 1.1 s with `error = NotFound("2")`, and — with only that assertion relaxed — fails the user-visible one at 21 s with the worker pane absent from `agent_records()` entirely. Reverting only the under-lock cleanup-hold refusal in `respawn_agent_for_pane_declared` turns it red again while `/033` stays green, which is what makes the two entries independent rather than one defect in two costumes.
- **Why the close is assembled rather than driven over the socket:** the scenario IS the interior of `StopAgent`'s arm, so a faithful reproduction has to be able to stand between two of its statements. Everything else is the production path. `CLOSE_WINDOW_HOLD` (1 s) is a fixed interval, unlike the two issue #709 removed from `/022`, because over-holding is safe in BOTH directions here — and there is deliberately no early exit on "the record has gone", since the record going IS the defect and ending the hold on it hands a broken build a pane that is no longer held: measured, that made the pre-fix build come back green in 1.18 s.
- **Does not assert:** the mirror-image window, where the hold is taken AFTER this respawn lifted the record out rather than before (`orchestration/delegate/034`); the tail of that same race, where a hold is taken between the recovery's wait ending and its next respawn attempt — the create leg's `DuplicatePaneId` retry covers it and no test forces it, because doing so needs a seam between two adjacent statements inside the registry; the give-up boundary (`/033`); what the TUI paints for the recovered card; the close's own pane-scoped cleanup beyond the role map.
- **Platform coverage:** mac+linux (unix-only, with the rest of this file).

##### orchestration/delegate/033 — A close that outruns `PANE_CLOSE_SETTLE_TIMEOUT` no longer makes the `clear = true` delegate give up on the role (issue #1114, the entry its title names).
- **Layer:** fast integration (as `/032`, with the close's pane-scoped cleanup deliberately stretched past the respawn's settle window).
- **Agent:** none — `cat` stand-ins, same reasoning as `/032`.
- **Asserts:** two preconditions that put this on the OTHER side of `/032`'s line — `close_agent` has already removed the pane's record, and the cleanup hold is still up, which is what `spawn_agent` refuses a fresh pane on. Then, with the hold kept up for `STARVED_CLOSE_TAIL` (8 s, against a 6 s settle window), that the worker pane still gets a live agent once the close finally lets go, receives the task pointer, and still routes in `pane_role_map`. Before the fix the recovery waited exactly 6 s, logged "attempting the fresh spawn anyway", and attempted a spawn that cannot succeed while the hold is up — so crossing that boundary was not a degraded outcome but issue #606's full one.
- **Verified load-bearing:** the pre-fix build fails at 28.2 s with `close still in flight = false` and the worker pane absent from `agent_records()`. Putting the recovery's deadline back to `PANE_CLOSE_SETTLE_TIMEOUT` — the one-line inverse of the fix — turns it red again while `/032` stays green.
- **Budget:** `STARVED_CLOSE_TAIL` has to EXCEED the 6 s constant or this is `/022` wearing a different fixture, and to stay under the recovery's own 30 s ceiling or a fixed build fails too. Eight seconds clears the first by two, and a `sleep` can only overshoot, so load moves it in the safe direction.
- **Does not assert:** that 30 s is ENOUGH for any particular machine — it is a wedged-daemon backstop rather than an estimate of a loaded close, which is the point of `PANE_CLOSE_RECREATE_TIMEOUT`'s doc; the natural rate at which a close outruns 6 s (measured in issue #1114 at 1 in 120 under 80x over-subscription, not re-derived here); `pane restart`'s CLI budget, which deliberately does not cover this leg (`hook::RESTART_ROLE_REPLY_TIMEOUT`).
- **Platform coverage:** mac+linux (unix-only, with the rest of this file).

##### orchestration/delegate/034 — A cleanup hold taken AFTER the respawn lifted the pane's record out still ends with the role live (issue #1114, Greptile P1 on PR #1119; de-flaked in issue #1148).
- **Layer:** fast integration (the real `handle_delegate_with_state` against daemon-owned PTYs, on the ORDINARY respawn leg — nothing is closing the pane when the delegate lands; no LLM and no `e2e` feature gate).
- **Agent:** none — a stand-in that ignores TERM and HUP and reads NOTHING (`trap '' TERM HUP; exec sleep 300`), because the respawn's own `AGENT_TERMINATE_GRACE` is what holds the gap open between lifting the record out and spawning the replacement, and only a worker that survives the whole grace makes that gap real. **Deliberately NOT `/022`'s `trap '' TERM; exec cat`, and issue #1148 is what that cost.** The respawn drops the PTY writer and master BEFORE it terminates (so PTY-blocked I/O unblocks first), which hands the old child EOF on stdin: a `cat` then exits **0** of its own accord, the trapped SIGTERM never mattering, and `terminate_child_with_grace_and_wait` returns after one 50 ms `try_wait` tick — or immediately, when the exit lands before its first tick. Measured: the gap was ~50 ms rather than ~3 s, and collapsed to **2.57 ms** whenever that sub-millisecond scheduling race went the other way, which a 5 ms poll steps straight over. `close_agent` is the mirror image — it terminates while still HOLDING the master — so `/022`'s `cat` is right for the close path and wrong for this one.
- **Asserts:** the precondition that the stand-in's traps are armed (the `UNKILLABLE-WORKER-ARMED` marker, issue #709's fix borrowed) and that NOTHING is closing the pane when the delegate lands — which is what makes this the ordinary leg rather than `/032`'s window. Then, with the hold taken at the observable edge of the gap (the pane's record having gone) and kept up past the grace, that the pane still ends with a LIVE replacement agent and the role still routes in `pane_role_map`. Also asserts, as a precondition, that the modelled `StopAgent`'s own `close_agent` FAILS — if it succeeded the respawn had removed nothing and the gap was never entered.
- **How the gap is established, which is the part issue #1148 rewrote:** the wait for the record to be lifted returns one of three verdicts rather than a bare boolean. `Entered` (no record) is the only one that proceeds. `Collapsed` — a record again, under an agent id that is NOT the one the delegate was aimed at — is a hard failure naming the successor, because it is proof the respawn ran the whole gap before the poll could look, and no retry can find a gap that no longer exists. `NeverStarted` — still on the delegated id when the budget expires — is the dispatch task never having reached the respawn. Its predecessor merged the last two and reported both as "the respawn never lifted the pane's record out", which was FALSE in every #1148 failure measured and cost that issue a bisect onto a nonsense culprit; `Collapsed` now says so in 0.09 s instead of burning 24 s first.
- **The window it is about, and why the sibling fixes do not cover it:** `StopAgent` reads its record and calls `hold_pane_for_cleanup` in two separate statements, so one that read a record just before the respawn removed it takes the hold just after. `/032`'s under-lock refusal cannot see that — the hold does not exist yet when it asks — and the create leg's own retry is never reached, because the FIRST respawn's `DuplicatePaneId` used to be returned straight to the caller. By then the worker is already terminated and its record gone, so the role is dead for the session.
- **Verified load-bearing:** removing only the `DuplicatePaneId if pane_held_for_cleanup` arm from the first respawn's match — a one-line inverse — fails it at 25.1 s with the worker pane absent from `agent_records()`; `/032` and `/033` stay green.
- **Budget:** `STARVED_STOPAGENT_HOLD` (5 s) must outlast the respawn's 3 s grace, or the refused spawn simply succeeds and the test proves nothing; and stay well under the recovery's 30 s ceiling, above which a fixed build fails too. A `sleep` can only overshoot, so load moves it in the safe direction. The wait for the gap to open is bounded by `common::daemon_task_start_budget`, not `child_boot_budget`: what it waits on is a freshly `tokio::spawn`ed dispatch task getting its turn, not a child producing its first byte (issue #1148 — same 8 s base and the same load scaling, so the rename changed no behaviour, but the old name is why a mis-sized wait went unquestioned).
- **Does not assert:** the task pointer reaching the replacement — the modelled `begin_pane_close` sweeps this pane's outstanding delegation on the way in, exactly as the real handler would, so delivery is not this scenario's to claim (`/032` and `/033` assert it on the paths where the close does not sweep it); the natural frequency of this interleave, which is narrower than either entry recorded on #1114 and is not measured here; that a `clear = true` respawn leaves a REAL agent any termination grace at all — it does not, for any agent that reads its PTY, and that is by design rather than a defect (issue #1148); the daemon-log line the refused spawn no longer produces.
- **Platform coverage:** mac+linux (unix-only — the stand-in is a POSIX shell script).

##### orchestration/delegate/035 — A `SessionStart` that arrives AFTER the readiness gate gave up submits the task pointer the worker was already holding, instead of being discarded (issue #1031).
- **Layer:** fast integration (the real `handle_delegate` → `dispatch_one_owned` → readiness gate → fallback write → the late-readiness submit recovery, against a daemon-owned PTY; no LLM and no `e2e` feature gate).
- **Agent:** none — a `claude`-named Python stand-in. The BASENAME is part of the fixture rather than decoration: fact S of the recovery's gate is the deck's own frozen launch identity for the pane, so `AgentType::from_command` has to resolve the role command, and a stub named anything the deck cannot resolve is refused a recovery however the arriving event identifies itself.
- **Asserts:** first the reproduction, deterministically — the 30 s `SESSION_START_WAIT_TIMEOUT` is crossed on a paused clock, the fallback writes the pointer, and the stand-in reports that it DROPPED the submit CR that followed it while keeping the payload, which is the "`❯ Read .dot-agent-deck/worker-task-…` sitting unsubmitted at the worker's own input" state issue #1031 measured twice in two minutes. Then that nothing submits it for a 2 s quiet window, so the next assertion is attributable to the event and not to a timer. Then that posting the genuine, identity-matching `SessionStart` that arrived 7.1 s and 11.1 s late in production makes the daemon submit — and that what is submitted is exactly the pointer the worker was holding, which is what tells a submit-only probe from a payload rewrite (a rewrite would report the pointer twice, issue #424 D5's `seedseed`).
- **Verified load-bearing:** replacing the arming condition with `false` — the one-line inverse of the fix — fails it at 8.4 s with `snapshot = "SWALLOW-STUB-READYRead .dot-agent-deck/worker-task-coder.md for your task.SWALLOW-STUB-SWALLOWED"`, i.e. the defect itself; `/036` stays green, which is what makes the two entries independent rather than one mechanism in two costumes.
- **Why the stand-in counts submits instead of watching a clock:** a time-based swallow window would make the test race the replacement interpreter's boot against the gate's expiry and lose it on a loaded box. Counting instead — submit #1 is swallowed, every later one is honoured — makes the reproduction exact and load-independent, and it models the measured mechanism directly ("Claude Code fires `SessionStart` early in its boot sequence, before its TUI input is ready to interpret `\r` as submit").
- **Why the clock is paused and then RESUMED:** the delegate path reads the bare `SESSION_START_WAIT_TIMEOUT` constant and takes no env override (only the scheduler's mirror of the gate does), so 30 s has to be crossed virtually — but the recovery's own interval is enforced by `prompt_delivery::AgentStartRearm` against `std::time::Instant`, which `tokio::time::pause` does not advance. A run that stayed paused would refuse the probe however far it were advanced and would report the defect it exists to catch.
- **Does not assert:** that the recovery is armed ONLY over an `Applied` write and never over an `Ambiguous` partial one (Greptile P1 on PR #1168) — forcing a genuine `Ambiguous` through `dispatch_one_owned` needs a writer fault seam only `src/state.rs`'s unit-level fixtures have, so that decision is enumerated exhaustively by `state::tests::late_readiness_recovery_is_armed_only_over_a_complete_written_pointer` instead, and `orchestration/delegate/031` pins the outcome semantics it reads; the arm where the fallback write's payload was DESTROYED rather than parked — a submit-only probe is deliberately a no-op there and the worker still ends at the silent-worker report (`/013`); real Claude boot timing, which the stand-in replaces on purpose; the six-fact gate's own refusals, which are unit-tested at their source in `src/prompt_delivery.rs`; the silent-worker report, pinned OFF here (`DOT_AGENT_DECK_DELEGATE_NO_EVENT_WINDOW_MS=0`, the e2e harness's own value) so the run also proves the recovery is not gated on that diagnostic's knob.
- **Platform coverage:** mac+linux (unix-only, with the rest of this file).

##### orchestration/delegate/036 — A delegate whose readiness gate was released by an OBSERVED `SessionStart` earns no late submit recovery from a later one (issue #1031, fact U).
- **Layer:** fast integration (as `/035`, on PRD #249's observed-readiness branch instead of its timeout fallback).
- **Agent:** none — the same `claude`-named stand-in, so the only difference from `/035` is WHEN the readiness event arrives.
- **Asserts:** the precondition that the observed-readiness write reached the stand-in and had its submit swallowed, then that a SECOND genuine `SessionStart` for the same agent produces no submit at all within a 2 s window. This is the double-submit guard: the gate's own release bound this delivery's hook generation, so the only thing a later start can be is a different conversation — a worker that consumed and submitted its pointer and then `/clear`ed must not be submitted into again.
- **Verified load-bearing:** inverting fact U — `rearm.note_payload_write(true)` in place of `note_payload_write(wait.generation.is_none())`, a one-line inverse — fails it at 2.3 s with `snapshot = "…SWALLOW-STUB-SWALLOWEDSWALLOW-STUB-SUBMITTED:Read .dot-agent-deck/worker-task-coder.md for your task."`, i.e. the second start earning a submit into a conversation the delivery already knew about. `/035` stays green on that same build, which is what makes the two entries independent. Note it is ALSO green with the whole fix disabled, by construction — a negative test of a mechanism that does not exist passes — so `/035`'s redness in that build is the other half of the pair.
...
- **Asserts:** on a project config whose `worker_response_timeout_minutes = 0` leaves BOTH delegation watches arming nothing at all, a delegate followed by the worker's `work-done` still submits the pointer into the orchestrator pane, carries no unsolicited label and no inlined report frame, and writes the fresh report to `.dot-agent-deck/work-done-coder.md`. This is the case that rules out inferring "never delegated" from `DelegationRetirement::Nothing`: doing so would have silently stopped reporting completions for every project that has turned the detector off.
...
##### orchestration/work-done/007 — An unsolicited report longer than the inline bound is cut in the feedback but saved in full to a file the feedback names, and no saved report overwrites another (issue #508).
- **Layer:** fast integration (real `handle_work_done` against daemon-owned PTYs; `cat` stand-ins, no LLM and no `e2e` feature gate).
- **Agent:** none (raw no-echo `cat` orchestrator observer plus a `cat` worker).
- **Asserts:** with an earlier delegation's report parked at `.dot-agent-deck/work-done-coder.md` and nothing delegated, the worker reports `work-done` twice, each time with a multi-line markdown report of ~7 KB whose opening and closing sentinels differ. For each report, in failure order: the feedback carries the unsolicited label, the `[UNTRUSTED-WORKER-REPORT:` frame and the report's opening (control: delivered at all); the closing sentinel is ABSENT from the feedback (control: it really was cut at the 4000-character bound); the feedback names an absolute path under the worker's `.dot-agent-deck/`, read out of the orchestrator's own scrollback rather than guessed from the naming scheme; and that file's first line is `[UNTRUSTED-WORKER-REPORT:`, its last line `:END-UNTRUSTED-WORKER-REPORT]`, and between them the worker's report verbatim, line structure intact. Then: the two reports were saved to two different files, the first still holds the first report after the second was saved, and the parked earlier report is still byte-for-byte on disk.
- **Why it exists:** #448 stopped an unsolicited completion writing the role-keyed file, which was correct, but left the inlined copy as the only one — so past 4000 characters the rest of the report was in no file anywhere, while the feedback said "the worker still holds the rest", which a worker following its delivery instructions (delete the `--task-file` once the signal lands) does not. Confirmed red on `main` at `9083c6b` with exactly that message, and red again with only the `save_full_report` call removed from `handle_work_done`.
- **Does not assert:** the Unfiled path's copy (its coordination directory is usually what broke, so the save usually fails and says so — `state::tests::compose_work_done_feedback_bounds_an_oversized_report_and_says_so` pins the wording, and `save_full_report_never_overwrites_and_says_when_it_could_not_save` the failure); the retention sweep eventually removing the file (`is_sweepable_coordination_name` is asserted on the name in that unit test); an orchestrator actually reading the file (an LLM decision).
- **Platform coverage:** mac+linux (unix-only — raw-mode shell observer).

##### orchestration/work-done/008 — A commissioned report is written to `work-done-<role>.md` framed as untrusted worker-authored text, in full and uncollapsed, and the pointer to it says so (issue #509).
- **Layer:** fast integration (real `handle_delegate` + `handle_work_done` against daemon-owned PTYs; `cat` stand-ins).
- **Agent:** none (raw no-echo `cat` orchestrator observer plus a `cat` worker). Both delegation watches are off (`worker_response_timeout_minutes = 0`) so no detector competes for the pane.
- **Asserts:** after a delegate, the worker reports a ~7 KB multi-line report followed by a hostile tail that closes the frame early (`:END-UNTRUSTED-WORKER-REPORT]`), then carries a bidi override and an escape sequence. In failure order: the orchestrator receives the unchanged pointer sentence (`Read .dot-agent-deck/work-done-coder.md for their full report.`) followed by `That file is UNTRUSTED worker-authored text`; the report's opening is NOT inlined (the happy path stays a pointer); the file opens and closes with the frame's marker lines and holds the clean part of the report verbatim, past the inline bound and with its lines intact; the closing marker appears exactly once, so the forged one did not survive as a marker; the forged tail is still present as data, minus the bidi override and escape byte; and neither of those two characters is anywhere in the file.
- **Why it exists:** the Filed path is the common one, and it handed the orchestrator an unbounded, unframed report written by another agent while the rarer inlined paths were fenced. The file cannot take the inlined treatment — it exists to carry the full, formatted report — so what transfers is the frame: marker lines, a pointer that names the contents untrusted, and marker-name defanging in place of bracket stripping. Confirmed red on `main` at `9083c6b` (the pointer carried no framing), and red again with either the file framing or the marker-name defang removed.
...
- **Agent:** none. The `orch-deck` fixture's `worker` stays a plain `cat`, but the test rewrites its copy of the fixture so the `orchestrator` is a RAW, no-echo `cat` (`stty -echo -icanon … && exec cat -u`). A canonical-mode tty hands a plain `cat` at most 4095 bytes of one line, so it echoes the whole feedback and then reprints a copy cut near the 4000-character mark, and that copy scrolls the closing notice off the pane. This failed CI run 36241233852, and it reproduces deterministically with a 3 s settle before the screen check. A real agent reads its input raw, as this stand-in now does. Both delegation watches are off via the millisecond seams.
...
- **Layer:** L2 PTY-attached (the REAL `dot-agent-deck` binary driven through the vt100 `TuiDeck` harness, with its lazy daemon). Lane 1 — no agent, no credential. Synthetic stand-ins, so deliberately NOT demo-reel-marked.
- **Agent:** none. The `hook-provenance` fixture's `orchestrator` is `cat`; its `worker` is a shell loop that runs the branch's own `dot-agent-deck work-done` binary (passed in as `$DAD_TEST_BIN` so an installed release on `PATH` cannot stand in for it) twice, each time gated on a trigger file the test creates. That is the one property that matters here: the worker's command is spawned BY THE DAEMON, so it inherits the per-spawn hook capability token exactly as a real agent does.
- **Asserts:** three steps in a fixed order, because the assertion is an ABSENCE. (1) The worker's first own `work-done` reaches the orchestrator's PTY — the legitimate path is live and the daemon's role maps are populated. (2) The real `work-done` CLI is run from the TEST process carrying only the worker's `DOT_AGENT_DECK_PANE_ID` and the hook socket path, which is issue #1077's forgery exactly; it exits NON-ZERO and its stderr names the daemon's reason, because since issue #1129 the daemon acknowledges this verb at the provenance gate (this assertion is the inverse of the one that stood here, which recorded the silence as the honest cost of the refusal). (3) The worker's second own `work-done` reaches the orchestrator's PTY — a LATER message has completed the whole round trip on the same path. Only then: the forged report's sentinel is absent from the orchestrator's PTY. The deck does NOT use `impersonating_pane_signals()`, so this runs under the shipped `enforce` policy.
- **Why the order:** the first cut of this test sent the forgery before anything had landed and passed with the gate disabled, because a forgery arriving before the role maps were populated is dropped as "unknown pane" — indistinguishable from a refusal. Step (1) removes that, and the test was re-run with `policy()` forced to `WarnOnly` to confirm it now fails (the forged report is written into the orchestrator's pane).
- **Does not assert:** the in-process decision matrix (unit-tested in `src/hook_provenance.rs`), the refusal reply a `delegate` caller receives on the connection, or the `SignalAck` a `work_done` receives (`src/daemon.rs`'s `hook_provenance_*` tests, over a real socket and registry; the CLI end of that exchange is `tests/hook_provenance.rs`); a same-uid process that reads the victim pane's `/proc/<pid>/environ` and presents the stolen token — which this mechanism does NOT stop, by design and in writing (`docs/develop/hook-provenance.md`); raw `AgentEvent` traffic, which the gate deliberately does not cover (issues #543 / #401).
...
- **Asserts:** after the spawn-time remit pointer (`Read .dot-agent-deck/orchestrator-context.md`) delivers once (confirmed via the log), injecting a synthetic `Compacting` `AgentEvent` for the SAME start-role pane/agent identity — confirmed applied via the daemon's own `ListAgents` live-status join before proceeding — pushes the log one pointer line past the SETTLED count the spawn-time delivery left behind, within 30s. The count is relative rather than a literal `2` because an unconfirmed spawn-time delivery earns one automatic replacement payload write (`MAX_PAYLOAD_SUBMISSIONS`, `src/prompt_delivery.rs`), which the fixture logs as an extra pointer line with no re-assertion behind it (issues #818/#832).
...
- **Asserts:** after the spawn-time remit pointer delivers once, injecting `Compacting` for the non-start `worker` role's pane/agent identity does not push the start role's delivery log past its settled `Read .dot-agent-deck/orchestrator-context.md` count within a 900ms bounded wait; injecting `Compacting` immediately afterward for the orchestrator START role's own identity, in the SAME orchestration, DOES push it one line past that count within 30s — the positive control that makes the negative check meaningful rather than a vacuous pass against an unimplemented feature.
...
- **Asserts:** with the start-role pane confirmed history-only, injecting `Compacting` for its identity does not push the delivery log past its settled count within a 900ms bounded wait, and the rendered grid surfaces `History-only session cannot accept live input` within 5s; once the SAME pane subsequently reports itself live again, the log rises one `Read .dot-agent-deck/orchestrator-context.md` line past that count within 30s — proving the re-assertion is gated on confirmed delivery rather than a direct, unconfirmed pane write. Deliberately asserts only on the rendered grid and the delivery-log line count — both pre-existing, stable observables — never on an internal helper or `SendResult` variant, so this test's correctness does not depend on internal refactoring of the delivery-confirmation machinery.
...
- **Layer:** L2 (real-binary PTY via the vt100 `TuiDeck` harness; the daemon's own `AppState::apply_event` and `deliver_orchestrator_prompt` render-loop path handle the injected event and the redelivery for real).
- **Agent:** none (`remit-reassert-orchestration` fixture, as `001`).
- **Asserts:** after the spawn-time remit pointer (`Read .dot-agent-deck/orchestrator-context.md`) delivers once (confirmed via the log), injecting a synthetic `SessionStart` `AgentEvent` stamped `AgentType::ClaudeCode` and carrying the `dot_agent_deck::event::CLEAR_SESSION_START_METADATA_KEY`/`_VALUE` marker for the SAME start-role pane/agent identity — confirmed applied via the daemon's own `ListAgents` live-status join, by first driving the pane onto a uniquely-named active tool and then waiting for that tool to be CLEARED (the `SessionStart`'s own footprint; `Idle` on its own is a value the event happens to produce rather than evidence it landed, and is also where a quiet pane already sits) — pushes the log one pointer line past the SETTLED count the spawn-time delivery left behind, within 30s; the re-assertion's own delivery is then allowed to settle and a bounded 900ms wait confirms the count STAYS there rather than rising again. Both counts are relative rather than literal `2`/`3` for the replacement-payload reason recorded under `001`, and neither settle will accept a count more than one delivery's worth of payload writes above what it started from. Exactly-once is additionally pinned INDEPENDENTLY of the line count, by sampling `.dot-agent-deck/orchestrator-context.md`'s modification time from before the trigger until after the hold and requiring exactly ONE rewrite: only a logical re-arm rewrites that file (`reassert_orchestrator_prompt`), while a replacement payload write re-sends the already-prepared string and never touches it — so the two cases a line count cannot separate (one retried delivery versus two confirmed ones, both `+2` lines) are separated here.
...
- **Layer:** L2 (real-binary PTY via the vt100 `TuiDeck` harness).
- **Agent:** none (`remit-reassert-orchestration` fixture, as `001`).
- **Asserts:** after the spawn-time remit pointer delivers once, injecting the `/clear`-originated `SessionStart` marker (stamped `AgentType::ClaudeCode`) for the non-start `worker` role's pane/agent identity — confirmed applied by the same prime-a-tool-then-wait-for-it-cleared barrier `004` uses, which is what keeps this barrier meaningful against a `cat` worker stub that is doing nothing — does not push the start role's delivery log past its settled `Read .dot-agent-deck/orchestrator-context.md` count within a 900ms bounded wait; injecting the same marker immediately afterward for the orchestrator START role's own identity, in the SAME orchestration, DOES push it one line past that count within 30s — the positive control that makes the negative check meaningful rather than a vacuous pass against an unimplemented feature.
...
- **Layer:** L2 (real-binary PTY via the vt100 `TuiDeck` harness).
- **Agent:** none (`remit-reassert-orchestration` fixture, as `001`).
- **Asserts:** after the spawn-time remit pointer delivers once, injecting the `/clear`-originated `SessionStart` marker stamped `AgentType::Codex` for the orchestrator START role's own pane/agent identity — confirmed applied by the same barrier `004`/`005` use — does not push the delivery log past its settled count within a 900ms bounded wait. Deliberately negative-only, unlike `002`/`005`'s same-guard-different-axis pattern: chasing this check with a same-pane positive-control injection (a second `SessionStart`, ClaudeCode-tagged, on the SAME pane) would ride `pane_hook_session` (`src/state.rs`) — the bookkeeping `delivery_target_changed` (`src/ui.rs`) compares against — into reading the pane as stale after two hops — an artifact of sequencing two `SessionStart`s onto one pane, a shape no real pane (whose `agent_type` is fixed for life) ever produces. The positive-control role is instead filled independently by `004` (a genuine single-hop ClaudeCode-tagged injection on this same pane shape), the same relationship `001`'s positive proof bears to `002`'s negative check on a different pane, applied here across the agent-type axis instead of the pane-identity axis.
...
- **Layer:** L2 (real-binary PTY via the vt100 `TuiDeck` harness).
- **Agent:** none (`remit-reassert-orchestration` fixture, as `001`).
- **Asserts:** after the spawn-time remit pointer delivers once, a `## Your task` section carrying a sentinel is seeded onto `.dot-agent-deck/orchestrator-context.md` — reproducing byte-for-byte the shape `prepare_orchestrator_prompt(config, cwd, Some(task))` leaves on disk for a `dispatch --task` orchestration (`src/spawn.rs`) — without launching a second, separately-dispatched fixture. Injecting `Compacting` for the start role's own identity then causes the delivery log to show the task-carrying pointer (containing "Then carry out that task") within 30s, and the context file on disk still contains the sentinel afterward. Regression coverage for the maintainer review on the fork's upstream PR #789 ("Required 1"): before the fix, the re-arm called `prepare_orchestrator_prompt(config, cwd, None)` directly, which wiped the `## Your task` section and delivered the no-task pointer instead, silently discarding a dispatched orchestration's task on every compaction.
...
- **Layer:** L1 (pure-data `compute_frame_layout` geometry + `key_action_for_mode`, the public L1 seam over the same mode-aware resolver chain the event loop runs; no PTY, no TestBackend render). Lives in `src/ui.rs`'s own `#[cfg(test)]` module because `compute_frame_layout` and `ActiveTabView` are module-private. Note the seam applies each scoping pass's MODE term but is deliberately tab-agnostic — it answers for the most permissive tab, and the tab half is applied at the live call site. Both halves are covered by `orchestration/layout/005`, which also pins what this chord does in the OTHER mode.
- **Agent:** none.
...
##### orchestration/dispatch/005 — A DISPATCHED orchestration's run-identifying tab label survives a detach/reattach instead of reverting to the bare config name (issue #960).
- **Layer:** L1/synthetic (warm in-process daemon + real attach socket; no PTY-attached binary and no LLM). Starts at the REAL producer — `crate::spawn::spawn`, the primitive `dot-agent-deck dispatch`, the scheduler and issue-dispatch all call — and drives the production reattach chain end to end from there: the role loop's `TabMembership` lands on the daemon's `AgentRecord` → `EmbeddedPaneController::hydrate_from_daemon` reads it back through `ListAgents` + `validate_tab_membership` → `partition_hydrated_panes` → `resolve_orch_config_for_hydration` → `TabManager::open_orchestration_tab_with_existing_role_panes`. Synthetic is the right tier because the claim is about a tab LABEL round trip, not about anything an agent does.
- **Agent:** none (six `sh -c 'sleep 30'` stand-ins across three two-role orchestrations).
- **Asserts:** three orchestrations in ONE reattach, because the interesting claim is a comparison. A **dispatch** into a worktree-shaped cwd (`dispatch-team` in `.../issue-960`) publishes `dispatch-team · issue-960` on its live `OrchestrationSurface`, EVERY one of its role panes carries that same title back through the daemon echo, and the rebuilt tab's strip label is that title rather than the bare config name. A **`Ctrl+n`-shaped control** (the membership `tab.rs` stamps, title included) reattaches under `My Custom Run` — it passed before the fix and must keep passing, so a future regression names WHICH producer broke. A **dispatch with no per-run identity** (cwd basename equal to the orchestration name) surfaces no title at all and correctly reattaches under its canonical `bare-team`, which is the leg that fails if the producer ever stamps `Some("")` — read as "no title" by both the hydration fallback and `validate_tab_membership` — instead of `None`.
- **Why it exists:** `surface_spawned_orchestration` computed the title and put it on the transient broadcast ALONE while the role loop stamped `display_title: None`. Nothing persists a broadcast, so the live tab read `mixed · issue-950` and the reattached one read bare `mixed` — and with N concurrent dispatches every tab reattaches under the same label, which is the exact confusion the suffix was added to prevent. It went unnoticed because no test drove a PRODUCER: `tab.rs`'s `orchestration_010_reattach_preserves_user_title` hand-feeds a title straight into the tab-open call, and every orchestration fixture in `tests/rehydration.rs` used `display_title: None`, so the field was never observed surviving anything. Verified load-bearing: with the producer half reverted the tab-label assertion fails `left: "dispatch-team", right: "dispatch-team · issue-960"` while the control still passes — the issue's own reported shape.
- **Does not assert:** how the label PAINTS once the tab strip runs out of room (`tabs/orchestration/013` covers the truncation, which is where the name-first ordering earns itself); the title on the LIVE tab as painted by `surface_one_orchestration` (asserted at its source, the broadcast field that function reads, since the live and reattached labels now come from one value); the secondary `PaneRecreateIdentity` path (`orchestration/delegate/022` covers the re-created worker's membership); the local-config branch of `resolve_orch_config_for_hydration` (the title argument is independent of it, and the synthesised branch is the harsher case for the fallback); routing, delegation, PTY attach or scrollback replay.
...
##### dispatch/return/001 — A dispatched orchestration's terminal completion returns to the pane that dispatched it as a submitted turn (PRD #220 M2.0/M2.1).
- **Layer:** L2 synthetic, lane 1 (`TuiDeck` on the `orch-deck` fixture) driving the REAL `dot-agent-deck dispatch --orchestration=demo-orch` and `dot-agent-deck work-done --done` CLIs through the deck's hook socket. The dispatch creates a real sibling git worktree and the fixture's two real role PTYs; there is no real agent and no credential.
- **Agent:** none. The dispatched orchestration's `orchestrator` and `worker` commands are `cat`. The caller runs a narrow Python terminal probe: raw/no-echo stdin, a readiness marker, and one labelled output line per terminator (`SUBMITTED:` for CR, `NOTICE:` for LF), so the rendered result proves the daemon submitted a turn rather than merely making bytes visible. This is the one property plain `cat` cannot distinguish.
- **Asserts:** in failure order, the caller probe registers and prints its readiness marker; the dispatch CLI exits successfully; the existing acknowledgement appears in the caller's rendered grid as a `SUBMITTED:dispatch:` line carrying `spawned isolated` and the quoted unit name (the control that proves guarded caller delivery and the probe itself work); the dispatched worktree's start-role pane is registered with orchestration membership; its real `work-done --done` CLI exits successfully; and one rendered completion turn in the CALLER begins `SUBMITTED:dispatch:`, carries the whole word `completed`, fences the exact unit as `[UNTRUSTED-ROLE-LABEL: orch-return-probe :END-UNTRUSTED-ROLE-LABEL]`, and fences the distinctive report as `[UNTRUSTED-WORKER-REPORT: orchestration-return-report-7f31 :END-UNTRUSTED-WORKER-REPORT]`. Because the long turn wraps, the helper isolates only the right-hand pane cells from that submitted opening through the worker-report closing marker, then applies the harness's wrap-insensitive search normalization to that bounded region; it never concatenates the whole grid.
- **Why it exists:** before Phase 2, dispatch captures the caller's `(pane_id, agent_id)` only long enough to deliver the spawn acknowledgement, then discards it. A terminal `--done` from the dispatched orchestration's own start role reaches `handle_work_done`, logs `orchestration complete (orchestrator --done)`, and returns with no consequence. The caller therefore starts a unit successfully and never hears from it again. This test begins at both real CLI boundaries and ends on the rendered caller conversation, so a callback that is merely stored, merely logged, sent to the unit's own tab, or written as a passive notice cannot satisfy it.
- **Does not assert:** callback refusal after caller identity hand-over, pane-gone drop-and-log, retry policy, or a durable outcome store; the exact completion sentence beyond its four stable components; real-agent compliance with the `work-done` instruction; ordinary worker-to-orchestrator feedback (`orchestration/work-done/004` and `orchestration/route/001` own that non-regression).
- **Platform coverage:** mac+linux (PTY-attached lane 1; POSIX-style PATH and the harness's local socket transport).

##### dispatch/return/002 — The dispatch return callback survives detaching and reattaching the caller pane before the unit completes (PRD #220 M2.1).
- **Layer:** L2 synthetic, lane 1 (`TuiDeck` on `orch-deck`) using the same real dispatch/work-done CLI and real daemon-owned role PTYs as `dispatch/return/001`, plus the real TUI's two-sided Ctrl+D transition: detach the caller from `TYPING` into dashboard `COMMAND` mode, then return to that pane's `TYPING` mode, between those two commands. No LLM, network or credential.
- **Agent:** none (`cat` for both dispatched roles; the caller is the same raw terminal probe as `/001`, which labels CR delivery `SUBMITTED:` and LF delivery `NOTICE:`).
- **Asserts:** in failure order, the caller and dispatch preconditions from `/001`; the dispatched orchestration's start role exists before any UI transition; the first Ctrl+D visibly reaches dashboard `COMMAND` mode with the `caller` card selected and that caller's submitted acknowledgement in the split-layout preview; the second Ctrl+D visibly restores `TYPING`, removes `COMMAND`, and keeps that same acknowledgement visible; the dispatched orchestrator's terminal work-done CLI succeeds; and the caller's reattached grid gains one wrapped turn beginning `SUBMITTED:dispatch:`, carrying whole word `completed`, the exact unit inside its `UNTRUSTED-ROLE-LABEL` frame, and unique report `reattach-return-report-82ac` inside its `UNTRUSTED-WORKER-REPORT` frame. It uses `/001`'s bounded right-pane-region normalization.
- **Why it exists:** the PRD names callback loss across detach/reattach as the risk that leaves a dispatcher waiting forever. Pane PTYs and their scrollback already live in the daemon, so a correct callback belongs beside that state and survives a TUI leaving and returning to the pane. The acknowledgement-before/`COMMAND`/`TYPING` sequence is load-bearing: it proves both halves of the real Ctrl+D state transition and identifies the selected pane on each side, rather than treating an unchanged mode or a different tab as a persistence test. Split layout deliberately retains the selected pane's preview while detached, so disappearance would be the wrong precondition.
- **Does not assert:** killing and relaunching the entire TUI client, daemon restart (not promised), caller identity hand-over, pane-gone handling, or a durable result after the pane closes; completion while the caller is still detached (the contract is retention across the cycle, and this scenario completes after the original pane is visibly reattached).
- **Platform coverage:** mac+linux (PTY-attached lane 1; POSIX-style PATH and the harness's local socket transport).

##### dispatch/return/003 — A dispatched `--single` unit can complete back to its caller, while an unrelated pane's work-done remains on the unknown-pane path (PRD #220 M2.3).
- **Layer:** L2 synthetic, lane 1 (`TuiDeck` on `minimal`) driving the REAL `dot-agent-deck dispatch --single` CLI, then two REAL `work-done --done` CLI calls through the hook socket: first from the unrelated caller control and then from the dispatched unit. The daemon creates the real sibling worktree and PTY; all child behaviour is deterministic and credential-free.
- **Agent:** none. Both caller and single unit use the raw terminal probe from `/001`: it stays alive like `cat`, prints an explicit readiness marker, and labels a CR-terminated delivery as `SUBMITTED:` so the test distinguishes a turn from a notice without a real agent.
- **Asserts:** in failure order, caller readiness, successful single dispatch, submitted acknowledgement in the caller, and a dispatched `dispatch-<name>` record rooted in the expected worktree whose own probe is ready; a `work-done --done` from the caller (neither a role nor a dispatched unit) exits successfully but produces the existing `work-done from unknown pane` warning naming that pane; the single unit's work-done CLI exits successfully; the caller renders one wrapped turn beginning `SUBMITTED:dispatch:`, with whole word `completed`, the exact unit inside its `UNTRUSTED-ROLE-LABEL` frame, and unique report `single-return-report-4d92` inside its `UNTRUSTED-WORKER-REPORT` frame; and, only after that positive return barriers daemon ingestion, no pane snapshot contains the control's unique `unknown-pane-report-93e1-must-not-route` text. It uses `/001`'s bounded right-pane-region normalization.
- **Why it exists:** a single dispatch has no orchestration membership and today never enters any useful work-done branch: `handle_work_done` requires `pane_role_map`, which only orchestration-role spawn populates, so it warns `work-done from unknown pane` and returns. Phase 2 must give the dispatched single pane enough daemon-owned identity to resolve its retained callback without weakening that admission gate for arbitrary panes. The adjacent unknown control pins both sides of that distinction and its absence is barriered by the successful return, not by a sleep or an early empty read.
- **Does not assert:** how a real single agent learns to invoke `work-done --done`, real-agent execution, callback refusal after caller identity hand-over, pane-gone handling, exact completion prose, or the ordinary non-dispatched orchestration routes already owned by `orchestration/work-done/*` and `orchestration/route/001`.
- **Platform coverage:** mac+linux (PTY-attached lane 1; POSIX-style PATH and the harness's local socket transport).

##### dispatch/return/004 — A dispatched unit's completion is refused, with no bytes written, when the caller pane has changed hands before the unit completes (PRD #220 M2.1 / issue #617).
- **Layer:** fast integration (the real `AppState::handle_work_done` completion path and `daemon::deliver_dispatch_result` identity gate against daemon-owned PTYs; no daemon process, git, LLM, credential or `e2e` feature gate).
- **Agent:** none — the dispatched unit, original caller and successor are raw no-echo `cat` observers. The successor deliberately inherits the caller's `DOT_AGENT_DECK_PANE_ID` while receiving a new registry agent id; readiness markers prove each stub reached raw mode before the test relies on its scrollback.
- **Asserts:** in failure order, the caller and unit observers are ready; registering the unit retains exactly one return edge; closing the original caller without the deliberate-close sweep and spawning the successor produces a different registry agent id on the same pane; terminal `handle_work_done` consumes the return edge even though delivery is refused; an authorized `GuardedSend::Applied` barrier reaches the successor; after that barrier the successor's snapshot contains none of the fenced completion turn — neither `dispatch:`, the distinctive unit that would occupy `UNTRUSTED-ROLE-LABEL`, nor the distinctive report that would occupy `UNTRUSTED-WORKER-REPORT`; and the real warning names the caller pane, `WrongSession`, the identity-gate refusal, and `nothing written`.
- **Why it exists:** the acknowledgement's identity race is already pinned by `orchestration/dispatch/006`, but the completion sits behind a different path and a much wider race window — the dispatched unit's entire lifetime. A pane id is reusable, so delivery keyed only by that string would submit the old unit's report as a turn to an unrelated successor that may act on it. The test deliberately uses `close_agent` rather than `begin_pane_close` so the retained edge survives until completion and the guarded delivery itself is the only protection under test; bypassing that identity binding leaks the full report into the barriered successor snapshot and makes the absence assertion fail.
- **Does not assert:** dispatch spawning or callback registration through `handle_dispatch` (`dispatch/return/001-003` own the real CLI boundary), deliberate-close eviction (`dispatch/return/005`), the `Ambiguous` partial-write branch, retry beyond asserting the entry is consumed, or real-agent behavior.
- **Platform coverage:** mac+linux (unix-only fast test — every observer is a POSIX-shell raw `cat`).

##### dispatch/return/005 — Closing a dispatch caller evicts its retained return edge, so a later unit completion is dropped and logged without reaching any alternate pane (PRD #220 M2.2).
- **Layer:** fast integration (the real `AgentPtyRegistry::begin_pane_close` / `finish_pane_close` lifecycle and `AppState::handle_work_done` against daemon-owned PTYs; no daemon process, git, LLM, credential or `e2e` feature gate).
- **Agent:** none — raw no-echo `cat` observers stand in for the still-live dispatched unit, the caller that is deliberately closed, and an unrelated alternate recipient. Distinct readiness markers prove all relevant PTYs are live before the close and completion.
- **Asserts:** in failure order, caller, unit and alternate observers are ready; registering the unit retains exactly one return edge; `begin_pane_close` on the CALLER immediately reduces the retained count to zero before the child is closed; the full close lifecycle completes; the unit's terminal `handle_work_done` returns without panic and does not recreate or reroute an edge; an authorized `GuardedSend::Applied` barrier reaches the unrelated pane; its drained snapshot contains none of the fenced completion turn — neither `dispatch:`, the unit name that would occupy `UNTRUSTED-ROLE-LABEL`, nor the report that would occupy `UNTRUSTED-WORKER-REPORT`; the close log says it dropped dispatch-return entries touching the caller pane; and the late completion is logged under its unit pane by the existing `work-done from unknown pane` path.
- **Why it exists:** once the caller is gone there is neither a valid recipient nor a promised durable outbox, so keeping its callback would leak daemon memory and leave a pane-reuse hazard waiting for a future occupant. The close sweep must cover entries that point TO the caller as well as entries keyed BY a closing unit. Temporarily skipping `evict_pane` leaves the observable count at one and makes the first post-close assertion fail, proving the test does not pass merely because the eventual guarded write would also refuse a missing pane.
- **Does not assert:** pane-id hand-over refusal (`dispatch/return/004`), unit-pane close eviction (covered by the registry unit tests), daemon restart or durable outcome storage, exact completion-report wording, or the TUI close dialog.
- **Platform coverage:** mac+linux (unix-only fast test — every observer is a POSIX-shell raw `cat`).

##### dispatch/return/006 — A real dispatched single agent independently signals completion and its discovered sentinel report lands visibly in the real dispatcher's pane (PRD #220 M2.3/M4.2). [reel]
- **Layer:** L2 PTY-attached, lane 2 (`TuiDeck` on `minimal`) driving the REAL new-pane dispatcher flow and two REAL interactive Claude Code panes through the branch binary. The dispatcher itself invokes `dot-agent-deck dispatch --single`; no test process invokes either `dispatch` or `work-done`. The freshly-built binary's directory is prepended to the PATH inherited by the deck, daemon and agents, and the fixture plus predicted sibling worktree are trusted in the isolated test HOME before either prompt is delivered.
- **Agent:** REAL interactive Claude Code Haiku (`claude-haiku-4-5-20251001`) for both the dispatcher and the dispatched unit, with `--allowedTools Bash Read Write` so the dispatcher command, checkout inspection, report-file write and cleanup cannot stop on a permission prompt. Flaky-tolerant credentialed lane 2: run once with `DOT_AGENT_DECK_REQUIRE_REAL_E2E=1`, never in CI. The caller is not a stand-in: it is the user-opened, seed-instructed dispatcher pane whose conversation receives the result.
- **Fixture sentinel:** the committed `minimal` checkout gains `dispatch-return-live-sentinel-6f2c.txt`, while the unit task names only the prefix `dispatch-return-live-sentinel-`. The returned full filename therefore comes from the unit inspecting its isolated worktree, not from echoing a filename supplied in either agent's prompt.
- **Asserts:** a single visible `ClaudeCode` dashboard card becomes a real dispatcher by submitting the distinctive dispatcher seed; after the user directs that agent to run one exact `dispatch --single` command, the dispatcher's rendered grid gains the fenced completion opening `dispatch: a unit you dispatched has completed`, carries exact unit `live-return-probe` inside `[UNTRUSTED-ROLE-LABEL: … :END-UNTRUSTED-ROLE-LABEL]`, and carries the unit-discovered full sentinel inside `[UNTRUSTED-WORKER-REPORT: … :END-UNTRUSTED-WORKER-REPORT]`. The grid predicate is wrap-insensitive for the opening and both frames while separately preserving the raw `dispatch:` prefix check, and it uses an explicit 240s `common::wait_until` bound instead of `deck.wait_until_grid`'s silent 10s cap.
- **Why it exists:** `dispatch/return/001-005` prove daemon-side retention, routing, identity refusal and eviction by manually invoking `work-done` from stubs, but none proves a fresh single agent can read the prompt composed by `dispatch_prompt`, do real work, create a usable report and choose to signal `work-done --done` on its own. This is the real user path required by rule 4 and closes the PRD's M4.2 gap: real dispatcher -> real dispatch -> real isolated unit work -> agent-originated completion -> visible dispatcher turn.
- **Does not assert:** orchestration-shaped return (covered synthetically by `dispatch/return/001`), detach/reattach retention (`/002`), arbitrary-pane admission (`/003`), identity hand-over refusal (`/004`), caller-close eviction (`/005`), the agent's prose beyond the uniquely named sentinel, or durable delivery after the dispatcher pane closes.
- **Platform coverage:** mac+linux (real-agent lane 2 is local-only; the test uses the Unix PTY/socket harness and POSIX PATH semantics).
- **Cost note:** two short interactive Haiku turns — one dispatcher command and one read-only filename discovery plus completion report — deliberately kept on the cheapest Claude tier.

##### dispatch/return/007 — A dispatched unit's report longer than the inline bound is cut in the completion turn but saved in full to a file the turn names (issue #508).
- **Layer:** fast integration (the real `AppState::handle_work_done` completion path and `daemon::deliver_dispatch_result` against daemon-owned PTYs; no daemon process, git, LLM, credential or `e2e` feature gate).
- **Agent:** none — the caller and the dispatched unit are raw no-echo `cat` observers sharing one temp directory, which stands in for the unit's worktree.
- **Asserts:** with the unit's return edge retained, a terminal completion carrying a ~7 KB multi-line report reaches the caller as a turn with the `dispatch: a unit you dispatched has completed` opening, the `[UNTRUSTED-WORKER-REPORT:` frame and the report's opening sentinel (control: delivered at all); the closing sentinel is ABSENT (control: really cut at 4000 characters); the turn names an absolute path under the unit's `.dot-agent-deck/`, read out of the caller's scrollback; and that file holds the whole report verbatim between `[UNTRUSTED-WORKER-REPORT:` and `:END-UNTRUSTED-WORKER-REPORT]` marker lines.
...
- **Asserts:** spawning three orchestration role agents (orchestrator + coder + reviewer), each tagged with its `TabMembership::Orchestration` `role_index` / `role_name` / `is_start_role` / `display_title`, then hydrating a fresh controller from the warm daemon reproduces every role as a pane; placing each hydrated pane at its `role_index` yields the panes in their saved display order; the start (orchestrator) role — the `start_role_index` cursor — is recoverable from `is_start_role`; and EVERY pane brings the tab's `display_title` (`TDD Cycle · run-42`, deliberately distinct from the orchestration name so a fallback cannot read as a pass) back with it. Regression guard that warm-daemon orchestration hydration (PRD #76 M2.12 + #111) survives detach/reattach so M2b.3's snapshot fallback is only needed when the daemon is empty. The title leg is issue #960's: this fixture used `display_title: None`, like every other orchestration fixture in the file, so the field was never observed surviving anything and a producer stamping `None` went unnoticed.
- **Does not assert:** the daemon-empty snapshot-fallback rebuild (`session/restore/008`); the orchestrator-prompt replay (intentionally NOT replayed on warm reconnect — `src/tab.rs` design decision 3); the full `OrchestrationConfig` re-resolution (the partition + `resolve_orch_config_for_hydration` path, exercised elsewhere); which PRODUCER stamped the `display_title` it round-trips — the membership is hand-built here, and `orchestration/dispatch/005` owns the producer question.
...
##### session/restore/016 — A real detach/reattach puts the user back on the tab they left, with each tab's own focused pane, instead of on the deck's landing default with every tab reset to its start role (issue #949).
- **Layer:** L2 (real-binary PTY; TWO decks in sequence against ONE external `daemon serve`, both with `DOT_AGENT_DECK_SESSION` redirected to a test-owned path that outlives the first deck — which is what models two attaches by the same user on one machine reading one `~/.config/dot-agent-deck/session.toml`).
- **Agent:** none (the three roles run `printf <sentinel>; sleep 600`; no LLM).
- **Asserts:** with the `orch-reattach-focus` fixture's 3-role orchestration open, focus walked two roles off the start role onto `scout`, and the deck then switched back to the Dashboard to supervise, a clean detach-quit followed by a FRESH deck against the same daemon and the same `session.toml` restores both halves of that position: the Dashboard is the highlighted tab (read from the tab strip's `Modifier::REVERSED` cells, since `render_tab_strip` adds no text marker) with no role pane expanded, and stepping Right onto the rebuilt orchestration tab expands `scout`'s pane rather than `lead`'s. Pre-fix RED on both halves — nothing persists focus and nothing carries it on attach, so the reattach lands on the orchestration tab (PRD #111's first-rebuilt-orchestration landing) with that tab's own start role focused by `restore_focus_on_switch_in`'s fallback. Also the local reproduction the issue asked for: it was reported against a remote daemon and the same code path runs for a local attach.
- **Does not assert:** that the daemon's scrollback replay repaints the restored pane's earlier output — measured absent from the rebuilt pane at reattach (the role's sentinel does not come back), which is a separate question from focus and is not made worse here; the Dashboard's own card selection, which is keyed by session id and re-armed per frame by `reconcile_dashboard_selection`, so nothing about it is persisted (see `TabManager::apply_focus_snapshot`); the locator resolving across SEVERAL rebuilt tabs, or a rebuild order that differs from the open order (`session/restore/017`); the degradation cases, including the daemon-EMPTY rebuild path, which deliberately restores only the id-free half (`session/restore/018` case (d)); the TOML schema in isolation (`config/saved-session/002`). Note the restored Dashboard legitimately reads `0/3 session(s)` here — the Dashboard filters out panes belonging to a mode/orchestration tab, so an empty overview is what the user was looking at when they detached and is faithfully what comes back.
- **Platform coverage:** mac+linux.

##### session/restore/017 — The remembered position round-trips by PANE ID across a rebuild that puts the tabs at different indices, and every tab's focus is restored, not just the active one (issue #949).
...
- **Layer:** L1 (in-process unit test; `src/tab.rs`, alongside `session/restore/017`).
- **Agent:** none.
- **Asserts:** (d) a position put through `SavedFocus::retain_pane_ids` with an EMPTY daemon-supplied set writes NO pane id even when one would resolve, while its id-free half (a remembered Dashboard) still applies — the filter every startup runs, because only a daemon-owned pane's id is an identity across processes: every pane on the daemon-EMPTY rebuild path, and a `Tab::Mode`'s locally-spawned side panes on the warm one, carry freshly allocated `allocate_id` counters that can name a DIFFERENT pane. (e) the SAME position with that id present in the set restores normally, so (d) pins the filter rather than something about the id. (a) a `SavedFocus` whose ids no tab owns — a pane the user closed, or a role whose agent died and came back as a synthetic dead-slot — names NO landing tab and writes nothing, so the orchestration tab still falls back to its start role, i.e. exactly today's behaviour rather than an error; (b) a dead-slot id sitting in a tab's `focused_role_pane_id` (reachable, since hydration puts synthetic ids in `role_pane_ids` and `record_focus` accepts any member of that list) is filtered out at CAPTURE, so it can never be written to disk and then shadow a live role; (c) a `dashboard_active` capture resolves to the Dashboard tab, located by KIND rather than by assuming index 0, while a fully defaulted `SavedFocus` resolves to `None` and leaves the deck's own landing choice alone.
- **Does not assert:** the successful round-trip (`session/restore/017`); the user-visible cycle (`session/restore/016`); that `src/ui.rs` builds the daemon-supplied set from `hydrate_from_daemon` and calls `retain_pane_ids` with it — out of L1 `TabManager` reach, and covered against the real binary by `session/restore/019` (empty set) and `session/restore/016` (populated set); that a `Tab::Mode` side pane is in fact absent from that set (structural — the hydration block spawns them AFTER the set is built from `hydrated`), nor the wrong-side-pane focus that filtering prevents; that a dead-slot id can never be written by an OLDER binary and read back by this one — the restore path drops it anyway, since `tab_index_owning_pane` rejects dead-slot ids, which is what (a) covers.
- **Platform coverage:** mac+linux+windows.

##### session/restore/019 — A cold start against an EMPTY daemon honours the remembered tab kind, ignores the remembered pane id, and comes up in command mode (issue #949).
- **Layer:** L2 (real-binary PTY; `DOT_AGENT_DECK_SESSION` redirected to a test-owned path carrying BOTH an orchestration snapshot and a `[focus]` table; daemon lazy-spawned and empty, which is the whole point — this is the one path where the restored panes are freshly allocated rather than adopted from a daemon).
...
##### session/live/007 — `DaemonClient::list_agents` scrubs and clamps a hostile `AgentRecord` at the wire boundary so a malformed daemon can't corrupt the rebuilt card (PRD #162 review-fix, parallels embed/attach/005; `display_name` added by issue #833).
...
- **Asserts (issue #833 extension):** the same record's `display_name` — the card TITLE `ui::render_card_grid` PREFERS, since hydration copies it into `ui.pane_display_names` and the dashboard loop fills `ui.display_names` from that — carries ESC, NUL, a `U+202E` right-to-left override and ~800 bytes of multi-byte padding, and comes back scrubbed of control bytes AND bidi overrides and clamped to `agent_pty::DISPLAY_NAME_MAX_LEN` on a character boundary. Rendering that returned string through `render_card_grid_to_buffer` — the seam that runs the deck's own `ui.display_names` → `SessionState.display_name` resolution — puts no control character or bidi override in any cell. Before the fix this was the one string on the record with no scrub at this boundary at all, and the daemon-side gate meant to keep it clean (`agent_pty::is_valid_display_name`) admitted the bidi override.
- **Does not assert:** the daemon-side join/serde (session/live/001–003); the seeding of the card's fields (session/live/004); the `agent_type` precedence fallback (session/live/008); the `tab_membership` scrub itself (embed/attach/005); the rename writer of `ui.display_names` (`dashboard/pane/012`); the exact sanitized output beyond "no raw control bytes or bidi overrides survive and the values are clamped".
- **Platform coverage:** mac+linux.
...
##### session/live/016 — A reconnected card's `Last:` readout says how long the agent has been quiet instead of resetting to `0s` (issue #804).
- **Layer:** L1 (`AppState::seed_hydrated_session` then `ui::render_card_to_buffer` into a ratatui buffer; no daemon, no PTY).
- **Agent:** none (hand-built `SessionSnapshot`s).
- **Asserts:** a card seeded from a snapshot whose `last_activity_ms` is an hour old renders ` Last: 1h  Tools: 14 ` in its bottom border, not the `0s` of a freshly minted placeholder; a snapshot without the field (an older daemon predating PRD #745 M9) and one whose value no `DateTime` can hold (`i64::MAX`) both keep the placeholder's seconds-form readout.
...
##### config/saved-session/002 — A `[focus]` table round-trips through TOML, a `session.toml` predating it loads as "nothing remembered", and a PARTIAL table does not take the saved panes down with it (issue #949).
- **Layer:** pure-data (in-crate `#[cfg(test)]` unit test on `config::SavedSession` / `SavedFocus`; no TUI harness, no I/O).
- **Agent:** none.
...
##### chain-smoke/pi/002 — A REAL `pi` WORKER receives a daemon-injected delegate (the agent-agnostic `worker-task-<role>.md` footer + guarded write-and-submit path), does the task (creates a uniquely-named sentinel), and signals `work-done` back — proving pi's SECOND role (PRD #201 completeness; the orchestrator role is pinned by `chain-smoke/pi/001` + `pi/live/002`).
...
- **Does not assert:** wheel-down direction (the same production route receives a direction parameter), real terminal mouse-report decoding (covered by the next scanner delegation), side-pane pointer hit-testing, which pointer positions reach the focused pane at all (this seam starts after that decision — covered by `mode/scroll/007`), real-agent behavior, or that any particular agent produces the synthetic repaint pattern.
- **Platform coverage:** mac+linux+windows.
...
##### mode/scroll/007 — A wheel event is routed by pointer position and never reaches a pane with a coordinate the pointer was not at.
...
##### scheduler/spawn/009 — A scheduled task's declared `shape` overrides what its `working_dir`'s config implies: `single` fires ONE card that actually runs the task's `command`, `orchestration:<name>` fires that named team over the declared default, and an undefined name abandons the fire through the notifier (issue #835).
- **Layer:** L2 (headless `dot-agent-deck daemon serve` driven via `RunNow`, same shape as `scheduler/spawn/002`).
- **Agent:** none (every spawnable role runs `cat`, which echoes the delivered prompt; the forced-single task runs `touch <marker>; exec cat`).
- **Asserts:** three tasks pointed at ONE dir defining `team` plus `other` (`default = true`) diverge only by their `shape` scalar — `shape = "single"` registers a NON-orchestration card, drops the `SINGLE_SHAPE_CMD_RAN` marker (so the task's own `command` genuinely ran, not merely a card opened), echoes the prompt, and leaves the orchestration-role count at **0**; `shape = "orchestration:team"` registers a `TabMembership::Orchestration` naming **`team`** rather than the `default = true` `other`, with the prompt echoed; `shape = "orchestration:nope"` surfaces a notification on daemon stderr naming **`nope`** and listing **`team`**, spawns **nothing** (agent count unchanged after a settle), and leaves the daemon alive.
- **Why it exists:** the shape used to be derivable ONLY from the target dir, so `resolved_target: None` sent every schedule through `decide_target` and any `working_dir` defining `[[orchestrations]]` fired a full team while silently ignoring the task's `command` — there was no way to say "one agent HERE", in the repo whose skills and git remote the task needs. The marker assertion is what separates this from `scheduler/spawn/002`: a single-agent card alone would not prove the ignored-`command` half of the defect is fixed. The unknown-name case pins the other half of the contract — an unresolvable shape must fire NOTHING, because degrading to a different shape is the same silent surprise with extra steps.
...
- **Agent:** none (run-now; observes per-issue worktrees + orchestrator count).
- **Asserts:** issues 1 and 2 are dispatched — worktree present **and** an orchestrator agent registered against it, waited for as two separate steps because they are two steps of the dispatch — issues 3–5 are left untouched (no worktrees), and exactly two orchestrator agents exist.
...
##### scheduler/idle-worker/013 — A work-done retires every generation outstanding for its worker pane, superseded ones included, so a worker that answered is never reported as silent — while a re-delegated worker that never answered at all still is, on the newer delegation's clock (issue #1080, reversing PRD #126 M1 review finding 6).
- **Layer:** fast integration (two `handle_delegate` calls against each of two worker panes on one clock, then a real `handle_work_done` call for one of them).
- **Agent:** none (`cat` stand-ins).
- **Asserts:** the worker that sent ONE completion after being delegated to twice produces NO idle prompt — under finding 6's oldest-first accounting that completion was spent on the superseded delegation one, leaving delegation two armed to fire later against work the same signal had just reported finished. The control worker, superseded identically but never answering, IS reported: once, and no earlier than delegation two's own deadline rather than the older delegation's — so the supersede still leaves exactly one live watch on the newest generation and the suppression is attributable to the completion rather than to the re-delegation. Exactly one prompt exists across all four delegations.
- **Was RED on the shipped behaviour and the product changed, not the test.** This spec asserted finding 6's opposite until issue #1080; the reversal is argued from `deck.log` on 2026-09-14, where PRD #742's `release` pane drifted permanently and its 07:13:18 completion left a record that reported it silent at 08:50:08. `AgentPtyRegistry::retire_outstanding_delegation`'s doc carries the trace and what the reversal gives up.
- **Does not assert:** the residual the reversal accepts — a worker re-delegated to BEFORE answering, whose completion now disarms the newer delegation's watch too, so a stall on that newer task goes unreported until the next delegate to the pane. Also not asserted: the `DelegationRetirement` variant returned to `handle_work_done` (observed only through the resulting prompt/no-prompt behavior; the variant and its `superseded_dropped` count are pinned by `agent_pty::retire_drops_the_whole_record_including_superseded_generations`).
- **Platform coverage:** mac+linux.
...
##### scheduler/idle-worker/018 — An AMBIGUOUS silence report KEEPS its payload record, because its `⚠` puts it on the side of issue #876's drain the daemon deliberately will not erase — so a byte-identical second report is refused rather than submitted on top of the leftover bytes.
- **Layer:** L1 (in-process production notice composition, the guarded submit path against two real `/bin/cat` PTY panes — one driven through a FAULTED writer so the `Ambiguous` outcome is produced by the production classification rather than supplied to it — and the production settle decision `settle_one_shot_payload_record` driven with each outcome that leaves a record).
- **Agent:** none.
- **Asserts:** the production `compose_delegate_silence_notice` text is delivered into an orchestrator pane through a writer that accepts the whole payload and then refuses the submit CR, so the submit is genuinely `Ambiguous`. The faulted writer HEALS, so issue #876's erases could land — and the assertion is that the daemon writes none: the byte log is the payload and nothing else. It declines because the notice opens with a `⚠`, so a partial write can cut a multi-byte character in half and "one byte written" stops meaning "one erase"; that precondition is asserted directly rather than assumed, so the test says red if the report ever becomes plain ASCII and silently changes branch. The bytes therefore really are stranded, the payload record survives, and once the user types a byte-identical second report is refused (`Stale`) instead of appending the leftovers to the user's unsent draft and submitting both as one turn. A second pane runs the identical sequence with `Applied` as a control and IS admitted, which is what makes the refusal a property of the outcome rather than of the harness. The complement of `scheduler/idle-worker/017`, which pins the `Applied` release itself, and the place where issue #876's accepted residual is asserted rather than described.
- **Does not assert:** the trigger that decides to send the report (`orchestration/delegate/013` and `/025`); the sibling one-shot callers, whose payloads fall on the DRAINED side of the same decision and are pinned by `scheduler/idle-worker/019` (`arm_idle_worker_watch`'s idle prompt) and `orchestration/delegate/031` (the delegate task pointer); that a real agent TUI honours the erases this payload does not get (`agent_pty`'s `erase_bytes_undo_exactly_our_write_and_leave_the_users_draft`); and the `DeliveryNotice` the daemon publishes alongside a stranded partial write.
- **Platform coverage:** mac+linux.

##### scheduler/idle-worker/019 — An AMBIGUOUS idle-worker prompt keeps its payload record only while its bytes are STILL IN the orchestrator's input box; erased back out, a byte-identical second prompt is admitted.
- **Layer:** L1 (in-process production prompt composition, the guarded submit path against three real `/bin/cat` PTY panes — two of them driven through a FAULTED writer so the `Ambiguous` outcome is produced by the production classification rather than supplied to it — and the production settle decision `settle_one_shot_payload_record` driven with each outcome that leaves a record).
- **Agent:** none.
- **Asserts:** the production `compose_idle_worker_prompt` text is delivered into three orchestrator panes. Two are genuinely `Ambiguous` — the whole payload accepted, the submit CR refused — differing only in whether the writer recovers in time for issue #876's erases. On the pane whose erases could not land the prompt bytes are still in the input box (asserted on the writer's byte log), the payload record survives, and once the user types a byte-identical second prompt is refused (`Stale`) instead of submitting the leftovers with the user's unsent draft. On the pane whose erases DID land the log carries one erase per payload byte, no record is kept, and the identical second prompt is `Applied`. A third pane runs the sequence with `Applied` as a control and is likewise admitted. Separately asserts the two preconditions the branch rests on: the coarse elapsed bucket (`format_idle_elapsed`) makes two prompts about one role byte-identical, so the repeat is ordinary rather than exotic; and the prompt is plain printable ASCII, so an exact undo is provable — which is what puts it on the opposite side of the drain from `scheduler/idle-worker/018`'s `⚠`-carrying report. `arm_idle_worker_watch`'s half of issue #715, and #876's fixed branch. The settle decision is deliberately run on the DRAINED pane too, because `Ambiguous` must stay out of the release arm whether or not the drain landed — releasing consumes a CONCURRENT delivery's record when this one left none (issue #424 S2).
- **Verified load-bearing:** stubbing `drain_stranded_payload` to abstain unconditionally turns the drained pane's erase-log assertion red (`left: 524, right: 1048`) while the stranded pane and the `Applied` control stay green.
- **Does not assert:** that `arm_idle_worker_watch` reaches the decision at all (the timer, the one-shot take and the identity/liveness gates are `scheduler/idle-worker/004`, `/008` and `/014`); that a real agent TUI honours the erases (the kernel line discipline stands in for one in `agent_pty`'s `erase_bytes_undo_exactly_our_write_and_leave_the_users_draft`, and no fault seam for a partial PTY write exists at the e2e layer); and the `DeliveryNotice` the daemon publishes when bytes ARE left behind.
- **Platform coverage:** mac+linux.

##### scheduler/idle-worker/020 — A worker whose first task was never answered, then re-delegated to and answered once, is never afterwards reported as not having responded — and a FRESH delegation to that same pane that goes silent still is (issue #1080).
- **Layer:** fast integration (real `handle_delegate`/`handle_work_done` against daemon-owned PTYs, in the production order the daemon log records).
- **Agent:** none (`cat` stand-ins).
...
- **Asserts:** against a project defining `loop` (a `planner` start role with a `prompt_template` and a `builder` role with a `description`), the reply is `ok` and its `context_path` names `<project>/.dot-agent-deck/orchestrator-context.md`; that file already exists when the reply arrives (the publish precedes the success, not the other way round) and contains the configured prompt template, `**builder**: …`, `## Delegation protocol`, `## Your task`, the task text and the `## Task precedence` statement (issue #703 — two documents reach the coordinator in one file, and before it only their ORDERING said which won), while NOT containing the `## Unattended run` notice: this verb's caller is the desktop's live-loop panel, which refuses to launch without a task prompt and has the operator who typed it watching the panes, which is why the attendance is a caller declaration rather than an inference from "a task was supplied"; the reply's `roles` carry `planner`/`start` and `builder`/not-start. Then a second preparation naming an orchestration the project does not define is refused with a code that is NOT `unimplemented`, carries no `PreparedWorkflow`, writes no context file under that project, and leaves the daemon holding zero panes. This re-establishes daemon-side the only end-to-end check that the context has the right CONTENT, which `desktop/src-tauri/src/lib.rs`'s `workflow_launch_prepares_canonical_context_in_config_order` provides today and loses when the write moves.
...
##### fleet/observe/002 — A remote deck is reached through a real `ssh -N -L` tunnel and observed beside a local one.
- **Layer:** L2 lane 1 (a private `sshd` on an ephemeral loopback port plus two headless `daemon serve` processes; no PTY and no TUI surface, for the same reason `fleet/observe/001` has none ([#953](https://github.com/vfarcic/dot-agent-deck/issues/953))).
- **Agent:** none (one `sh -c 'sleep 600'` stand-in per daemon, the pattern `tests/e2e_handshake.rs` uses). No credential: the `sshd` is the test's own, its host key and its `authorized_keys` keypair are generated in the test's temp dir, and nothing under the operator's `~/.ssh` is read or written.
...
- **Asserts:** the TUI-native control creates a separate orchestration tab containing `coordinator`, `builder`, and `reviewer`; the desktop-shaped path uses the desktop's 32×120 defaults, shares one orchestration id, and preserves role index/name/start-role/cwd/display-title membership, declared agent type and pane id across all prepared starts. Starting before attachment rebuilds the titled tab with all roles. Starting while the TUI is attached creates the same titled tab without a reconnect; switching into it shows `3 session(s)` and the grouped, role-named `ClaudeCode · coordinator`, `OpenCode · builder`, and `Pi · reviewer` cards. Regression guard: before `9d66ef6b`, the live roles could appear as flat dashboard cards without their orchestration tab.
...
- **Asserts:** the TUI-native close removes the named card and empties `ListAgents`; the desktop-shaped stop is confirmed and likewise empties `ListAgents`, after which the already-attached dashboard reaches `No active sessions` and stops rendering the name without a keypress, reconnect, or manual refresh. Regression guard: before `d441cabb`, `ListAgents` emptied without a post-stop `AgentEvent`, and after 15 seconds the grid still rendered `dot-agent-deck — 1 session(s)` and the `No agent · desktop-visible-agent` card with `0 active │ 0 tools`.
- **Does not assert:** the exact payload or count of daemon-authored pane-close events (only their visible end-state); the Tauri button itself; child signal escalation timing; the desktop's terminal-tile detach; filesystem cleanup; a naturally exiting or hook-emitting agent.
- **Platform coverage:** mac+linux (`#![cfg(all(feature = "e2e", unix))]` — `DaemonProc` binds Unix-domain sockets and `TuiDeck` uses a PTY).
...
- **Agent:** none (three `printf …; sleep 600` role stand-ins declared as Claude Code coordinator, OpenCode builder and Pi reviewer; no credential).
- **Asserts:** the native close stops all three roles, empties `ListAgents`, deletes the clean orchestration tab, and returns to the empty Dashboard; the desktop-shaped close confirms every concurrent role stop and empties `ListAgents`, after which the already-attached TUI removes the identically titled now-empty tab without a keypress, reconnect, or manual refresh. Regression guard: before `d441cabb`, all three role ids disappeared from `ListAgents` without post-stop `AgentEvent`s, but after 15 seconds the `Desktop prepared run [×]` tab remained with stale role cards and impossible aggregate counts (observed as `1/4 session(s)` with one stale role and `2/5 session(s)` with two on repeat).
- **Does not assert:** the exact payload or count of daemon-authored pane-close events (only their visible end-state); the Tauri Close control itself; partial-stop failure rendering; stop completion order; worktree cleanup; agent work or hooks; the desktop's per-role terminal detaches.
- **Platform coverage:** mac+linux (`#![cfg(all(feature = "e2e", unix))]` — the daemon/harness uses Unix-domain sockets and PTYs).
