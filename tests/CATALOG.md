...
##### scheduler/dispatch/013 — A fired `issue_dispatch` task drives the GENUINE `gh` → clone → per-issue worktree → real-agent path against LIVE GitHub: a real Haiku agent runs inside the cloned worktree and its rendered pane shows the repo's committed sentinel filename — the real-scenario showcase (CLAUDE.md rule 4) a `cat`/stub stand-in can never prove (PRD #120).
- **Layer:** L2 PTY (the real `dot-agent-deck` binary in an isolated PTY via the `TuiDeck` harness, asserted on the rendered vt100 grid — same harness as `scheduler/dispatch/011`). REAL seams, no stand-ins: REAL `gh` on the normal PATH (no `gh` stub) really enumerates/PR-checks/clones against live GitHub, with `GITHUB_TOKEN` threaded through the scrubbed deck env so the daemon's `gh` inherits auth; `default_command` (via `DOT_AGENT_DECK_CONFIG`) is a REAL `claude -p` run pinned to Haiku (`claude-haiku-4-5-20251001`) — the `chain-smoke/claude/001` launch mechanism + model — told to `ls -a` and print filenames verbatim, with a trailing `; cat` keeping the pane alive after the print-mode agent exits. `DOT_AGENT_DECK_EXPERIMENTAL=1` activates the flag-gated flow; the fire is driven by `RunNow` over the attach socket.
- **Fixture:** the permanent public repo `vfarcic/dot-agent-deck-tests` — a committed `DISPATCH_E2E_SENTINEL.md`, a plain `.dot-agent-deck.toml` with NO `[[orchestrations]]` (single-agent card → live-surfaces), and a PERMANENT open issue #1 labelled `agent-dispatch-test`. The schedule filters on that label with `max_per_run = 1`, so ONLY issue #1 is enumerated (deterministic). Clone + worktree live under a `tempfile::tempdir()` removed on drop.
- **Agent:** REAL Claude Code (Haiku), one cheap print-mode invocation (<$0.05/run). Flaky-tolerant pre-PR tier (real LLM + real network) — run once, not looped (rule 4). Runtime-skipped (Decision 26) when the `claude` CLI/credentials or `GITHUB_TOKEN` are absent.
- **Asserts:** after the fire the daemon registers the dispatched agent under the schedule name `github-issues` (precondition — proves the live clone + worktree + spawn happened) and a per-issue card surfaces live (its `Dir:` line shows the worktree basename `issue-1`); focusing the card, the committed sentinel filename `DISPATCH_E2E_SENTINEL.md` appears in the agent's rendered pane output (proving the daemon really cloned the repo and the real agent ran in the per-issue worktree and saw the files); and (best-effort) the fixture repo has no pushed `agent/issue-1` branch afterward (NO REMOTE WRITES).
- **Does not assert:** exact agent phrasing (asserts on the sentinel filename, robust to LLM variance); the clone/worktree/branch derivation or skip/dedup/cap/cleanup logic (covered by the headless `scheduler/dispatch/001-010` and the deterministic-stub `scheduler/dispatch/011-012`); the orchestration-tab dispatch path.
- **Platform coverage:** mac+linux.
