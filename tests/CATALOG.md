...
##### orchestration/delegate/034 — A cleanup hold taken AFTER the respawn lifted the pane's record out still ends with the role live (issue #1114, Greptile P1 on PR #1119).
...
- **Agent:** none — the same SIGTERM-ignoring stand-in `orchestration/delegate/022` uses (`trap '' TERM; exec cat`), here because the respawn's own `AGENT_TERMINATE_GRACE` is what holds the gap open between lifting the record out and spawning the replacement. A plain `cat` closes that gap in milliseconds and the hold cannot be placed inside it.
- **Asserts:** the precondition that the stand-in's trap is armed (the `STUBBORN-WORKER-ARMED` marker, issue #709's fix borrowed) and that NOTHING is closing the pane when the delegate lands — which is what makes this the ordinary leg rather than `/032`'s window. Then, with the hold taken at the observable edge of the gap (the pane's record having gone) and kept up past the grace, that the pane still ends with a LIVE replacement agent and the role still routes in `pane_role_map`. Also asserts, as a precondition, that the modelled `StopAgent`'s own `close_agent` FAILS — if it succeeded the respawn had removed nothing and the gap was never entered.
...
- **Budget:** `STARVED_STOPAGENT_HOLD` (5 s) must outlast the respawn's 3 s grace, or the refused spawn simply succeeds and the test proves nothing; and stay well under the recovery's 30 s ceiling, above which a fixed build fails too. A `sleep` can only overshoot, so load moves it in the safe direction.
- **Does not assert:** the task pointer reaching the replacement — the modelled `begin_pane_close` sweeps this pane's outstanding delegation on the way in, exactly as the real handler would, so delivery is not this scenario's to claim (`/032` and `/033` assert it on the paths where the close does not sweep it); the natural frequency of this interleave, which is narrower than either entry recorded on #1114 and is not measured here; the daemon-log line the refused spawn no longer produces.
- **Platform coverage:** mac+linux (unix-only — the stand-in is a POSIX shell script).
