...
##### status/supersede/017 — The reconnect overlay changes no newest-wins pick where the state already holds evidence for the pane or agent (issue #804).
...
- **Asserts:** with a `SessionStart` from the same agent that reached the TUI before hydration and TIES the snapshot's instant, the seeded card keeps its freshly minted value rather than creating a tie, and `pane_session_id`, `pane_writable`, `agent_writable` and `live_session_for` all still pick it (the competitor's key sorts after `pane-<id>`, so an exact tie would be settled against the seeded card by the join's `session_id` tiebreak); with an OLDER competitor the snapshot's instant is taken and the picks are unchanged; a future-stamped session from another agent that beat the minted card still beats it when the snapshot is stamped further out; a newer paneless report from the same agent keeps `agent_writable` on the seeded card; and a re-seed from a snapshot older than a frame the card has since seen does not move `last_activity` backward. Removing any one clause of the rule — the whole check, the snapshot-newer-than-evidence half, the no-later-than-the-minted-value half (red on the future-stamped competitor), the same-agent half of the evidence set, or reading the evidence after the replaced card is gone — turns this test red.
- **Does not assert:** the readout (`session/live/016`); agreement with the daemon's own decisions (`status/supersede/016`); a future-stamped snapshot against a successor (`status/supersede/018`); why the TUI can hold a second card for the pane at all — that pre-hydration `SessionStart` duplicate predates issue #804 and is left as it was.
- **Platform coverage:** mac+linux+windows.
