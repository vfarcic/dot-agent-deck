...
##### status/shell-activity/008 — A process-table sample reads a command line for the detached descendants of its own roots and for nothing else on the machine (issue #862).
- **Layer:** L1.
- **Agent:** none (the same real `sleep` grandchild `status/shell-activity/001` spawns and `setsid()`s — no agent involved).
- **Asserts:** with the test's own pid named as the sample's root, `process_table(&[root_pid])` returns `CommandLine::Read` carrying the uniquely-marked full argv for the real `setsid()`'d grandchild; `CommandLine::NotSampled` for `mid`, a real child of the test in the test's OWN POSIX session (so the scoping is by session, not merely by descendancy); and `NotSampled` for every other row in the table, asserted by enumerating the rows that carry a command line and requiring the grandchild to be the only one. Then, with **no** roots, `process_table(&[])` returns `NotSampled` for every row including the grandchild — which is what makes the previous claim a property of the roots rather than an accident of this process tree.
