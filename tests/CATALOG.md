...
- **Asserts:** a wheel at an off-centre point inside the focused pane's content area is delivered as exactly that cell in all four mode × child-mouse cells, with the forwarded SGR report carrying the pointer's own 1-based cell and a deck-owned scroll genuinely moving the view; a wheel at the corners and centre of every deck card the production dashboard painted is dropped entirely — nothing delivered, no bytes to the child, the deck's view unmoved — in both PaneInput-with-child-mouse and command mode, against a fixture with nonzero retained depth so "unmoved" is not vacuous; a sweep of **every cell of a whole frame** delivers to exactly the content area's cells and to nothing else, and every delivered cell equals the pointer's own offset rather than a saturated one, with the frame's card rects proven disjoint from the pane's content area first; and a frame that drew no pane at all drops the wheel rather than falling back to raw screen coordinates.
