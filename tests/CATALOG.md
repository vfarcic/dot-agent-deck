...
- **Layer:** L1 (in-process production wheel, boundary-sized one- and two-pane renders, command-banner overlay, production focus change, and full `handle_key_event` seams with injected `Instant`s and a recording child-input channel; no PTY subprocess).
- **Agent:** none (agent-agnostic mature DECSTBM/cursor-repainting panes, including both panes in the focus-affinity frame).
- **Asserts:** the exact 80-column full-tier boundary renders the complete long sentence, 79 columns falls back to the centred short sentence, the exact 41-column short-tier boundary still renders it without offering PageUp, and 40 columns omits both tiers without modifying either border or any guard column beyond the pane; when the single-line command banner and notice are live together, the intact notice is the frame's only reversed run and no command-banner text remains; after production focus moves from pane A to pane B, exactly one notice remains on A and none appears on B; the normal wide notice remains a single centred REVERSED line explaining redraw-in-place, absent scrollback, and nothing to scroll without suggesting PageUp; it clears exactly at the shared 2.5-second `COMMAND_BANNER_TTL`, refreshes without duplication through the original expiry, and clears at the refreshed expiry; the next unbound printable and bound Ctrl+D each dismiss it without losing normal child forwarding or mode transition.
- **Does not assert:** real terminal mouse-report decoding (covered by the next scanner delegation), wall-clock scheduling in the live 16ms loop, horizontal multi-column pane geometry, real-agent behavior, or that any particular agent produces the synthetic repaint pattern.
- **Platform coverage:** mac+linux+windows.
