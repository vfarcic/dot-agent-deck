...
- **Asserts:** the subprocess does not report success, prints nothing at all on stdout (the caller forwards whatever is there), carries no Rust panic, and fails with neither clap's generic usage code `2` nor a clap `Usage:` banner — ruling out "this build has no such subcommand" as the reason; and the inode still exists afterwards, since unlinking it is the daemon's own recovery and not a read-only resolver's. This is the availability half of issue #1174: `remote_tunnel::REMOTE_SOCKET_PROBE`'s filesystem tests answer about the inode, and a dead daemon's inode is still a socket, so the snippet names it and the forward comes up against nothing.
...
- **Asserts:** the same four handled-refusal properties as `/001` (no success, empty stdout, no panic, not clap's `2`/`Usage:`), plus that stderr names the refused mode. Load-bearing because the listener is genuinely live, so only the mode clause can refuse it — and a mode clause is exactly what the shell snippet has no portable `test` spelling for, at any of its rungs.
