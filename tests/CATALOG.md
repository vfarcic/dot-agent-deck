...
- **Asserts:** in failure order, the caller and unit observers are ready; registering the unit retains exactly one return edge; closing the original caller without the deliberate-close sweep and spawning the successor produces a different registry agent id on the same pane; terminal `handle_work_done` consumes the return edge even though delivery is refused; an authorized `GuardedSend::Applied` barrier reaches the successor; after that barrier the successor's snapshot contains none of `dispatch:`, the distinctive unit name or its distinctive report; and the real warning names the caller pane, `WrongSession`, the identity-gate refusal, and `nothing written`.
...
- **Asserts:** in failure order, caller, unit and alternate observers are ready; registering the unit retains exactly one return edge; `begin_pane_close` on the CALLER immediately reduces the retained count to zero before the child is closed; the full close lifecycle completes; the unit's terminal `handle_work_done` returns without panic and does not recreate or reroute an edge; an authorized `GuardedSend::Applied` barrier reaches the unrelated pane; its drained snapshot contains none of `dispatch:`, the unit name or report; the close log says it dropped dispatch-return entries touching the caller pane; and the late completion is logged under its unit pane by the existing `work-done from unknown pane` path.
