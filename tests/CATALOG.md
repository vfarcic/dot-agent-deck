...
- **Does not assert:** the exact timeout duration the fix settled on (only that it is comfortably under 15s); `SocketReply`'s three-way outcome (only `request_from_socket`'s two-way `None` collapse is exercised here — the richer outcome, which `main.rs` reads on the `delegate` and `pane restart` paths, is exercised by `error/socket/006`); real daemon behavior; Windows named-pipe timeout semantics.
- **Platform coverage:** mac+linux (Unix-domain socket).
