...
##### prompt/pane-input/037 — The other two spawn-time delivery gates wait for a conversation too: a dispatch and an orchestrator remit (issue #1005).
- **Layer:** L1, in `src/ui.rs`'s own test module (the production `process_pending_dispatches` and `deliver_orchestrator_prompt` over a recording pane controller).
- **Agent:** none (a synthetic Claude Code card-surfacing `SessionStart` and a synthetic genuine one).
- **Asserts:** for a pane whose only session is the card-surfacing `SessionStart`, neither site puts bytes in the pane across two passes and neither consumes its payload; the agent's own `SessionStart` then releases exactly one write on each, and the orchestrator's names the generation it entered.
- **Why both are here rather than folded into `prompt/pane-input/036`.** Issue #1005 names two call sites and there are three: `process_pending_dispatches` is the third, it is the one the issue mislabels, and it is the FASTEST to write — it has no readiness buffer at all, so a resolved agent type puts the delegated task in the pane on the very next frame. The orchestrator site's buffer is pre-spent in the fixture so the only thing under observation is the readiness gate.
- **Does not assert:** the seed path (`prompt/pane-input/036`); confirmation, retry or deadline behaviour on either path (`prompt/pane-input/023`–`/030`); the daemon-owned delegate path.
- **Platform coverage:** mac+linux+windows.
