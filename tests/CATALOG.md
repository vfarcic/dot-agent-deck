...
- **Does not assert:** the tail of the same race, where a cleanup hold is taken between the recovery's wait ending and its next respawn attempt — the `DuplicatePaneId` retry in `respawn_or_recreate_agent_for_pane` covers it and no test forces it, because doing so needs a seam between two adjacent statements inside the registry; the give-up boundary (`/033`); what the TUI paints for the recovered card; the close's own pane-scoped cleanup beyond the role map.
