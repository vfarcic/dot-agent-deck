...
- **Agent:** none.
- **Asserts:** (d) a position put through `SavedFocus::without_pane_ids` — what the daemon-EMPTY rebuild path applies, since the panes it restores carry freshly allocated `allocate_id` counters rather than the daemon's stable ids, so a remembered number can name a DIFFERENT role — writes NO pane id even when one would resolve, while its id-free half (a remembered Dashboard) still applies. (a) a `SavedFocus` whose ids no tab owns — a pane the user closed, or a role whose agent died and came back as a synthetic dead-slot — names NO landing tab and writes nothing, so the orchestration tab still falls back to its start role, i.e. exactly today's behaviour rather than an error; (b) a dead-slot id sitting in a tab's `focused_role_pane_id` (reachable, since hydration puts synthetic ids in `role_pane_ids` and `record_focus` accepts any member of that list) is filtered out at CAPTURE, so it can never be written to disk and then shadow a live role; (c) a `dashboard_active` capture resolves to the Dashboard tab, located by KIND rather than by assuming index 0, while a fully defaulted `SavedFocus` resolves to `None` and leaves the deck's own landing choice alone.
- **Does not assert:** the successful round-trip (`session/restore/017`); the user-visible cycle (`session/restore/016`); that `src/ui.rs` actually calls `without_pane_ids` on the rebuild path (a one-line call site at the startup restore seam, out of L1 `TabManager` reach); that a dead-slot id can never be written by an OLDER binary and read back by this one — the restore path drops it anyway, since `tab_index_owning_pane` rejects dead-slot ids, which is what (a) covers.
- **Platform coverage:** mac+linux+windows.
