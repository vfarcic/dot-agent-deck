...
- **Does not assert:** the NATIVE `get-seed` pull that normally wins the race (`take_pending_seed_native` and its flag are unit-tested beside it); the leave-it-alone branch taken when the pane's seed belongs to the successor (`prompt/pane-input/034`); the two production callers that arm it; the `Ambiguous` partial-write branch.
