...
##### tabs/orchestration/014 — An orchestration tab rebuilt from daemon role metadata that no longer matches `.dot-agent-deck.toml` paints a `[config drift]` marker after its name, and only that tab does (issue #554).
...
- **Asserts:** with one drifted and one matching orchestration tab, the strip paints `review [active] [config drift]` for the drifted one and `build [active]` for the other, and the marker appears exactly once. At width 28 the orchestration name still reads and a `…` is present, so the marker — a suffix — is what truncation elides, matching the name-first ordering `tabs/orchestration/013` pins.
