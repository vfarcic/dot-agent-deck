...
##### orchestration/provenance/001 — On the REAL binary, under the DEFAULT policy, a `work-done` forged from outside a pane with nothing but that pane's id is refused, while the same verb issued from inside the pane still lands (issue #1077).
...
- **Does not assert:** the in-process decision matrix (unit-tested in `src/hook_provenance.rs`) or the refusal reply a `delegate` caller receives on the connection (`src/daemon.rs`'s `hook_provenance_*` tests, over a real socket and registry); a same-uid process that reads the victim pane's `/proc/<pid>/environ` and presents the stolen token — which this mechanism does NOT stop, by design and in writing (`docs/develop/hook-provenance.md`); raw `AgentEvent` traffic, which the gate deliberately does not cover (issues #543 / #401).
