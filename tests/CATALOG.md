...
- **Does not assert:** exact model response phrasing, ordering between the three agents, or a fixed boot duration; the bootstrap's `PreToolUse` is synthetic scheduling evidence and not treated as readiness or as the genuine start that authorizes attempt 3.
- **Platform coverage:** mac+linux.
