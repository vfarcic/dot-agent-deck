...
- **Agent:** none.
- **Asserts:** an automatic payload lands, the user types an unsent draft, and the production `compose_respawn_failed_notice` text is then delivered through the same `write_notice_guarded` call the respawn-error arm of `dispatch_one_owned` makes; a following submit-only probe is refused and leaves the draft-plus-notice snapshot unchanged rather than submitting it. Keyed on the DELIVERY MECHANISM rather than on which notice it is: this pins the whole `write_notice_guarded` family, whose one remaining production member is the respawn-failure notice. Issue #702 moved the delegate silence notice out of that family onto the submitted path, where the question does not arise, and issue #708 moved the worker-exited and respawn-no-live-worker notices after it — which is why this test drove `compose_worker_exited_notice` until #708 and drives the respawn-failure text now.
- **Does not assert:** the broader idle-worker detection policy or the exact diagnostic prose, only that a production deferred-notice caller cannot reauthorize a blind probe; the trigger that decides to send the notice (a failed `respawn_agent_for_pane`), which is stubbed out here; and the SUBMITTED family's own separate limitation, where a pane holding an unsent human draft has that draft submitted along with the daemon text (issue #544).
- **Platform coverage:** mac+linux.
...
- **Asserts:** after the delegate lands and the worker's process ends on its own, the worker pane is confirmed NOT in a close transition (so the close-time sweep is provably not what retired the records), the new EOF-triggered notice appears in the orchestrator's pane within 5s and names the exited worker's pane id, and neither the older idle-timeout prompt nor the older delegate-possibly-not-delivered silence notice appears at all. Issue #708: the notice is SUBMITTED — the single byte following its stable final clause (`names the role and how long it had been delegated.`) is CR, not LF — and names the remediation options (notify the user, re-delegate, reassign). That byte is exact because the orchestrator stub's `stty -echo -icanon -icrnl -opost` is chained with `&&` before its readiness marker, so no CR/LF translation sits on either side of the pane. Verified red on the pre-fix build (`terminator = Some(10)`), and red again with only `deliver_worker_exited_notice`'s call reverted to `write_notice_guarded`.
...
- **Does not assert:** the trigger that decides to send the report (`arm_delegate_silence_watch`'s window, covered by `orchestration/delegate/013` and `/025`); the DEFERRED family's own laundering question, which is `scheduler/idle-worker/015`'s and lives on `write_notice_guarded`; the same release at the two report sites issue #708 moved onto the submitted path (the worker-exited and dead-replacement reports), which call the same `settle_one_shot_payload_record` this test exercises but have no second-report test of their own; and the identity/liveness gates on the send, which `scheduler/idle-worker/008` and `/014` pin.
- **Platform coverage:** mac+linux.
