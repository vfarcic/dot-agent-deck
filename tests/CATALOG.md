...
- **Does not assert:** the per-frame call site that feeds live `SessionStatus` into the resolver during `run_tui` (the render loop wiring, `src/ui.rs` — now covered by `tabs/orchestration/011`); any visible rendering of the focused-pane border/highlight; hook-driven status transitions (statuses here are synthetic, not sourced from real events).
- **Platform coverage:** mac+linux+windows.

##### tabs/orchestration/011 — The `run_tui` render-loop call site that applies `auto_focus_waiting_pane`'s result actually reaches the render pass, not just `TabManager`'s internal bookkeeping (fork-only, no PRD/issue; closes a Greptile P2 gap left open on PR #5, which was merge-only with no source changes allowed).
- **Layer:** L1 (in-process `TestBackend` render via `compute_frame_layout` + `render_frame`, driven through a real `ratatui::Terminal<TestBackend>`; `src/ui.rs`'s own `#[cfg(test)]` module, same pattern as `orchestration/layout/004`, because `compute_frame_layout`/`render_frame`/`ActiveTabView`/`FrameContent` are module-private and unreachable from `tests/*.rs`).
- **Agent:** none (`OpenTabPC` mock `PaneController`, extended with a `focused_pane_id()` that mirrors `EmbeddedPaneController`'s live "which pane is focused now" read; synthetic `SessionStatus` map, no panes/PTYs).
- **Asserts:** with a real `TabManager`-opened 2-role Orchestration tab and the higher-order `coder` role manually focused, marking the lower-order `orchestrator` role `WaitingForInput` and then replaying `src/ui.rs`'s per-frame call site VERBATIM — `TabManager::auto_focus_waiting_pane` then `pane.focus_pane` on its `Some(new_id)` result, exactly as the render loop applies it — leaves the SAME pane controller reporting `orchestrator` as focused; feeding that live focus into `compute_frame_layout` + `render_frame` under `PaneLayout::Stacked` then shows the auto-focused `orchestrator` role's rect reclaiming the full pane-column height while the manually-focused-but-superseded `coder` role's rect collapses to zero height. Unlike `tabs/orchestration/010` (which asserts only on `TabManager`'s internal `focused_role_pane_id` field), this test proves the resolver's result actually propagates through `pane.focus_pane` into what a real render pass draws.
...
- **Does not assert:** the render-loop call site itself actually applying the result to a live pane controller / render pass (would mirror `tabs/orchestration/011`'s technique for `auto_focus_waiting_pane`, deferred — not required by PRD #373 M1's stated scope). The 30-second inactivity timer and its blocked-keystroke reset (PRD #373 M2/M3) were deleted outright by PRD #393 M4, so there is nothing left here to cover.
- **Platform coverage:** mac+linux+windows.

##### tabs/orchestration/020 — Pins the fix for a review blocker: the all-clear waiting-history observation now runs once per frame UNCONDITIONALLY (`TabManager::observe_waiting_panes`, called before the auto-focus chain) instead of inside `auto_focus_all_clear`, so a waiting episode observed in a SINGLE frame — the frame `auto_focus_waiting_pane` consumes when it steers focus onto the newly-waiting role — is still remembered and still edge-triggers the M1 focus move when it resolves (PRD #373 M1, fork-only).
- **Layer:** L1 (in-process unit test; `src/tab.rs`, alongside `orchestration_012`).
- **Agent:** none (mock `PaneController`; synthetic `SessionStatus` map, no panes/PTYs).
- **Asserts:** with a real `TabManager`-opened 3-role Orchestration tab and focus manually on the non-orchestrator `alpha`, driving the real per-frame sequence (unconditional `observe_waiting_panes`, then `auto_focus_waiting_pane` → `auto_focus_all_clear` gated exactly as `src/ui.rs` gates them): a quiet frame moves nothing; the frame `beta` becomes `WaitingForInput` steers focus onto it via `auto_focus_waiting_pane`, which WINS the chain so `auto_focus_all_clear` never runs on the only frame `beta` is observed waiting; the very next frame, with `beta` resolved and no intervening still-waiting frame at all, the all-clear move still fires and returns the orchestrator role, whose id also becomes the tab's `focused_role_pane_id`; a further quiet frame does not fire again (still edge-, not level-triggered).
- **Does not assert:** the mechanism's internal shape (the `all_clear_pending` latch is an implementation detail — this pins the outcome); the multi-frame waiting episode (covered by `tabs/orchestration/012`); the render-loop's application of the result to a live pane controller (covered by `tabs/orchestration/011`). The inactivity branch it used to name was deleted by PRD #393 M4.
- **Platform coverage:** mac+linux+windows.
...
##### tabs/orchestration/025 — Decision 5's LOCKED half, as a characterization pin: with three non-orchestrator role panes concurrently `WaitingForInput`, focus visits them in ascending `role_pane_ids` order, advancing to the next-lowest still-waiting role each time the currently-focused one resolves, then returns to the orchestrator role on the all-clear edge once all three have resolved (PRD #393 M4b / decision 5, fork-only).
- **Layer:** L1 (in-process unit test; `src/tab.rs`, alongside `orchestration_010`/`012`/`020`).
- **Agent:** none (mock `PaneController`; synthetic `SessionStatus` map, no panes/PTYs).
- **Asserts:** with a real `TabManager`-opened 4-role Orchestration tab (`orchestrator` < `alpha` < `beta` < `gamma`) and the real per-frame sequence (unconditional `observe_waiting_panes`, then `auto_focus_waiting_pane` -> `auto_focus_all_clear`, gated exactly as `src/ui.rs` gates them today): a quiet frame moves nothing; `alpha`, `beta`, and `gamma` transitioning to `WaitingForInput` together steers focus to the LOWEST-order one (`alpha`) first; resolving `alpha` while `beta`/`gamma` are still waiting advances focus to `beta`; resolving `beta` while `gamma` is still waiting advances focus to `gamma`; resolving `gamma` — nothing left waiting — fires the all-clear edge and returns focus to the orchestrator role; a further quiet frame does not move focus again. Deliberately GREEN on arrival: `auto_focus_waiting_pane`/`auto_focus_all_clear` already implement exactly this and M4a left them ungated — this pins decision 5's promise as a regression guard for M4b's gating change, rather than introducing new behavior.
- **Does not assert:** anything about the deck-global lock itself (`ui.command_entry_locked` is not read anywhere in this test — see `tabs/orchestration/026` for the gated/unlocked half); the two-pane tie-break edge cases already covered exhaustively by `tabs/orchestration/010`; the render-loop's application of the result to a live pane controller (covered by `tabs/orchestration/011`).
- **Platform coverage:** mac+linux+windows.

##### tabs/orchestration/026 — Decision 5's UNLOCKED half plus the latch-clearing rule: while unlocked, no auto-focus branch fires at all (a waiting pane in flight does not steal focus and its resolution does not fire an all-clear either), and re-locking after such a stretch must NOT fire a stale all-clear move for an episode the human already handled by hand, while still resuming normal steering/all-clear pinning for a fresh episode (PRD #393 M4b / decision 5, fork-only).
- **Layer:** L1 (in-process unit test; `src/tab.rs`, alongside `orchestration_010`/`012`/`020`/`025`).
- **Agent:** none (mock `PaneController`; synthetic `SessionStatus` map, no panes/PTYs).
- **Asserts:** with a real `TabManager`-opened 3-role Orchestration tab, `beta` goes `WaitingForInput` while locked and steals focus (as `orchestration_010` pins); the deck then unlocks MID-EPISODE — `beta` still waiting — and `TabManager::clear_waiting_pane_latch` is called at that exact point, mirroring the locked->unlocked transition's required effect; with focus then manually re-affirmed on `beta` (the pane already focused from the steal above — deliberately NOT `alpha`, so the fresh episode below is a genuine focus move rather than a same-pane no-flicker no-op), a local `frame` helper whose `locked: bool` flag models the FUTURE gated `src/ui.rs` call site (skipping `observe_waiting_panes` and the whole chain entirely when `false`) confirms `beta` — still nominally waiting — does not steal focus back, and `beta` resolving to `Idle` while unlocked fires no all-clear move either, so `beta` stays focused throughout. **The stale-latch assertion:** re-locking (`frame(locked: true)`) after that stretch must return `None` and leave focus on `beta` — without the prior `clear_waiting_pane_latch` call, `observe_waiting_panes` would compare its stale pre-unlock `had_waiting_pane == true` against the current idle status, misread it as a fresh edge, and fire a spurious focus move off `beta` to the orchestrator. Finally, a fresh waiting episode under the re-engaged lock (`alpha` waits, then resolves) proves normal steering and all-clear pinning both resume.
- **Does not assert:** the real `src/ui.rs` per-frame call site actually being gated on `ui.command_entry_locked` (out of L1 `TabManager` reach — the local `frame` helper's `locked` flag only models the FUTURE call site's shape; the real wiring is `orchestration/focus/002`'s / M5's job); the toggle handler (`src/ui.rs:7238`) actually calling `clear_waiting_pane_latch` on the real locked->unlocked transition (same reason); the LOCKED-only steering/ordering contract, covered exhaustively by `tabs/orchestration/010`/`025`.
- **Platform coverage:** mac+linux+windows.

##### tabs/orchestration/027 — The locked->unlocked lock toggle must clear the waiting-episode latch on EVERY Orchestration tab, not just the one active at the moment of the toggle: a background tab's already-resolved waiting episode must not be replayed as a stale all-clear edge that yanks focus away from where the user left it on return (PRD #393 review blocker 1, PR #51 — `src/ui.rs:7291-7293` / `src/tab.rs:560-570`).
- **Layer:** L1 (in-process unit test; `src/tab.rs`, alongside `orchestration_010`/`012`/`020`/`025`/`026`).
- **Agent:** none (mock `PaneController`; synthetic `SessionStatus` map, no panes/PTYs).
- **Asserts:** with TWO Orchestration tabs (`A`, `B`) open: while `A` is active and locked, its `alpha` role goes `WaitingForInput`, latching `A`'s `had_waiting_pane = true` and stealing focus onto `alpha` (as `orchestration_010` pins); the user switches to `B` and unlocks — `TabManager::clear_waiting_pane_latch` fires with `B`, not `A`, active; while unlocked, `A`'s worker resolves to `Idle` unobserved (the chain never runs against a background tab, and wouldn't run at all while unlocked even if `A` were active); the user re-locks and returns to `A`. `A`'s first locked frame back must be a no-op — the deck-global toggle's latch clear must have reached `A` too, even though `A` wasn't the active tab at the moment of the toggle — so focus stays on `alpha` rather than being yanked to the orchestrator role by a stale `true` -> `false` edge. Pins the outcome (every Orchestration tab's edge state is reset on the transition), not the mechanism the coder uses to reach it.
- **Does not assert:** the real `src/ui.rs` per-frame call site's gating on `ui.command_entry_locked` (out of L1 `TabManager` reach, same limitation as `026`); the toggle handler actually calling `clear_waiting_pane_latch` on the real transition (same reason); the single-tab straddling trace itself, covered exhaustively by `026`.
- **Platform coverage:** mac+linux+windows.

##### tabs/orchestration/028 — The waiting-focus branch must defer a focus steal, not apply it immediately, while a keystroke is still queued for the currently-focused waiting pane — Greptile's confirmed P1 on PRD #393's lock-governed focus chain (`.dot-agent-deck/waiting-focus-race-findings.md`): the waiting branch had no pending-input guard, unlike its all-clear sibling.
- **Layer:** L1 (in-process unit test; `src/tab.rs`, alongside `orchestration_010`/`012`/`020`/`025`/`026`/`027`).
- **Agent:** none (mock `PaneController`; synthetic `SessionStatus` map, no panes/PTYs).
- **Asserts:** with a real `TabManager`-opened 3-role Orchestration tab (`orchestrator` < `alpha` < `beta`), `beta` (higher role order) goes `WaitingForInput` and steals focus with no input pending, as `orchestration_010` pins; `alpha` (LOWER role order than `beta`) then ALSO goes `WaitingForInput` on a frame where `input_pending` is true (modeling a keystroke still queued for `beta`) — the steal to `alpha` must be deferred, returning `None` and leaving focus on `beta`, not yanked away from the pane the queued keystroke is aimed at; once `input_pending` clears on a later frame, the deferred steer to `alpha` must still fire, proving the guard DEFERS the move rather than dropping it, mirroring `TabManager::auto_focus_all_clear`'s existing "no one-shot latch" contract. Drives the not-yet-existing `TabManager::auto_focus_locked(pane_status, input_pending)`, the seam this test asks the coder to add, folding both `auto_focus_waiting_pane` and `auto_focus_all_clear` behind ONE shared `input_pending` guard mirroring the real per-frame call site's intended shape. **RED today: `auto_focus_locked` does not exist, so this fails to compile** (same pattern as `orchestration/lock/012`'s `build_pane_status_for_gate`).
- **Does not assert:** the real `src/ui.rs` per-frame call site actually computing `input_pending` from `crossterm::event::poll` or applying the result via `pane.focus_pane` (out of L1 `TabManager` reach — needs a PTY-attached L2 test, and the underlying race is not economically reproducible there either; see the finding doc); the deck-global lock gate itself (`ui.command_entry_locked`, covered by `026`/`027`); the multi-waiter ordering contract, covered exhaustively by `010`/`025`.
- **Platform coverage:** mac+linux+windows.
...
##### orchestration/delegate/019 — Two panes reporting `work-done` for the same role name and the same cwd (fork #76's two-orchestrations collision) must not clobber each other's report.
...
##### orchestration/delegate/020 — A report already sitting at the daemon's own work-done output path must survive a subsequent, differently-sourced `work-done` call, and the collision must be announced rather than silent (upstream #331).
...
##### orchestration/delegate/021 — `dot-agent-deck work-done --task-file <path>` must be refused, client-side, when `<path>` resolves inside the daemon's own `.dot-agent-deck/work-done-*.md` output namespace (upstream #331's own proposed fix).
...
- **Layer:** L2 (real-binary PTY via the vt100 `TuiDeck` harness).
- **Agent:** none (`orch-focus-lifecycle` fixture — `orchestrator`, `alpha`, `beta`, all plain `printf`+`sleep` stubs, no LLM tokens spent; `WaitingForInput`/`Thinking` are driven SYNTHETICALLY by injecting bare `AgentEvent`s over the hook socket, the same technique `orchestration/lock/010` uses, rather than by a real agent's own hook).
- **Asserts:** on the RENDERED GRID only (which role's pane box is expanded — the `┌<role>` fused title border only the focused role renders in `PaneInput` mode — and, for the final state, a typed sentinel's echo inside it): (1) a freshly opened tab shows the orchestrator's expanded box, the default LOCKED state; (2) injecting `WaitingForInput` for the non-orchestrator `alpha` role steers focus onto alpha's own expanded box; (3) injecting `Thinking` for `alpha` (status clears) returns focus to the orchestrator's expanded box, the all-clear edge; (4) `Ctrl+d` then `Ctrl+e` surfaces the `Pane entry: unlocked` status message; (5) with the tab now unlocked, jumping to the non-orchestrator `beta` role (deliberately NOT `alpha` — a fresh episode on `alpha` must be a genuine steer-attempt, not a same-pane no-op `auto_focus_waiting_pane` would no-op on regardless) and then a FRESH `WaitingForInput`/`Thinking` pair on `alpha` moves focus nowhere: `beta`'s expanded box, and a keystroke typed at the end, survive both the waiting pane appearing and its all-clear — proving `src/ui.rs`'s per-frame call site (`if ui.command_entry_locked { observe_waiting_panes(...); ... }`) genuinely never calls `observe_waiting_panes` at all while unlocked, leaving no auto-focus branch to fight the human's manual choice.
- **Does not assert:** the keystroke-forwarding gate itself (`orchestration/lock/004`/`010`); the `WaitingForInput` carve-out that lets a keystroke through a locked pane (`orchestration/lock/009`/`010`); the `TabManager`-level mechanism in isolation, including the stale-latch guard on re-locking (`tabs/orchestration/010`/`012`/`020`/`025`/`026`); the pure `Ctrl+e` mode-scoping proof (`orchestration/lock/007`/`008`); a real agent genuinely driving `WaitingForInput` in production (out of scope here for the same reason `orchestration/lock/010`'s doc comment gives — a real Haiku worker would self-skip in CI and leave this headline behavior with zero automated coverage); a re-lock after the unlocked stretch (already covered at the `TabManager` level by `tabs/orchestration/026`'s stale-latch assertion).
- **Platform coverage:** mac+linux.
...
##### orchestration/lock/004 — While an orchestration tab is locked (the default), a keystroke aimed at a non-orchestrator role's PTY is dropped before reaching it; the orchestrator pane's own input is never gated; `Ctrl+e`, sent from command mode, unlocks the tab and keystrokes to the non-orchestrator role then forward normally (PRD #361 Item 3, unlock ritual updated for PRD #393 M1's command-mode scoping).
- **Layer:** L2 (real-binary PTY via the vt100 `TuiDeck` harness).
- **Agent:** none (`orch-deck` fixture — two stub `cat` roles, no LLM tokens spent).
- **Asserts:** with a real orchestration tab open (default LOCKED), a sentinel typed into the still-focused orchestrator role is echoed by its `cat` process (never gated, lock state notwithstanding); focusing the non-orchestrator "worker" role (`1`-`9` jump) and typing a sentinel while still locked never reaches the worker's PTY (no terminal echo appears within a 2s poll window, since a dropped keystroke never reaches `write_raw_bytes`); entering command mode (`Ctrl+d`), sending `Ctrl+e` to unlock, returning to `PaneInput` (`Ctrl+d`), then typing a third sentinel into the same still-focused worker pane DOES forward and appear, proving the unlock takes effect immediately for the currently-focused pane.
- **Does not assert:** the visible status-message text on a dropped keystroke (the PRD names an example, `"Pane locked — Ctrl+d then Ctrl+e to unlock"` since PRD #393 M6 reworded it to name the command-mode chord first, following the `RequestConfigGen` no-op-with-feedback convention, but does not mandate exact wording); per-tab isolation (covered by `orchestration/lock/003`); global chords during the lock (covered by `orchestration/lock/005`).
- **Platform coverage:** mac+linux.
...
##### orchestration/lock/006 — With a REAL interactive Claude Haiku agent in the non-orchestrator role (not the `cat` stub `orchestration/lock/004` uses), the command-entry lock still gates a directive typed toward its PTY while locked, and still forwards it once `Ctrl+e`, sent from command mode, unlocks (CLAUDE.md rule 4 / Greptile PR #374 follow-up; unlock ritual updated for PRD #393 M1's command-mode scoping).
- **Layer:** L2 (real-binary PTY via the vt100 `TuiDeck` harness).
...
- **Asserts:** with a real orchestration tab open (default LOCKED) and the worker role focused, a create-sentinel-file directive typed into its PTY never results in the sentinel file appearing on disk (the agent never received it); entering command mode (`Ctrl+d`), sending `Ctrl+e` to unlock, and returning to `PaneInput` (`Ctrl+d`), then a second directive with a different sentinel typed into the still-focused worker pane DOES result in that file being created, proving a genuine agent — not just `cat` echo — receives and acts on input only once unlocked; the first (locked) sentinel is confirmed still absent at the end, ruling out a buffer-then-flush implementation instead of an outright drop.
...
##### orchestration/lock/007 — `scope_command_entry_lock`, the pure function replacing #374's inline un-resolution, claims `Action::ToggleOrchestrationLock` only in `UiMode::Normal` on an Orchestration tab, and passes every other action through untouched everywhere else (PRD #393 M1).
- **Layer:** L1 (pure-function unit test; `src/ui.rs`'s own `#[cfg(test)]` module, right beside `scope_split_stage`'s own `layout_005` test — no PTY, no TestBackend render).
- **Agent:** none.
- **Asserts:** table-driven over the full cross product of `is_orchestration_tab` (true/false) x every `UiMode` variant (enumerated via the same exhaustive `all_ui_modes` helper `layout_005` uses, so a future variant can't silently drop out of the table) x the action being `Some(Action::ToggleOrchestrationLock)`, `Some(Action::Quit)` (a representative other action), or `None`: `ToggleOrchestrationLock` survives ONLY at `(is_orchestration_tab = true, mode = UiMode::Normal)` and is un-resolved to `None` at every other cell; `Action::Quit` passes through completely untouched at EVERY cell, including `(is_orchestration_tab = false, mode != Normal)` — the case that rules out a blanket "drop the action" implementation; `None` in always yields `None` out.
- **Does not assert:** dispatch through the real `KeyEvent -> Action` resolver or a live orchestration tab's rendered geometry; the real-pane proof that the byte reaches a focused role pane's PTY (covered by the PTY-attached `orchestration/lock/008`). This is a mechanism test only — it proves nothing in a real pane on its own; it exists so a later failure localises (the same lesson PRD #387's Defect 1 cost: a correct mechanism attached to an untested real-pane path shipped a bug this test class alone cannot catch).
- **Platform coverage:** mac+linux+windows.

##### orchestration/lock/008 — `Ctrl+e` is claimed only in command mode, as the user actually experiences it: a focused real bash/readline orchestrator pane genuinely receives `0x05` (readline's end-of-line) in `PaneInput` mode, and the chord still toggles the deck's command-entry lock once `Ctrl+d` reaches command mode first (PRD #393 M1, the chord fix).
- **Layer:** L2 (real-binary PTY via the vt100 `TuiDeck` harness).
- **Agent:** none (`orch-lock-bash-role` fixture — a real interactive `bash --noprofile --norc -i` orchestrator role plus a `cat`-stub worker role, no LLM tokens spent).
...
##### orchestration/lock/009 — While a locked non-orchestrator pane's live status is `WaitingForInput`, `gate_pane_input_key` passes a keystroke through unchanged instead of dropping it; the carve-out closes again the instant the status clears (PRD #393 M3 decision 4).
- **Layer:** L1 (`gate_pane_input_key` called directly against a focus-echoing `FocusEchoPC`; no PTY, no TestBackend render).
- **Agent:** none.
- **Asserts:** with a real two-role orchestration open (deck LOCKED, the default) and the non-orchestrator "worker" role genuinely focused (`FocusEchoPC` echoes the last `focus_pane` call, so this isn't incidental mock behavior): a keystroke is dropped when no status is recorded for the worker's pane (today's `orchestration/lock/004` baseline); the SAME keystroke passes through UNCHANGED once the worker's pane reports `WaitingForInput`; and the SAME keystroke is dropped again once that pane's status moves to `Working` — the carve-out does not outlive the status that opened it. Also asserts the orchestrator pane's own input stays unaffected by ANY status recorded against ITS pane (never gated, waiting or not), and that an unlocked deck ignores `WaitingForInput` entirely (nothing for M3 to carve a hole out of when the lock itself is off).
- **Does not assert:** Dashboard/Mode tabs staying ungated regardless of lock state (covered by `orchestration/lock/011`, unaffected by M3); the real PTY-forward path or how a pane's status actually becomes `WaitingForInput` in production (covered by the PTY-attached `orchestration/lock/010`); the M4 auto-focus-onto-waiting-panes behavior (a separate concern, `tabs/orchestration/025`/`026`).
- **Platform coverage:** mac+linux+windows.

##### orchestration/lock/010 — The real-PTY proof of decision 4's carve-out: a locked worker pane that genuinely reports `WaitingForInput` over the hook socket has its keystrokes forwarded to its real PTY and visible on the grid; once the status clears the gate re-engages (PRD #393 M3). Deliberately NOT a real-agent test — see the test's doc comment for why a real-Haiku worker (the PRD's suggested "strongest candidate") would self-skip in CI and leave M3's headline behavior with zero automated coverage here.
- **Layer:** L2 (real-binary PTY via the vt100 `TuiDeck` harness).
- **Agent:** none (`orch-deck` fixture — two stub `cat` roles, no LLM tokens spent; `WaitingForInput` is driven SYNTHETICALLY by injecting a bare `AgentEvent` over the hook socket — the same wire the real `agent-event` CLI, Pi's status-reporting path, already rides — rather than by a real agent's own hook).
- **Asserts:** with a real orchestration tab open (default LOCKED) and the worker role focused: a sentinel typed into the worker's PTY is dropped before any status is ever reported for it (the ordinary `orchestration/lock/004` baseline); injecting a synthetic `AgentEvent{event_type: WaitingForInput, pane_id: worker}` over the hook socket, then typing a second sentinel, DOES reach the worker's PTY and echo on the grid (retried via `TuiDeck::send_keys_until_grid_string_within`, since the status arrives over an async daemon round-trip this test has no in-process signal to await instead of the grid itself — issue #395's exact race, guarded against rather than reproduced); injecting a synthetic `AgentEvent{event_type: Thinking}` for the same pane (status clears), re-focusing the worker explicitly (isolating this from the SEPARATE PRD #373/#393 M4 all-clear auto-focus, which may have already steered focus back to the orchestrator on its own), then typing a third sentinel is dropped again — the lock re-engages the instant the carve-out's condition stops holding.
- **Does not assert:** a real agent's own hook payload driving `WaitingForInput` in production (deliberately out of scope — see the doc comment); the auto-focus-onto-waiting-panes behavior itself (`tabs/orchestration/025`/`026`); global chords or per-tab lock storage (covered elsewhere).
- **Platform coverage:** mac+linux.
...
##### orchestration/lock/012 — Fail-closed ambiguity: two sessions colliding on the same `pane_id` (one `WaitingForInput`, one not) must NOT earn decision 4's carve-out — `gate_pane_input_key` must deny the exemption and drop the keystroke — while a single, unambiguous `WaitingForInput` session still passes one through unchanged (PRD #393 review blocker 2, PR #51 — `src/ui.rs:4380-4387` / `:12369-12374`).
- **Layer:** L1 (`gate_pane_input_key` called directly against a focus-echoing `FocusEchoPC`, fed through a new `build_pane_status_for_gate` join built from a synthetic `AppState`; no PTY, no TestBackend render).
- **Agent:** none.
- **Asserts:** with a real two-role orchestration open (deck LOCKED, the default) and the "worker" role genuinely focused (mirrors `orchestration/lock/009`'s setup): an `AppState` with TWO sessions sharing the worker's `pane_id` — one `Working`, one `WaitingForInput` — resolves through `build_pane_status_for_gate` to something other than `WaitingForInput` for that pane_id, and `gate_pane_input_key` fed that result drops the keystroke (`Action::Continue`), never `Action::ForwardToPane`. A second `AppState` with a SINGLE clean `WaitingForInput` session on the same `pane_id` resolves to `WaitingForInput` and still earns the carve-out (`Action::ForwardToPane` unchanged), reproving `orchestration/lock/009`'s claim through the new resolver so the fail-closed fix can't buy safety by breaking the already-working unambiguous case.
- **Does not assert:** `build_pane_status`'s own general dedupe/iteration-order behavior when a `pane_id` collides (that's #398, filed and scoped separately — this test's resolver is a NEW, carve-out-specific join, not a fix to `build_pane_status` itself); which of two colliding sessions that happen to AGREE on status "wins" (not exercised here — only the disagreeing case is pinned, per the reviewer's exact trace); the real production trigger for a duplicate same-`pane_id` session (PRD #110's `agent_id: None` pre-F9 hook shape, #401) actually occurring end-to-end against a live daemon.
- **Platform coverage:** mac+linux+windows.
