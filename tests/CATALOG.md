...
- **Asserts (issue #833 extension):** the card's *preferred* title source, `ui.display_names`, is defended at both of its writers rather than at the render seam. `pane::RenameOutcome::applied` — the gate the dashboard dispatch loop mirrors into that map — refuses a name whose only planted character is a `U+202E` bidi override, which the byte-level `is_valid_display_name` check admitted because the override encodes as three UTF-8 bytes each above `0x20`; running that refusal through the same three-arm mirror the loop applies leaves the card titled as it was, with no control character or bidi override in any cell. Control: `  café-агент-日本語  ` is still Applied, trimmed.
- **Does not assert:** the other writer of `ui.display_names` — the daemon's `list_agents` echo of `AgentRecord.display_name`, which is scrubbed at the wire boundary and covered by `session/live/007`; the title's char-vs-display-column fit budget (`truncate_styled_segments`, issue #357).
- **Platform coverage:** mac+linux+windows.
