...
##### orchestration/delegate/031 — An AMBIGUOUS delegate task pointer KEEPS its payload record, so the next delegation's identical pointer is refused and reported rather than submitted on top of the leftover bytes.
- **Layer:** L1 (in-process production task-file resolution and pointer composition, the detailed guarded submit path against two real `/bin/cat` PTY panes, and the production settle decision `settle_one_shot_payload_record` driven with each outcome that leaves a record).
- **Agent:** none.
- **Asserts:** after the production pointer (`resolve_delegate_task_body` + `compose_delegate_prompt`, task file written into a real tempdir) is submitted into a worker pane and the settle decision is run with `Ambiguous`, the payload record survives, so once the user types the next delegation's pointer is refused as `RefusedUserInput` — the outcome that publishes a `DeliveryNotice` on the worker's card — instead of appending the leftover pointer bytes to the user's unsent draft and submitting both as one turn. A second pane runs the identical sequence with `Applied` as a control and IS admitted. Separately asserts what makes the hazard guaranteed rather than merely likely here: two different task bodies compose the SAME fixed one-liner, so a later delegation's bytes are byte-identical by construction. `dispatch_one_owned`'s half of issue #715, and the sharper of the two siblings `scheduler/idle-worker/018` deferred.
- **Does not assert:** that a real `Ambiguous` arises from this seam — a `/bin/cat` PTY writer cannot be faulted into a partial write, and the registry state the decision reads is identical either way, since both classification arms call `note_automatic_write` with the same payload. Also not asserted: that `Ambiguous` still sets `dispatch_one_owned`'s `delivered` flag, so the commission stays owed and the silence watch stays armed (unchanged by #715 and deliberately NOT folded into this decision — the flag answers "did this reach the worker?", the record answers "is the input box empty again?"); that the refused later delegation's `DeliveryNotice` and commission release actually fire (`RefusedUserInput`'s own arm, reached here only as an outcome value); and that an `Ambiguous` submit stops STRANDING the pointer bytes in the input box — it does not, and nothing anywhere removes them. What this pins is only that they are no longer merged into a later turn.
- **Platform coverage:** mac+linux.
...
- **Layer:** L1 (in-process production notice composition, the guarded submit path against two real `/bin/cat` PTY panes, and the production settle decision `settle_one_shot_payload_record` driven with each outcome that leaves a record).
- **Agent:** none.
...
- **Does not assert:** that a real `Ambiguous` arises from this seam — a `/bin/cat` PTY writer cannot be faulted into a partial write, and the classification itself is unit-tested against a fault-injecting writer in `agent_pty` (`deliver_payload_classifies_partial_write_as_ambiguous`); the registry state is identical either way, since both classification arms call `note_automatic_write` with the same payload, so only the outcome the daemon acts on varies here. Also not asserted: the trigger that decides to send the report (`orchestration/delegate/013` and `/025`); the sibling one-shot callers, which issue #715 has since narrowed the same way and which are pinned by `scheduler/idle-worker/019` (`arm_idle_worker_watch`'s idle prompt) and `orchestration/delegate/031` (the delegate task pointer).
- **Platform coverage:** mac+linux.

##### scheduler/idle-worker/019 — An AMBIGUOUS idle-worker prompt KEEPS its payload record, so a byte-identical second prompt into the same orchestrator is refused rather than submitted on top of the leftover bytes.
- **Layer:** L1 (in-process production prompt composition, the guarded submit path against two real `/bin/cat` PTY panes, and the production settle decision `settle_one_shot_payload_record` driven with each outcome that leaves a record).
- **Agent:** none.
